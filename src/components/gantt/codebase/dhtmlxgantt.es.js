...
dhtmlxGantt v.9.0.11 Professional Evaluation
...
function Qe(t) {
  t._get_linked_task = function(s, l) {
    var c = null, u = l ? s.target : s.source;
    return t.isTaskExists(u) && (c = t.getTask(u)), c;
  ...
  t._isLinksCacheEnabled = function() {
    ...
    e = {}, i = {}, a = {}, r = {}, n = !1;
  }, t._formatLink = function(s, l, c) {
    if (n && e[s.id]) return e[s.id];
    var u = [], d = this._get_link_target(s), h = this._get_link_source(s);
    if (!h || !d || t.isSummaryTask(d) && t.isChildOf(h.id, d.id) || t.isSummaryTask(h) && t.isChildOf(d.id, h.id)) return u;
    ...
    c || (c = { start_date: d.start_date, end_date: d.end_date }, this.isSummaryTask(d) && !o(d) && ((c = this.getSubtaskDates(d.id)).start_date = c.end_date, this.eachTask(function(S) {
      S.type !== this.config.types.project && !S.$target.length && S.start_date < c.start_date && (c.start_date = S.start_date);
    }, d.id)));
    for (var v = this._getImplicitLinks(s, d, function(S) {
      return !f || _ || S.$target.length || t.getState("tasksDnd").drag_id == S.id ? 0 : t.calculateDuration({ start_date: c.start_date, end_date: S.start_date, task: d });
      ...
      t._linkedTasks[$.target] = t._linkedTasks[$.target] || {}, t._linkedTasks[$.target][$.source] = !0, u.push(t._convertToFinishToStartLink(w.task, $, h, d, m.taskParent, w.taskParent));
    }
    return n && (e[s.id] = u), u;
    ...
    return !0;
  }, t._getImplicitLinks = function(s, l, c, u) {
    var d = [];
    if (this.isSummaryTask(l) && !o(l)) {
      ...
      for (var f in this.eachTask(function(y) {
        ...
        if (t._isAutoSchedulable(k)) {
          ...
          h = !1;
          for (var b = 0; b < v.length && s.type != t.config.links.start_to_start && s.type != t.config.links.start_to_finish; b++) {
          ...
          if (!h) {
            ...
            let w = 0;
            y && (w = c(k)), d.push({ task: k.id, taskParent: k.parent, lag: w, subtaskLink: !0 });
          }
        }
      }
    } else d.push({ task: l.id, taskParent: l.parent, lag: 0 });
    return d;
  }, t._getDirectDependencies = function(s, l) {
    t._linkedTasks = t._linkedTasks || {};
    for (var c = [], u = [], d = l ? s.$source : s.$target, h = 0; h < d.length; h++) {
      var _ = this.getLink(d[h]);
    ...
    for (h = 0; h < c.length; h++) u = u.concat(this._formatLink(c[h]));
    return u;
  }, t._getInheritedDependencies = function(s, l) {
    var c, u = !1, d = [];
    return this.isTaskExists(s.id) && this.eachParent(function(h) {
      var _;
      u || (n && (c = l ? i : a)[h.id] ? d = d.concat(c[h.id]) : this.isSummaryTask(h) && (this._isAutoSchedulable(h) ? (_ = this._getDirectDependencies(h, l), n && (c[h.id] = _), d = d.concat(_)) : u = !0));
    }, s.id, this), d;
    ...
    var c = this._getDirectSuccessors(s);
    return l ? c : c.concat(this._getInheritedSuccessors(s));
  }, t._getPredecessors = function(s, l) {
    var c, u = String(s.id) + "-" + String(l);
    if (n && r[u]) return r[u];
    var d = this._getDirectPredecessors(s);
    return c = l ? d : d.concat(this._getInheritedPredecessors(s)), n && (r[u] = c), c;
  }, t._convertToFinishToStartLink = function(s, l, c, u, d, h) {
    var _ = { target: s, link: t.config.links.finish_to_start, id: l.id, lag: l.lag || 0, sourceLag: 0, targetLag: 0, trueLag: l.lag || 0, source: l.source, preferredStart: null, sourceParent: d, targetParent: h, hashSum: null, subtaskLink: l.subtaskLink }, f = 0;
    switch (String(l.type)) {
      ...
      case String(t.config.links.start_to_finish):
        f = -c.duration - u.duration, _.sourceLag = -c.duration, _.targetLag = -u.duration;
        break;
      default:
        f = 0;
    }
...
function nn() {
  return { getVertices: function(t) {
    ...
    for (var e = {}, i = [], a = this.groupAdjacentEdges(n), r = !1, o = [], s = 0; s < t.length; s++) {
      var l = k(t[s]);
      if (!l.visited) for (var c = [l], u = 0; c.length; ) {
        var d = c.pop();
        d.visited || (d.index = u, d.lowLink = u, u++, i.push(d), d.onStack = !0, d.visited = !0), r = !1, n = a[d.id] || [];
        for (var h = 0; h < n.length; h++) {
          var _ = k(n[h].target);
          if (_.edge = n[h], _.index === void 0) {
            c.push(d), c.push(_), r = !0;
            break;
          }
          _.onStack && (d.lowLink = Math.min(d.lowLink, _.index));
        }
        if (!r) {
          if (d.index == d.lowLink) {
            for (var f = { tasks: [], links: [], linkKeys: [] }; (_ = i.pop()).onStack = !1, f.tasks.push(_.id), _.edge && (f.links.push(_.edge.id), f.linkKeys.push(_.edge.hashSum)), _ != d; ) ;
            o.push(f);
          }
          c.length && (_ = d, (d = c[c.length - 1]).lowLink = Math.min(d.lowLink, _.lowLink));
        }
      }
    }
    return o;
...
function an(t) {
  return { getVirtualRoot: function() {
    ...
    for (var l = [], c = 0; c < o.length; c++) a[r = o[c].hashSum] || (a[r] = !0, l.push(o[c]));
    for (c = 0; c < s.length; c++) a[r = s[c].hashSum] || (a[r] = !0, l.push(s[c]));
    return l;
    ...
    for (var r, o = n === void 0 ? t.config.root_id : n, s = (e = {}, {}), l = [{ from: o, includePredecessors: i, isChild: !1 }]; l.length; ) {
      ...
      if (!e[o = c.from]) {
        ...
        for (var d = this._collectRelations(r, u, i, s), h = 0; h < d.length; h++) {
          var _ = d[h];
          let v = !0;
  ...
  constructor(n) {
    this.isAsapTask = (e) => {
      ...
      if (this._gantt.eachTask((l) => o(l), e), i) for (let l = 0; l < i.length; l++) {
        const c = i[l];
        r[c.target] || (s = this._gantt.getTask(c.target), o(s)), r[c.source] || (s = this._gantt.getTask(c.source), o(s));
      }
...
class Fe {
  ...
  resolveRelationDate(n, e, i) {
    ...
    const s = this._gantt.getTask(n), l = e.predecessors, c = {};
    let u = null;
    ...
    let d = null;
    if (a) if (s.duration) a = this._gantt.getClosestWorkTime({ date: a, dir: "future", task: s }), d = this._gantt.calculateEndDate({ start_date: a, duration: s.duration, task: s });
    else {
      let f = "future";
      ...
      if (c[k.finish_to_finish] !== void 0) {
        const v = l.length === 1;
        let b = !0;
        for (const g in c) if (g != k.finish_to_finish && c[k.finish_to_finish] < c[g]) {
          b = !1;
          break;
        }
        (v || b) && (f = "past");
      }
      a = d = this._gantt.getClosestWorkTime({ date: a, dir: f, task: s });
    }
    const h = i[n], _ = Pt.Create(h);
    return _.link = r, _.task = n, _.start_date = a, _.end_date = d, _.kind = "asap", u && (_.earliestSchedulingStart = u, _.earliestSchedulingEnd = this._gantt.calculateEndDate({ start_date: u, duration: s.duration, task: s })), _;
  }
...
class ri {
  ...
  generatePlan(n, e) {
    const i = this._graphHelper, a = this._gantt, r = this._constraintsHelper, o = this._alapStrategy, s = this._asapStrategy, { orderedIds: l, reversedIds: c, relationsMap: u, plansHash: d } = this.buildWorkCollections(n, e, i);
    let h;
    return this.processConstraints(l, d, a, r), h = a.config.schedule_from_end ? this.iterateTasks(c, l, r.isAlapTask, o, s, u, d) : this.iterateTasks(l, c, r.isAsapTask, s, o, u, d), h;
  }
  applyProjectPlan(n) {
    const e = this._gantt;
    let i, a, r, o;
    ...
    for (let l = 0; l < n.length; l++) {
      ...
      let c = null;
      i.start_date && a.start_date.valueOf() !== i.start_date.valueOf() && (c = i.start_date), c && (a.start_date = c, a.end_date = e.calculateEndDate(a), s.push(a.id), e.callEvent("onAfterTaskAutoSchedule", [a, c, r, o]));
    }
    return s;
  }
  iterateTasks(n, e, i, a, r, o, s) {
    ...
    for (let u = 0; u < n.length; u++) {
      const d = n[u], h = l.getTask(d);
      if (!l._isAutoSchedulable(h)) continue;
      const _ = a.resolveRelationDate(d, o[d], s);
      this.limitPlanDates(h, _), i(h) ? this.processResolvedDate(h, _, c, s) : s[h.id] = _;
    }
    for (let u = 0; u < e.length; u++) {
      const d = e[u], h = l.getTask(d);
      if (l._isAutoSchedulable(h) && !i(h)) {
        const _ = r.resolveRelationDate(d, o[d], s);
        this.limitPlanDates(h, _), this.processResolvedDate(h, _, c, s);
      }
    }
  ...
  summaryLagChanged(n, e, i) {
    ...
    for (const s in a) {
      ...
      const c = n.getTask(s), u = n.calculateDuration({ start_date: c.start_date, end_date: c.end_date, task: c }), d = n.calculateDuration({ start_date: l.min, end_date: l.max, task: c });
      d !== u && (c.start_date = l.min, c.end_date = l.max, c.duration = d);
    }
    for (const s in r) {
      const l = r[s];
      let c, u;
      const d = a[l.source], h = a[l.target];
      d && (c = { start_date: d.start_date, end_date: d.end_date }), h && (c = { start_date: h.start_date, end_date: h.end_date }), n._formatLink(l, c, u).forEach(function(_) {
        for (const f in e) e[f].predecessors.forEach(function(k) {
          const v = k.id === _.id, b = k.target === _.target, g = k.source === _.source;
          v && b && g && (k.lag = _.lag, k.sourceLag = _.sourceLag, k.targetLag = _.targetLag, k.hashSum = _.hashSum);
        });
      }), o = !0;
    }
    return o;
  ...
  getProjectUpdates(n, e, i, a, r, o, s) {
    if (n.getTask(a[r]).type === n.config.types.project) {
      ...
      const l = o[a[r]];
      let c = e[i[r]];
      c && (r != "source" || c.start_date && c.end_date || (c = n.getTask(c.task)), l.min = l.min || c.start_date, l.min > c.start_date && (l.min = c.start_date), l.max = l.max || c.end_date, l.max < c.end_date && (l.max = c.end_date), s[a.id] = a);
    }
  }
  ...
  buildWorkCollections(n, e, i) {
    ...
    for (let c = 0, u = r.length; c < u; c++) {
      const d = r[c], h = a.getTask(d);
      a._isAutoSchedulable(h) && (l[d] = { successors: [], predecessors: [] }, s[d] = null);
    }
    for (let c = 0, u = e.length; c < u; c++) {
      const d = e[c];
      s[d.id] === void 0 && (o.unshift(d.id), r.unshift(d.id), s[d.id] = null, l[d.id] = { successors: [], predecessors: [] });
    }
    for (let c = 0, u = n.length; c < u; c++) {
      const d = n[c];
      l[d.source] && l[d.source].successors.push(d), l[d.target] && l[d.target].predecessors.push(d);
    }
  ...
  processConstraints(n, e, i, a) {
    for (let r = 0; r < n.length; r++) {
      ...
      if (l && l !== U.ASAP && l !== U.ALAP) {
        const c = a.processConstraint(s, Pt.Create());
        e[s.id] = c;
      }
    }
  }
...
function he(t, n, e) {
  ...
  for (; i.length > 0; ) if (s = i.shift(), !e[s]) {
    ...
    for (let u = 0; u < n.length; u++) {
      const d = n[u];
      d.source == s || d.sourceParent == s ? (e[d.target] || (i.push(d.target), o[d.id] = !0, n.splice(u, 1), u--), r[d.hashSum] = d) : d.target != s && d.targetParent != s || (e[d.source] || (i.push(d.source), o[d.id] = !0, n.splice(u, 1), u--), r[d.hashSum] = d);
    }
  }
  const l = [];
  let c = [];
  for (const u in o) l.push(u);
  for (const u in r) c.push(r[u]);
  ...
  constructor(n, e) {
    this.getConnectedGroupRelations = (i) => he(i, this._linksBuilder.getLinkedTasks(), {}).processedLinks, this.getConnectedGroup = (i) => {
      ...
      return function(r) {
        const o = {}, s = [];
        let l, c, u;
        for (let d = 0; d < r.length; d++) if (l = r[d].source, c = r[d].target, u = null, o[l] ? o[c] || (u = c) : u = l, u) {
          const h = r.length;
          s.push(he(u, r, o)), h !== r.length && (d = -1);
        }
        return s;
  ...
  constructor(n, e, i) {
    this.isCircularLink = (a) => !!this.getLoopContainingLink(a), this.getLoopContainingLink = (a) => {
      ...
      for (let u = 0; u < c.length; u++) {
        const d = c[u].links;
        for (let h = 0; h < d.length; h++) if (d[h] == a.id) return c[u];
      }
      return null;
    }, this.findCycles = () => {
...
function li(t, n, e, i) {
  const a = function() {
    ...
    function h(x, $) {
      return !!d(x.start_date, $.start_date, x, $) || t.getConstraintType(x) !== t.getConstraintType($) || !!d(x.constraint_date, $.constraint_date, x, $) || !(!d(x.start_date, $.start_date, x, $) && (!d(x.end_date, $.end_date, x, $) && x.duration === $.duration || x.type === t.config.types.milestone)) || void 0;
    }
    function _(x) {
    ...
    }), t.attachEvent("onBeforeLinkAdd", c), t.attachEvent("onBeforeLinkAdd", u), t.attachEvent("onBeforeLinkUpdate", c), t.attachEvent("onBeforeLinkUpdate", u), t.attachEvent("onBeforeTaskDrag", function(x, $, S) {
    ...
    if (t.ext && t.ext.inlineEditors) {
      ...
      x.attachEvent("onBeforeSave", function(S) {
        if ($[S.columnName]) {
          ...
          const T = S.columnName === "duration", E = t.config.schedule_from_end && S.columnName === "start_date", C = !t.config.schedule_from_end && S.columnName === "end_date", D = t.config.inline_editors_date_processing !== "keepDuration" && (E || C), M = S.columnName === "constraint_date";
          (T || D || M) && (t.getTask(S.id).$keep_constraints = !0);
        }
        return !0;
      });
    }
          ...
          if (t.config.auto_scheduling && !t._autoscheduling_in_progress) {
            ...
            if (h(E, C)) {
              if (g(T, E), t.config.auto_scheduling_move_projects && o == T) {
                let M = !0;
                t.calculateDuration(E) !== t.calculateDuration(C) && (f(T, r), M = !1), D ? t.autoSchedule() : (M && f(T, r), t._autoSchedule(T, r));
...
function K(t) {
  ...
  if (t.getBoundingClientRect) {
    var r = t.getBoundingClientRect(), o = document.body, s = document.documentElement || document.body.parentNode || document.body, l = window.pageYOffset || s.scrollTop || o.scrollTop, c = window.pageXOffset || s.scrollLeft || o.scrollLeft, u = s.clientTop || o.clientTop || 0, d = s.clientLeft || o.clientLeft || 0;
    n = r.top + l - u, e = r.left + c - d, i = document.body.offsetWidth - r.right, a = document.body.offsetHeight - r.bottom;
  } else {
...
function Ut(t) {
  ...
  for (e.sort(function(r, o) {
    ...
    var a = e[i];
    (di(a) || hi(a) || ui(a)) && ci(a) || (e.splice(i, 1), i--);
  }
  return e;
}
function En() {
...
function ct(t, n) {
  var e = document.documentElement, i = K(n);
...
const Nn = Object.freeze(Object.defineProperty({ __proto__: null, addClassName: xt, closest: dt, getActiveElement: De, getChildNodes: An, getClassName: it, getClosestSizedElement: In, getFocusableNodes: Ut, getNodePosition: K, getRelativeEventPosition: ct, getRelativeNodePosition: Mn, getRootNode: Dt, getScrollSize: En, getTargetNode: At, hasClass: function(t, n) {
  return "classList" in t ? t.classList.contains(n) : new RegExp("\\b" + n + "\\b").test(t.className);
}, hasShadowParent: function(t) {
  return !!Dt(t);
}, insertNode: Cn, isChildOf: et, isShadowDomSupported: Ln, locateAttribute: tt, locateClassName: yt, removeClassName: Nt, removeNode: Dn, toNode: We }, Symbol.toStringTag, { value: "Module" })), Y = typeof window < "u" ? window : global;
let _i = class {
  constructor(t) {
  ...
  attach(t, n, e) {
    ...
    this._domEvents.attach(l, "mouseup", (f) => {
      ...
      this._touchStarts = !0, this._longTapTimer = setTimeout(() => {
        this._touchStarts && (d(c(f)), this._touchStarts = !1, this._touchActive = !0);
      ...
      if (this._touchActive) {
        let k = u();
        if (k && i.utils.dom.closest(f.target, k)) return;
    ...
    const u = () => {
      ...
      o = null;
      let k = u();
      k && i.utils.dom.closest(f.target, k) || (r.registerProvider("clickDrag", () => ({ autoscroll: this._mouseDown })), n && f[n] !== !0 || (o = this._getCoordinates(f, t)));
    }, h = (f) => {
      if (o = null, (!n || f[n] === !0) && this._mouseDown === !0) {
        this._mouseDown = !1;
        const k = this._getCoordinates(f, t);
        t.dragEnd(k);
      }
    }, _ = (f) => {
      if (n && f[n] !== !0) return;
      const k = this._gantt.ext.clickDrag, v = (this._gantt.config.drag_timeline || {}).useKey;
      if (k && v && !n && f[v]) return;
      let b = null;
      if (!this._mouseDown && o) return b = this._getCoordinates(f, t), void (Math.abs(o.relative.left - b.relative.left) > 5 && s());
...
function _t(t) {
  ...
  t.attachEvent = function(e, i, a) {
    e = "ev_" + e.toLowerCase(), n.listeners[e] || (n.listeners[e] = function(o) {
      var s = {}, l = 0, c = function() {
        var u = !0;
        for (var d in s) {
          var h = s[d].apply(o, arguments);
          u = u && h;
        }
        return u;
      };
      return c.addEvent = function(u, d) {
        if (typeof u == "function") {
          var h;
          if (d && d.id ? h = d.id : (h = l, l++), d && d.once) {
            var _ = u;
            u = function() {
              _(), c.removeEvent(h);
            };
...
class gi {
  constructor(n, e, i) {
    ...
    this._el = document.createElement("div"), this.defaultRender = (r, o) => {
      ...
      const s = this._el, l = Math.min(r.relative.top, o.relative.top), c = Math.max(r.relative.top, o.relative.top), u = Math.min(r.relative.left, o.relative.left), d = Math.max(r.relative.left, o.relative.left);
      if (this._singleRow) {
        const h = this._getTaskPositionByTop(this._startPoint.relative.top);
        s.style.height = h.height + "px", s.style.top = h.top + "px";
      } else s.style.height = Math.abs(c - l) + "px", s.style.top = l + "px";
      return s.style.width = Math.abs(d - u) + "px", s.style.left = u + "px", s;
...
function fi(t) {
  (function() {
    ...
    function a(c) {
      t.eventRemove(c, "keydown", o), t.event(c, "keydown", o), n.push(c);
    }
    function r() {
      var c = n.pop();
      c && t.eventRemove(c, "keydown", o), i();
    }
    function o(c) {
      var u = c.currentTarget;
      u == n[n.length - 1] && t.$keyboardNavigation.trapFocus(u, c);
    }
    function s() {
      a(t.getLightbox());
    }
...
class pi {
  constructor(n) {
    this.show = (e, i) => {
      ...
      const o = r.querySelector(".gantt_cal_qi_title"), s = o.querySelector(".gantt_cal_qi_tcontent"), l = o.querySelector(".gantt_cal_qi_tdate"), c = r.querySelector(".gantt_cal_qi_content"), u = r.querySelector(".gantt_cal_qi_controls");
      i._waiAria.quickInfoHeader(r, [e.header.title, e.header.date].join(" ")), s.innerHTML = e.header.title, l.innerHTML = e.header.date, e.header.title || e.header.date ? o.style.display = "" : o.style.display = "none", c.innerHTML = e.content;
      const d = e.buttons;
      d.length ? u.style.display = "" : u.style.display = "none";
      let h = "";
      for (let _ = 0; _ < d.length; _++) {
        const f = i._waiAria.quickInfoButtonAttrString(i.locale.labels[d[_]]);
        h += `<div class="gantt_qi_big_icon ${d[_]} dhx_gantt_${d[_]}" title="${i.locale.labels[d[_]]}" ${f}>
            <div class='dhx_menu_icon dhx_gantt_icon ${d[_]} gantt_menu_icon dhx_gantt_${d[_]}'></div>
            <div>${i.locale.labels[d[_]]}</div>
         </div>`;
      }
  ...
  _get_event_counter_part(n, e, i, a) {
    ...
    let c = o;
    if (r.utils.dom.isChildOf(c, i)) for (; c && c !== i; ) s += c.offsetLeft, c = c.offsetParent;
    const u = r.getScrollState();
    return c ? { left: s, top: l, dx: s + o.offsetWidth / 2 - u.x > i.offsetWidth / 2 ? 1 : 0, dy: l + o.offsetHeight / 2 - u.y > a.offsetHeight / 2 ? 1 : 0, width: o.offsetWidth, height: o.offsetHeight } : null;
  }
  ...
  _show_quick_info(n, e) {
    ...
    if (i.config.quick_info_detached) {
      ...
      const o = a.offsetWidth, s = a.offsetHeight, l = i.getScrollState(), c = r.xViewport, u = r.yViewport, d = c.offsetWidth + l.x - o, h = n.top - l.y + s;
      let _ = n.top;
      h > u.offsetHeight / 2 && (_ = n.top - (s + n.height + 2 * e), _ < l.y && h <= u.offsetHeight && (_ = n.top)), _ < l.y && (_ = l.y);
      const f = Math.min(Math.max(l.x, n.left - n.dx * (o - n.width)), d), k = _;
...
var ge, mi = {}.constructor.toString();
function X(t) {
  var n, e;
...
const On = Object.freeze(Object.defineProperty({ __proto__: null, bind: B, copy: X, defined: G, event: Rn, eventRemove: Hn, mixin: R, uid: ut }, Symbol.toStringTag, { value: "Module" }));
function Ve(t, n) {
  t = t || Rn, n = n || Hn;
  var e = [], i = { attach: function(a, r, o, s) {
    ...
    for (var l = 0; l < e.length; l++) {
      var c = e[l];
      c.element === a && c.event === r && c.callback === o && c.capture === s && (e.splice(l, 1), l--);
    }
  }, detachAll: function() {
...
class vi {
  ...
  _calculateTooltipPosition(n) {
    const e = this._gantt, i = this._getViewPortSize(), a = this.getNode(), r = { top: 0, left: 0, width: a.offsetWidth, height: a.offsetHeight, bottom: 0, right: 0 }, o = e.config.tooltip_offset_x, s = e.config.tooltip_offset_y, l = document.body, c = ct(n, l), u = K(l);
    c.y += u.y, r.top = c.y, r.left = c.x, r.top += s, r.left += o, r.bottom = r.top + r.height, r.right = r.left + r.width;
    const d = window.scrollY + l.scrollTop;
    return r.top < i.top - d ? (r.top = i.top, r.bottom = r.top + r.height) : r.bottom > i.bottom && (r.bottom = i.bottom, r.top = r.bottom - r.height), r.left < i.left ? (r.left = i.left, r.right = i.left + r.width) : r.right > i.right && (r.right = i.right, r.left = r.right - r.width), c.x >= r.left && c.x <= r.right && (r.left = c.x - r.width - o, r.right = r.left + r.width), c.y >= r.top && c.y <= r.bottom && (r.top = c.y - r.height - s, r.bottom = r.top + r.height), r;
  }
  _getViewPortSize() {
    ...
    return e === n.$task_data ? (a = n.$task, r = 0, o = 0, i = K(n.$task)) : i = K(a), { left: i.x + o, top: i.y + r, width: i.width, height: i.height, bottom: i.y + i.height + r, right: i.x + i.width + o };
  }
}
class ki {
  constructor(n) {
    this._listeners = {}, this.tooltip = new vi(n), this._gantt = n, this._domEvents = Ve(), this._initDelayedFunctions();
  }
  destructor() {
  ...
  attach(n) {
    ...
    let a = null;
    const r = (o) => {
      const s = At(o), l = dt(s, n.selector);
      if (et(s, this.tooltip.getNode())) return;
      const c = () => {
        a = l, n.onmouseenter(o, l);
      };
      a ? l && l === a ? n.onmousemove(o, l) : (n.onmouseleave(o, a), a = null, l && l !== a && c()) : l && c();
    };
...
const rn = { onBeforeUndo: "onAfterUndo", onBeforeRedo: "onAfterRedo" }, sn = ["onTaskDragStart", "onAfterTaskUpdate", "onAfterParentExpand", "onAfterTaskDelete", "onBeforeBatchUpdate"];
class yi {
  constructor(n, e) {
  ...
  setNestedTasks(n, e) {
    ...
    for (let c = 0; c < e.length; c++) a = this.setInitialTask(e[c]), o = o.concat(this._getLinks(a)), r.push(a);
    const s = {};
    for (let c = 0; c < o.length; c++) s[o[c]] = !0;
    const l = [];
    for (const c in s) l.push(this.setInitialLink(c));
  ...
  _attachEvents() {
    ...
    for (const l in rn) e.attachEvent(l, () => (this.startIgnore(), !0)), e.attachEvent(rn[l], () => (this.stopIgnore(), !0));
    for (let l = 0; l < sn.length; l++) e.attachEvent(sn[l], () => (this.startBatchAction(), !0));
    ...
    r.attachEvent("onBeforeItemMove", (l, c, u) => (this.isMoveEventsIgnored() || i(), !0)), r.attachEvent("onAfterItemMove", (l, c, u) => (this.isMoveEventsIgnored() || this.onTaskMoved(a(l)), !0)), e.attachEvent("onRowDragStart", (l, c, u) => (this.toggleIgnoreMoveEvents(!0), i(), !0));
    let o = null, s = !1;
    if (e.attachEvent("onBeforeTaskDrag", (l) => {
      if (o = e.getState().drag_id, o === l) {
        const c = e.getTask(l);
        e.isSummaryTask(c) && e.config.drag_project && (s = !0);
      }
      ...
      e.attachEvent("onGanttLayoutReady", () => {
        l && e.ext.inlineEditors.detachEvent(l), c && e.ext.inlineEditors.detachEvent(c), c = e.ext.inlineEditors.attachEvent("onEditStart", (u) => {
  ...
  _applyAction(n) {
    ...
    o[i.task] = { add: "addTask", get: "getTask", update: "updateTask", remove: "deleteTask", move: "moveTask", isExists: "isTaskExists" }, o[i.link] = { add: "addLink", get: "getLink", update: "updateLink", remove: "deleteLink", isExists: "isLinkExists" }, r.batchUpdate(function() {
      for (let s = 0; s < n.commands.length; s++) {
        e = n.commands[s];
        const l = o[e.entity][e.type], c = o[e.entity].get, u = o[e.entity].isExists;
        if (e.type === a.add) r[l](e.oldValue, e.oldValue.parent, e.oldValue.$local_index);
          ...
          const d = r[c](e.value.id);
          for (const h in e.value) {
            let _ = !(h.startsWith("$") || h.startsWith("_"));
            ["$open"].indexOf(h) > -1 && (_ = !0), _ && (d[h] = e.value[h]);
          }
          r[l](e.value.id);
...
const $i = { auto_scheduling: function(t) {
  Ze(t), Qe(t);
  var n = an(t), e = nn(), i = ze.Create(t), a = new ri(t, e, i), r = new si(t, n), o = new oi(t, e, n);
  ...
  t.attachEvent("onParse", function() {
    ...
    if (t.callEvent("onBeforeAutoSchedule", [l]) !== !1) {
      ...
      var u = i.getConstraints(l, t.isTaskExists(l) ? c : null), d = [], h = e.findLoops(c);
      if (h.length) t.callEvent("onAutoScheduleCircularLink", [h]);
        ...
        var _ = a.generatePlan(c, u);
        (function(f) {
          f.length && t.batchUpdate(function() {
            for (var k = 0; k < f.length; k++) t.updateTask(f[k]);
          }, s);
        })(d = a.applyProjectPlan(_));
      }
      t._autoscheduling_in_progress = !1, t.callEvent("onAfterAutoSchedule", [l, d]);
    }
  }, t.autoSchedule = function(l, c) {
    var u;
    c = c === void 0 || !!c, u = l !== void 0 ? t.config.auto_scheduling_compatibility ? n.getLinkedTasks(l, c) : r.getConnectedGroupRelations(l) : n.getLinkedTasks(), t._autoSchedule(l, u);
  ...
  function e() {
    ...
    t.ext.clickDrag && t.ext.clickDrag.destructor(), t.ext.clickDrag = new _i(t);
  ...
  Ze(t), Qe(t);
  var n = function(i) {
    var a = an(i), r = nn(), o = { _freeSlack: {}, _totalSlack: {}, _slackNeedCalculate: !0, _linkedTasksById: {}, _successorsByTaskId: {}, _projectEnd: null, _calculateSlacks: function() {
      ...
      if (l.length) {
        ...
        for (var u = 0; u < s.length; u++) s[u].hashSum in c && (s.splice(u, 1), u--);
      }
      const d = r.topologicalSort(s).reverse(), h = {};
      s.forEach((f) => {
      ...
      this._projectEnd = i.getSubtaskDates().end_date, this._calculateFreeSlack(s, d, h, _), this._calculateTotalSlack(s, d, h, _);
    }, _isCompletedTask: function(s) {
      return i.config.auto_scheduling_use_progress && s.progress == 1;
    }, _calculateFreeSlack: function(s, l, c, u) {
      const d = this._freeSlack = {}, h = {};
      ...
      s.forEach((f) => {
        ...
        v -= f.lag || 0, d[f.source] !== void 0 ? d[f.source] = Math.min(v, d[f.source]) : d[f.source] = v;
      });
      for (const f in h) {
        if (_[f]) continue;
        const k = h[f];
        this._isCompletedTask(k) || k.unscheduled ? d[k.id] = 0 : d[k.id] = i.calculateDuration({ start_date: k.end_date, end_date: this._projectEnd, task: k });
      }
    ...
    }, _calculateTotalSlack: function(s, l, c, u) {
      this._totalSlack = {}, this._slackNeedCalculate = !1;
      for (var d = {}, h = i.getTaskByTime(), _ = 0; _ < l.length; _++) {
        const k = i.getTask(l[_]);
        if (this._isCompletedTask(k)) d[k.id] = 0;
        else if (c[k.id] || i.isSummaryTask(k)) {
          const v = c[k.id].linked;
          let b = null;
          for (var f = 0; f < v.length; f++) {
            const g = v[f], m = i.getTask(g.target);
            let p = 0;
            d[m.id] !== void 0 && (p += d[m.id]), p += u.getDist(k, m), p -= g.link.lag || 0, b = b === null ? p : Math.min(b, p);
          }
          d[k.id] = b || 0;
        } else d[k.id] = this.getFreeSlack(k);
      }
      return h.forEach((function(k) {
        d[k.id] !== void 0 || i.isSummaryTask(k) || (d[k.id] = this.getFreeSlack(k));
      }).bind(this)), this._totalSlack = d, this._totalSlack;
      ...
      if (this._totalSlack[l] === void 0) {
        if (i.isSummaryTask(i.getTask(l))) {
          ...
          return i.eachTask((function(u) {
            var d = this._totalSlack[u.id];
            d !== void 0 && (c === null || d < c) && (c = d);
          }).bind(this), l), this._totalSlack[l] = c !== null ? c : i.calculateDuration({ start_date: s.end_date, end_date: this._projectEnd, task: s }), this._totalSlack[l];
        }
        return 0;
      }
  ...
  var e = function(i) {
    return i._isProjectEnd = function(a) {
      ...
      i.attachEvent("onAfterLinkAdd", s), i.attachEvent("onAfterLinkUpdate", s), i.attachEvent("onAfterLinkDelete", s), i.attachEvent("onAfterTaskAdd", s), i.attachEvent("onTaskIdChange", function(l, c) {
  ...
  t.config.highlight_critical_path = !1, e.init(n), t.isCriticalTask = function(i) {
    ...
    if (o[0]) for (l = 0; l < o.length; l++) {
      var c = this.getLink(o[l]), u = this._getSlack(i, a, this._convertToFinishToStartLink(c.id, c, i, a, i.parent, a.parent));
      (r > u || l === 0) && (r = u);
    }
    ...
    c = +s > +l ? -this.calculateDuration({ start_date: l, end_date: s, task: i }) : this.calculateDuration({ start_date: s, end_date: l, task: i });
    var u = r.lag;
    return u && 1 * u == u && (c -= u), c;
  };
  ...
  let r = null;
  function o(u, d) {
    d.width = u.width, d.height = u.height, d.top = u.top, d.left = u.left, d.position = u.position, d.zIndex = u.zIndex;
  }
  let s = !1;
  function l() {
    ...
    n() ? s && (u = "onExpand", function() {
      const d = t.ext.fullscreen.getFullscreenElement(), h = document.body;
      o(d.style, a), i = { overflow: h.style.overflow, padding: h.style.padding ? h.style.padding : null, paddingTop: h.style.paddingTop ? h.style.paddingTop : null, paddingRight: h.style.paddingRight ? h.style.paddingRight : null, paddingBottom: h.style.paddingBottom ? h.style.paddingBottom : null, paddingLeft: h.style.paddingLeft ? h.style.paddingLeft : null }, h.style.padding && (h.style.padding = "0"), h.style.paddingTop && (h.style.paddingTop = "0"), h.style.paddingRight && (h.style.paddingRight = "0"), h.style.paddingBottom && (h.style.paddingBottom = "0"), h.style.paddingLeft && (h.style.paddingLeft = "0"), h.style.overflow = "hidden", d.style.width = "100vw", d.style.height = "100vh", d.style.top = "0px", d.style.left = "0px", d.style.position = "absolute", d.style.zIndex = 1, a.modified = !0, r = function(_) {
        let f = _.parentNode;
        const k = [];
      ...
      const d = t.ext.fullscreen.getFullscreenElement(), h = document.body;
      a.modified && (i.padding && (h.style.padding = i.padding), i.paddingTop && (h.style.paddingTop = i.paddingTop), i.paddingRight && (h.style.paddingRight = i.paddingRight), i.paddingBottom && (h.style.paddingBottom = i.paddingBottom), i.paddingLeft && (h.style.paddingLeft = i.paddingLeft), h.style.overflow = i.overflow, i = { overflow: null, padding: null, paddingTop: null, paddingRight: null, paddingBottom: null, paddingLeft: null }, o(a, d.style), a.modified = !1), r.forEach((_) => {
  ...
  t.ext.fullscreen = { expand() {
    ...
    s = !0;
    const u = document.body, d = u.webkitRequestFullscreen ? [Element.ALLOW_KEYBOARD_INPUT] : [], h = u.msRequestFullscreen || u.mozRequestFullScreen || u.webkitRequestFullscreen || u.requestFullscreen;
    h && h.apply(u, d);
  }, collapse() {
    ...
    n.config.keyboard_navigation = !0, n.config.keyboard_navigation_cells = !1, n.$keyboardNavigation = {}, n._compose = function() {
      ...
      e.$keyboardNavigation.shortcuts = { createCommand: function() {
        ...
        for (var a = [], r = this.getExpressions(this.trim(i)), o = 0; o < r.length; o++) {
          for (var s = this.getWords(r[o]), l = this.createCommand(), c = 0; c < s.length; c++) this.commandKeys[s[c]] ? l.modifiers[s[c]] = !0 : this.specialKeys[s[c]] ? l.keyCode = this.specialKeys[s[c]] : l.keyCode = s[c].charCodeAt(0);
          a.push(l);
        }
        return a;
      ...
      e.$keyboardNavigation.getFocusableNodes = Ut, e.$keyboardNavigation.trapFocus = function(i, a) {
        ...
        for (var r = e.$keyboardNavigation.getFocusableNodes(i), o = De(), s = -1, l = 0; l < r.length; l++) if (r[l] == o) {
          s = l;
          break;
        }
        if (a.shiftKey) {
      ...
      e.$keyboardNavigation.TaskRow = function(i) {
        ...
        if (!i) {
          ...
          let s, l;
          s = e.$task ? e.$task.offsetWidth : o.inner_width, l = e.$grid_data || e.$task_data ? (e.$grid_data || e.$task_data).offsetHeight : o.inner_height;
          const c = e.config.show_chart && e.$ui.getView("timeline");
          a.top < o.y || a.top + r > o.y + l ? e.scrollTo(null, a.top - 20) : e.config.scroll_on_click && c && (a.left > o.x + s ? e.scrollTo(a.left - e.config.task_scroll_offset) : a.left + a.width < o.x && e.scrollTo(a.left + a.width - e.config.task_scroll_offset));
        }
        e.$keyboardNavigation.KeyNavNode.prototype.focus.apply(this, [i]), function() {
          var a = e.$ui.getView("grid");
          if (a && a.$grid_data) {
            var r = parseInt(a.$grid.scrollLeft), o = parseInt(a.$grid_data.scrollTop), s = a.$config.scrollX;
      ...
      function l(v) {
        ...
        if (!s(v)) {
          ...
          if (g && (e.activeNode instanceof n.$keyboardNavigation.TaskCell && et(v.target, n.$task) && (g = new n.$keyboardNavigation.TaskCell(g.taskId, e.activeNode.columnIndex)), b = g, n.config.show_grid && n.$ui.getView("grid") && n.config.keyboard_navigation_cells)) {
            const m = v.target.classList.contains("gantt_row"), p = v.target.closest(".gantt_task_line"), y = n.utils.dom.getNodePosition(n.$grid).x, w = y + n.$grid.offsetWidth, x = n.utils.dom.getNodePosition(document.activeElement).x;
            if (m || p && (x < y || w < x)) {
      ...
      var c = n.attachEvent("onGanttReady", function() {
        if (n.detachEvent(c), n.$data.tasksStore.attachEvent("onStoreUpdated", function(b) {
          if (n.config.keyboard_navigation && e.isEnabled()) {
            ...
            const p = m.getItemTop(b), y = m.$grid_data.scrollTop, w = y + m.$grid_data.getBoundingClientRect().height;
      ...
      n.attachEvent("onTaskCreated", function(v) {
        ...
        if (e.isEnabled()) {
          if (v == u && (d = !0, setTimeout(() => {
            d = !1, u = null;
          })), d && b.type == n.config.types.placeholder) return;
      ...
      var k = {};
      _t(k), n.mixin(k, { addShortcut: function(v, b, g) {
        var m = _(g);
  ...
  t.ext || (t.ext = {}), t.ext.quickInfo = new pi(t), t.config.quickinfo_buttons = ["icon_edit", "icon_delete"], t.config.quick_info_detached = !0, t.config.show_quick_info = !0, t.templates.quick_info_title = function(a, r, o) {
    ...
    const o = t.utils.dom.closest(r.target, ".gantt_add"), s = t.utils.dom.closest(r.target, ".gantt_close"), l = t.utils.dom.closest(r.target, ".gantt_open");
    return !o && !s && !l && setTimeout(function() {
  ...
  function a(u, d, h) {
    u && (i(u.id, d) && (u.id = h), i(u.parent, d) && (u.parent = h));
  }
  function r(u, d, h) {
    a(u.value, d, h), a(u.oldValue, d, h);
  }
  function o(u, d, h) {
    u && (i(u.source, d) && (u.source = h), i(u.target, d) && (u.target = h));
  }
  function s(u, d, h) {
    o(u.value, d, h), o(u.oldValue, d, h);
  }
  function l(u, d, h) {
    const _ = n;
    for (let f = 0; f < u.length; f++) {
      const k = u[f];
      for (let v = 0; v < k.commands.length; v++) k.commands[v].entity === _.command.entity.task ? r(k.commands[v], d, h) : k.commands[v].entity === _.command.entity.link && s(k.commands[v], d, h);
    }
  }
  function c(u, d, h) {
    const _ = n;
    for (let f = 0; f < u.length; f++) {
      const k = u[f];
      for (let v = 0; v < k.commands.length; v++) {
        const b = k.commands[v];
        b.entity === _.command.entity.link && (b.value && b.value.id === d && (b.value.id = h), b.oldValue && b.oldValue.id === d && (b.oldValue.id = h));
      }
    }
  }
  t.config.undo = !0, t.config.redo = !0, t.config.undo_types = { link: "link", task: "task" }, t.config.undo_actions = { update: "update", remove: "remove", add: "add", move: "move" }, t.ext || (t.ext = {}), t.ext.undo = { undo: () => n.undo(), redo: () => n.redo(), getUndoStack: () => n.getUndoStack(), setUndoStack: (u) => n.setUndoStack(u), getRedoStack: () => n.getRedoStack(), setRedoStack: (u) => n.setRedoStack(u), clearUndoStack: () => n.clearUndoStack(), clearRedoStack: () => n.clearRedoStack(), saveState: (u, d) => e.store(u, d, !0), getInitialState: (u, d) => d === t.config.undo_types.link ? e.getInitialLink(u) : e.getInitialTask(u) }, t.undo = t.ext.undo.undo, t.redo = t.ext.undo.redo, t.getUndoStack = t.ext.undo.getUndoStack, t.getRedoStack = t.ext.undo.getRedoStack, t.clearUndoStack = t.ext.undo.clearUndoStack, t.clearRedoStack = t.ext.undo.clearRedoStack, t.attachEvent("onTaskIdChange", (u, d) => {
    const h = n;
    l(h.getUndoStack(), u, d), l(h.getRedoStack(), u, d);
  }), t.attachEvent("onLinkIdChange", (u, d) => {
    const h = n;
    c(h.getUndoStack(), u, d), c(h.getRedoStack(), u, d);
  }), t.attachEvent("onGanttReady", () => {
    n.updateConfigs();
  });
}, grouping: function(t) {
  function n(s, l, c) {
    if (!s || Array.isArray(c) && !c[0]) return 0;
    if (s && !Array.isArray(c)) {
      const d = [];
      return s.map(function(h) {
        d.push({ resource_id: h, value: 8 });
      }), d;
    }
    if (c[0].resource_id || (c = [{ resource_id: c, value: 8 }]), typeof s == "string" && (s = s.split(",")), s.length == 1) return c[0].resource_id = s[0], [c[0]];
    const u = [];
    s.length > 1 && (s = [...new Set(s)]);
    for (let d = 0; d < s.length; d++) {
      let h = s[d], _ = c.map(function(f) {
        return f.resource_id;
      }).reduce(function(f, k, v) {
        return k === h && f.push(v), f;
      }, []);
      if (_.length > 0) _.forEach((f) => {
        c[f].resource_id = h, u.push(c[f]);
      });
      else {
        let f = t.copy(c[0]);
  ...
  function o() {
    ...
    var l = Qt(function() {
      ...
      if (s._groups.regroup) {
        ...
        s._groups.regroup(), c && t.scrollTo(c.x, c.y);
      }
      return !0;
    });
    this.$data.tasksStore.attachEvent("onAfterUpdate", function() {
      ...
      l.$pending || s._groups.is_active() && l();
    });
  }
  t._groups = { relation_property: null, relation_id_property: "$group_id", group_id: null, group_text: null, loading: !1, loaded: 0, dynamicGroups: !1, set_relation_value: void 0, _searchCache: null, init: function(s) {
    ...
    var c = s.$data.tasksStore.getParent;
    this._searchCache = null, s.attachEvent("onBeforeTaskMove", function(d, h, _) {
      ...
      if (this._groups.save_tree_structure && s.isTaskExists(v.parent) && s.isTaskExists(h)) {
        ...
        let m = !1, p = g;
        for (; p; ) d == p.parent && (m = !0), p = s.isTaskExists(p.parent) ? s.getTask(p.parent) : null;
        if (m) return !1;
      }
      return !0;
    }), s.attachEvent("onRowDragStart", function(d, h) {
      var _ = s.getTask(d);
    ...
    }), s.attachEvent("onRowDragEnd", function(d, h) {
    ...
    }), s.$data.tasksStore._listenerToDrop = s.$data.tasksStore.attachEvent("onStoreUpdated", s.bind(o, s)), s.$data.tasksStore.getParent = function(d) {
      return l.is_active() ? l.get_parent(s, d) : c.apply(this, arguments);
    };
    var u = s.$data.tasksStore.setParent;
    s.$data.tasksStore.setParent = function(d, h) {
      ...
      if (l.set_relation_value instanceof Function && s.isTaskExists(h)) {
        ...
        d[l.group_id] === void 0 && (d[l.group_id] = _), l.save_tree_structure && d[l.group_id] != _ && (d[l.group_id] = _), _ && (_ = typeof _ == "string" ? _.split(",") : [_]), d[l.relation_property] = l.set_relation_value(_, d.id, d[l.relation_property]) || _;
      } else if (s.isTaskExists(h)) {
        var v = s.getTask(h);
        l.dynamicGroups || (v.$virtual ? d[l.relation_property] = v[l.relation_id_property] : d[l.relation_property] = v[l.relation_property]), this._setParentInner.apply(this, arguments);
      } else l.dynamicGroups && (d[l.group_id] === void 0 || !d.$virtual && d[l.relation_property][0] === [][0]) && (d[l.relation_property] !== l.group_id ? d[l.relation_property] = d[l.relation_property] || 0 : d[l.relation_property] = 0);
      return s.isTaskExists(h) && (d.$rendered_parent = h, !s.getTask(h).$virtual) ? u.apply(this, arguments) || h : void 0;
    }, s.attachEvent("onBeforeTaskDisplay", function(d, h) {
      ...
      return l.is_active() && (l.loaded--, l.loaded <= 0 && (l.loading = !1, l._clearCache(), s.eachTask(s.bind(function(d) {
        this.get_parent(s, d);
      }, l)))), !0;
    ...
    var d = s.config.root_id;
    if (!this.loading && u !== void 0) {
      this._searchCache || this._buildCache();
      var h = this._searchCache[u];
      s.isTaskExists(h) && h != l.id && (d = this._searchCache[u]), this._groups_pull[u] = d;
    }
    return d;
  }, clear: function() {
    ...
    for (var c = [], u = 0; u < s.length; u++) {
      var d = t.copy(s[u]);
      d.type = l, d.open === void 0 && (d.open = !0), d.$virtual = !0, d.readonly = !0, d[this.relation_id_property] = d[this.group_id], d.text = d[this.group_text], c.push(d);
    }
    return c;
  }, clear_temp_tasks: function(s) {
    for (var l = 0; l < s.length; l++) s[l].$virtual && (s.splice(l, 1), l--);
  }, generate_data: function(s, l) {
    var c = s.getLinks(), u = s.getTaskByTime();
    this.clear_temp_tasks(u), u.forEach(function(_) {
      _.$calculate_duration = !1;
    });
    var d = [];
    this.is_active() && l && l.length && (d = this.generate_sections(l, s.config.types.project));
    var h = { links: c };
    return h.data = d.concat(u), h;
  }, update_settings: function(s, l, c) {
    this.clear(), this.relation_property = s, this.group_id = l, this.group_text = c;
  }, group_tasks: function(s, l, c, u, d) {
    this.update_settings(c, u, d);
    ...
    var d = s.relation_property || null, h = s.group_id || "key", _ = s.group_text || "label";
    this._groups.regroup = function() {
      ...
      var b = function(g, m, p) {
        ...
        return y = g.groups ? p._groups.dynamicGroups ? function(w, x) {
          var $ = {}, S = [], T = {}, E = x.relation_property, C = x.delimiter || ",", D = !1, M = 0;
          ht(x.groups, function(N) {
            N.default && (D = !0, M = N.group_id), T[N.key || N[x.group_id]] = N;
          });
          for (var I = 0; I < w.length; I++) {
            var A, L, P = w[I][E];
            if (Lt(P)) if (P.length > 0) A = i(P), L = P.map(function(N, z) {
              var O;
              return O = N && typeof N == "object" ? N.resource_id : N, (N = T[O]).label || N.text;
            }).sort(), L = [...new Set(L)].join(C);
            else {
              if (D) continue;
              A = 0, L = x.default_group_label;
            }
            else if (P) L = T[A = P].label || T[A].text;
            else {
              if (D) continue;
              A = 0, L = x.default_group_label;
            }
            A !== void 0 && $[A] === void 0 && ($[A] = { key: A, label: L }, A === M && ($[A].default = !0), $[A][x.group_text] = L, $[A][x.group_id] = A);
          }
          return (S = function(N) {
            var z = [];
            for (var O in N) N.hasOwnProperty(O) && z.push(N[O]);
            return z;
          }($)).forEach(function(N) {
            N.key == M && (N.default = !0);
          }), S;
      ...
      }), l._groups.group_tasks(l, b, d, h, _), !0;
  ...
  function n(i) {
    ...
    if (t.config.timeline_placeholder && t.$task_data && (l = t.$task_data.scrollHeight + "px"), r.style.height = l, i.end_date) {
      ...
      r.style.width = Math.max(c - s, 0) + "px";
    }
    if (i.text) {
      let u = null;
  ...
  t.config.multiselect = !0, t.config.multiselect_one_level = !1, t._multiselect = { _selected: {}, _one_level: !1, _active: !0, _first_selected_when_shift: null, getDefaultSelected: function() {
    ...
    var i = this.getSelected(), a = this.getFirstSelected(), r = !1, o = this.getLastSelected(), s = t.config.multiselect, l = (function() {
      ...
      for (var d = 0; d < i.length; d++) i[d] !== e && this.unselect(i[d], n);
    }).bind(this), c = (function() {
      if (o) {
        if (e) {
          var u = t.getGlobalTaskIndex(this.getFirstSelected()), d = t.getGlobalTaskIndex(e), h = t.getGlobalTaskIndex(o);
          u != -1 && h != -1 || (u = d, this.reset());
          for (var _ = o; t.getGlobalTaskIndex(_) !== u; ) this.unselect(_, n), _ = u > h ? t.getNext(_) : t.getPrev(_);
          for (_ = e; t.getGlobalTaskIndex(_) !== u; ) this.select(_, n) && !r && (r = !0, a = _), _ = u > d ? t.getNext(_) : t.getPrev(_);
        }
      } else o = e;
    }).bind(this);
    return s && (n.ctrlKey || n.metaKey) ? (this.isSelected(e) || this.setFirstSelected(e), e && this.toggle(e, n)) : s && n.shiftKey ? (t.isTaskExists(this.getFirstSelected()) && this.getFirstSelected() !== null || this.setFirstSelected(e), i.length ? c() : l()) : l(), this.isSelected(e) ? this.setLastSelected(e) : a ? e == o && this.setLastSelected(n.shiftKey ? a : this.getDefaultSelected()) : this.setLastSelected(null), this.getSelected().length || this.setLastSelected(null), this.getLastSelected() && this.isSelected(this.getFirstSelected()) || this.setFirstSelected(this.getLastSelected()), !0;
  } }, function() {
    var n = t.selectTask;
  ...
  t.attachEvent("onBeforeGanttRender", function() {
    ...
    return l = l || t.uid(), n[l] = function(c, u) {
      var d = document.createElement("div");
      return d.setAttribute("data-overlay-id", c), d.className = "gantt_overlay", d.style.display = "none", { id: c, render: u, isVisible: !1, isAttached: !1, node: d };
    }(l, s), l;
  ...
  return t.ext = t.ext || {}, t.ext.export_api = t.ext.export_api || { _apiUrl: "https://export.dhtmlx.com/gantt", _preparePDFConfigRaw(n, e) {
      ...
      if (i) {
        ...
        for (let o = 0; o < e.data.length; o++) {
          ...
          for (let l = 0; l < r.length; l++) {
            const c = i(s, r[l]);
            c && e.data[o].styles.push({ index: l, styles: t.ext.export_api._getStyles(c) });
          }
        }
      }
    }
    ...
    for (let o = 0; o < i.length; o++) {
      ...
      const d = u.format || u.template || (u.date ? t.date.date_to_str(u.date) : t.config.date_scale);
      for (let h = 0; h < u.trace_x.length; h++) {
        const _ = u.trace_x[h];
        l = s + Math.round(u.width[h] / a);
        const f = { text: d(_), start: s, end: l, styles: "" };
        if (n.cellColors) {
    ...
    for (let c = 0; c < s.length; c++) {
      if (r[s[c]]) continue;
      const u = e[s[c]], d = i[u._target];
      l[d.id] && t.ext.export_api._onCircDependencyFind(u, e, a, r), l[d.id] = !0, t.ext.export_api._clearCircDependencies(d, e, i, a, r, u);
    }
    a[n.id] = !1;
...
const xi = { KEY_CODES: { UP: 38, DOWN: 40, LEFT: 37, RIGHT: 39, SPACE: 32, ENTER: 13, DELETE: 46, ESC: 27, TAB: 9 } };
...
function Mt(t, n) {
  ...
  if (n.length === 1) return typeof n[0] == "string" ? (e.url = n[0], e.async = !0) : (e.url = n[0].url, e.async = n[0].async || !0, e.callback = n[0].callback, e.headers = n[0].headers), n[0].data ? typeof n[0].data != "string" ? e.data = on(n[0].data) : e.data = n[0].data : e.data = "", e;
  switch (e.url = n[0], t) {
    case "GET":
    case "DELETE":
    ...
    case "POST":
    case "PUT":
      n[1] ? typeof n[1] != "string" ? e.data = on(n[1]) : e.data = n[1] : e.data = "", e.callback = n[2], e.headers = n[3];
  }
  return e;
}
const ln = { date_to_str: (t, n, e) => {
...
} }, cn = { date_to_str: (t, n, e) => (i) => t.replace(/%[a-zA-Z]/g, (a) => {
  switch (a) {
    case "%d":
...
function wi(t) {
  ...
  var i = { init: function() {
    ...
    var s = r >= 0, l = !o.getHours() && a.getHours(), c = a.getDate() <= o.getDate() || a.getMonth() < o.getMonth() || a.getFullYear() < o.getFullYear();
    return s && l && c && a.setTime(a.getTime() + 36e5 * (24 - a.getHours())), r > 1 && l && a.setHours(0), a;
  }, add: function(a, r, o) {
    ...
    var o = ln;
    return e() && (o = cn), o.date_to_str(a, r, t);
  }, str_to_date: function(a, r) {
    var o = ln;
    return e() && (o = cn), o.str_to_date(a, r, t);
    ...
    var l = s.getFullYear(), c = Math.round((s.getTime() - new Date(l, 0, 1).getTime()) / 864e5);
...
var Ue = function(t) {
  var n;
  ot.apply(this, [t]), this._branches = {}, this.pull = {}, this.$initItem = function(s) {
    var l = s;
    t.initItem && (l = t.initItem(l));
    var c = this.getItem(s.id);
  ...
  return this._attachDataChange(function() {
    ...
    var u = r[l.id];
    return o && (u && i[l.id] && !e[l.id] && (u = !!c), i[l.id] && !e[l.id] && (l.$split_subtask = !0)), l.$expanded_branch = !!r[l.id], !!u;
...
Ue.prototype = R({ _buildTree: function(t) {
  ...
  if (!gt(t, e)) {
    ...
    if (n == -1 && (n = s.length + 1), gt(o, e) && this.getBranchIndex(t) == n) return;
    if (this.callEvent("onBeforeItemMove", [t, e, n]) === !1) return !1;
    for (var l = [], c = 0; c < s.length; c++) Gt(s[c], null, this, this._ganttConfig) && (l.push(s[c]), s.splice(c, 1), c--);
    this._replace_branch_child(o, t);
    var u = (s = this.getChildren(e))[n];
    (u = lt(u, a)) ? s = s.slice(0, n).concat([t]).concat(s.slice(n)) : s.push(t), l.length && (s = s.concat(l)), gt(r.$rendered_parent, o) || gt(o, e) || (r.$rendered_parent = o), this.setParent(r, e), this._branches[e] = s;
    var d = this.calculateItemLevel(r) - r.$level;
    r.$level += d, this.eachItem(function(h) {
      h.$level += d;
...
const Di = function(t, n) {
  const e = n.getDatastore(t), i = function(s, l) {
    const c = l.getLayers(), u = e.getItem(s);
    if (u && e.isVisible(s)) for (let d = 0; d < c.length; d++) c[d].render_item(u);
  }, a = function(s) {
    const l = s.getLayers();
    ...
    for (let _ = 0; _ < l.length; _++) {
      ...
      if (f.get_visible_range) {
        var d = f.get_visible_range(e);
        if (d.start !== void 0 && d.end !== void 0) {
          var h = d.start + " - " + d.end;
          u[h] ? k = u[h] : (k = e.getIndexRange(d.start, d.end), u[h] = k);
        } else {
          if (d.ids === void 0) throw new Error("Invalid range returned from 'getVisibleRange' of the layer");
          k = d.ids.map(function(v) {
            return e.getItem(v);
          });
        }
      } else c || (c = e.getVisibleItems()), k = c;
    ...
    if (s.update_items) {
      ...
      if (s.get_visible_range) {
        ...
        if (l.start !== void 0 && l.end !== void 0 && (c = e.getIndexRange(l.start, l.end)), l.ids !== void 0) {
          let u = l.ids.map(function(d) {
            return e.getItem(d);
          });
          u.length > 0 && (u = u.filter((d) => d !== void 0), c = c.concat(u));
        }
      ...
      s.prepare_data && s.prepare_data(c, s), s.update_items(c);
    }
  };
  function o(s) {
    return !!s.$services.getService("state").getState("batchUpdate").batch_update;
  }
  e.attachEvent("onStoreUpdated", function(s, l, c) {
    if (J(n)) return !0;
    const u = n.$services.getService("layers").getDataRender(t);
    u && (u.onUpdateRequest = function(d) {
      r(d);
    });
  }), e.attachEvent("onStoreUpdated", function(s, l, c) {
    o(n) || (s && c != "move" && c != "delete" ? (e.callEvent("onBeforeRefreshItem", [l.id]), e.callEvent("onAfterRefreshItem", [l.id])) : (e.callEvent("onBeforeRefreshAll", []), e.callEvent("onAfterRefreshAll", [])));
    ...
    if (e.callEvent("onBeforeIdChange", [s, l]), !o(n) && !e.isSilent()) {
      ...
      c ? (function(u, d, h) {
        for (let _ = 0; _ < u.length; _++) u[_].change_id(d, h);
      }(c.getLayers(), s, l, e.getItem(l)), i(l, c)) : n.render();
    }
  });
};
function fe() {
...
const Ai = { create: function() {
  var t = R({}, { createDatastore: function(n) {
    ...
    if (this.mixin(a, function(s) {
      var l = null, c = s._removeItemInner;
      function u(d) {
        l = null, this.callEvent("onAfterUnselect", [d]);
      }
      return s._removeItemInner = function(d) {
        return l == d && u.call(this, d), l && this.eachItem && this.eachItem(function(h) {
          h.id == l && u.call(this, h.id);
        }, d), c.apply(this, arguments);
      }, s.attachEvent("onIdChange", function(d, h) {
        s.getSelectedId() == d && s.silent(function() {
          s.unselect(d), s.select(h);
        });
      }), { select: function(d) {
        if (d) {
          if (l == d) return l;
          if (!this._skip_refresh && !this.callEvent("onBeforeSelect", [d])) return !1;
          this.unselect(), l = d, this._skip_refresh || (this.refresh(d), this.callEvent("onAfterSelect", [d]));
        }
        return l;
        ...
        (d = d || l) && (l = null, this._skip_refresh || (this.refresh(d), u.call(this, d)));
      } };
    }(a)), n.name) {
...
function jn(t) {
  ...
  return { getSum: function(i, a, r) {
    ...
    if (!(r > s.length - 1 || l <= 0 || o > s.length - 1)) {
      var c = i - this.getSum(s, r, o);
      this.adjustSize(c, s, r, o), this.adjustSize(-c, s, o + 1), a.full_width = this.getSum(s);
    }
    ...
    for (var s = o - r + 1, l = this.getSum(a, r, o), c = r; c <= o; c++) {
      var u = Math.floor(i * (l ? a[c] / l : 1 / s));
      l -= a[c], i -= u, s--, a[c] += u;
    }
    a[a.length - 1] += i;
...
${s}`), a = o.subscales || [];
  ...
  }, prepareConfigs: function(i, a, r, o, s, l, c) {
    for (var u = this.splitSize(o, i.length), d = r, h = [], _ = i.length - 1; _ >= 0; _--) {
      var f = _ == i.length - 1, k = this.initScaleConfig(i[_], s, l);
      f && this.processIgnores(k), this.initColSizes(k, a, d, u[_]), this.limitVisibleRange(k), f && (d = k.full_width), h.unshift(k);
    }
    ...
    for (var c = i.ignore_x || {}, u = 0; u < i.trace_x.length; u++) if (c[i.trace_x[u].valueOf()] || i.display_count == i.count) i.width[u] = 0;
    else {
      var d = 1;
      i.unit == "month" && (d = Math.round((n.add(i.trace_x[u], i.step, i.unit) - i.trace_x[u]) / 864e5)), i.width[u] = d;
    }
    ...
    for (var l = a.trace_x, c = i.trace_x, u = r || 0, d = o || c.length - 1, h = 0, _ = 1; _ < l.length; _++) {
      var f = i.trace_indexes[+l[_]];
      f !== void 0 && f <= d && (s && s.apply(this, [h, _, u, f]), u = f, h = _);
    }
  }, alineScaleColumns: function(i, a, r, o) {
    this.iterateScales(i, a, r, o, function(s, l, c, u) {
      var d = this.getSum(i.width, c, u - 1);
      this.getSum(a.width, s, l - 1) != d && this.setSumWidth(d, a, s, l - 1);
    });
  }, eachColumn: function(i, a, r, o, s) {
    var l = new Date(r), c = new Date(o);
    n[i + "_start"] && (l = n[i + "_start"](l));
    var u = new Date(l);
    for (+u >= +c && (c = n.add(u, a, i)); +u < +c; ) {
      s.call(this, new Date(u));
      var d = u.getTimezoneOffset();
      u = n.add(u, a, i), u = t._correct_dst_change(u, d, a, i), n[i + "_start"] && (u = n[i + "_start"](u));
    }
    ...
    var l = a.length - 1, c = a[l], u = n.add(c, i.step, i.unit);
    if (+u > +i.max_date && l > 0 && (s = i.width[l] - Math.floor(i.width[l] * ((u - i.max_date) / (u - c))), o += i.width[l] - s, i.width[l] = s), o) {
      for (var d = this.getSum(i.width), h = 0, _ = 0; _ < i.width.length; _++) {
        var f = Math.floor(o * (i.width[_] / d));
        i.width[_] += f, h += f;
      }
...
function Mi(t) {
  var n = function(u) {
    var d = new jn(u).primaryScale(), h = d.unit, _ = d.step;
  ...
  }(t), e = n.unit, i = n.step, a = function(u, d) {
    var h = { start_date: null, end_date: null };
    if (d.config.start_date && d.config.end_date) {
      h.start_date = d.date[u + "_start"](new Date(d.config.start_date));
      var _ = new Date(d.config.end_date), f = d.date[u + "_start"](new Date(_));
      _ = +_ != +f ? d.date.add(f, 1, u) : f, h.end_date = _;
    }
    return h;
  }(e, t);
  if (!a.start_date || !a.end_date) {
    ...
    if (o.length && r) {
      var l = o[0].start_date, c = t.date.add(l, 1, t.config.duration_unit);
    ...
    a.start_date && a.end_date || (a = { start_date: /* @__PURE__ */ new Date(), end_date: /* @__PURE__ */ new Date() }), t.eachTask(function(u) {
      t.config.deadlines && u.deadline && pe(a, u.deadline, u.deadline), u.constraint_date && u.constraint_type && t.config.constraint_types && u.constraint_type !== t.config.constraint_types.ASAP && u.constraint_type !== t.config.constraint_types.ALAP && pe(a, u.constraint_date, u.constraint_date), t.config.baselines && u.baselines && u.baselines.forEach(function(d) {
        pe(a, d.start_date, d.end_date);
      });
...
function un(t, n, e) {
  dn(t, n.$source, e), dn(t, n.$target, e);
}
const Me = { getSubtreeLinks: function(t, n) {
  var e = {};
  return t.isTaskExists(n) && un(t, t.getTask(n), e), t.eachTask(function(i) {
    un(t, i, e);
  }, n), e;
...
class Ii {
  ...
  attach() {
    ...
    this._dataProcessorHandlers.push(e.attachEvent("onAfterTaskAdd", function(s, l) {
      ...
      return e.config.cascade_delete && (i[s] = { tasks: Me.getSubtreeTasks(e, s), links: Me.getSubtreeLinks(e, s) }), !n.deleteAfterConfirmation || (n.setGanttMode("tasks"), n.setUpdated(s, !0, "deleted"), !1);
    })), this._dataProcessorHandlers.push(e.attachEvent("onAfterTaskDelete", function(s, l) {
      n.setGanttMode("tasks");
      const c = !a(s), u = e.config.cascade_delete && i[s];
      if (c || u) {
        if (u) {
          const d = n.updateMode;
          n.setUpdateMode("off");
          const h = i[s];
          ...
          i[s] = null, d !== "off" && n.sendAllData(), n.setGanttMode("tasks"), n.setUpdateMode(d);
        }
        c && (n.storeItem(l), n.deleteAfterConfirmation || n.setUpdated(s, !0, "deleted")), n.updateMode === "off" || n._tSend || n.sendAllData();
      }
    ...
    this._dataProcessorHandlers.push(e.attachEvent("onTaskIdChange", function(s, l) {
      ...
      if (c.length) {
        ...
        for (let d = 0; d < c.length; d++) {
          const h = this.getTask(c[d]);
          r[h.id] = h;
        }
      }
      const u = function(d) {
        let h = [];
        return d.$source && (h = h.concat(d.$source)), d.$target && (h = h.concat(d.$target)), h;
      }(this.getTask(l));
      if (u.length) {
        o = o || {};
        for (let d = 0; d < u.length; d++) {
          const h = this.getLink(u[d]);
          o[h.id] = h;
        }
      }
      ...
      return !0;
    }), n.attachEvent("insertCallback", function(s, l, c, u) {
      const d = s.data || e.xml._xmlNodeToJSON(s.firstChild), h = { add: e.addTask, isExist: e.isTaskExists };
      u === "links" && (h.add = e.addLink, h.isExist = e.isLinkExists), h.isExist.call(e, l) || (d.id = l, h.add.call(e, d));
    }), n.attachEvent("updateCallback", function(s, l) {
      const c = s.data || e.xml._xmlNodeToJSON(s.firstChild);
    ...
    }), n.attachEvent("deleteCallback", function(s, l, c, u) {
      const d = { delete: e.deleteTask, isExist: e.isTaskExists };
      u === "links" ? (d.delete = e.deleteLink, d.isExist = e.isLinkExists) : u === "assignment" && (d.delete = function(h) {
        e.$data.assignmentsStore.remove(h);
      }, d.isExist = function(h) {
        return e.$data.assignmentsStore.exists(h);
      }), d.isExist.call(e, l) && d.delete.call(e, l);
...
let ee = se, Wn = class {
  ...
  afterUpdate(t, n, e) {
    ...
    const l = (d) => {
      const h = s.action || this.getState(d) || "updated", _ = s.sid || d[0], f = s.tid || d[0];
      t.afterUpdateCallback(_, f, h, s, a);
    };
    if (s) return Array.isArray(e) && e.length > 1 ? e.forEach((d) => l(d)) : l(e), t.finalizeUpdate(), void this.setGanttMode(a);
    const c = o.xmltop("data", n.xmlDoc);
    if (!c) return this.cleanUpdate(e);
    const u = o.xpath("//data/action", c);
    if (!u.length) return this.cleanUpdate(e);
    for (let d = 0; d < u.length; d++) {
      const h = u[d], _ = h.getAttribute("type"), f = h.getAttribute("sid"), k = h.getAttribute("tid");
  ...
  init(t) {
    ...
    const n = new Ii(this.$gantt, this);
    ...
    e = e.replace("editing=true&", ""), this.getUpdates(e, (i) => {
      ...
      if (r.length) {
        ...
        for (let o = 0; o < r.length; o++) {
          const s = r[o].getAttribute("status"), l = r[o].getAttribute("id"), c = r[o].getAttribute("parent");
          switch (s) {
            case "inserted":
              this.callEvent("insertCallback", [r[o], l, c]);
              break;
            case "updated":
              this.callEvent("updateCallback", [r[o], l, c]);
              break;
            case "deleted":
              this.callEvent("deleteCallback", [r[o], l, c]);
          }
        }
        this._silent_mode = !1;
      }
  ...
  _sendData(t, n) {
    ...
    if (this._tMode === "CUSTOM") {
      const l = this.getState(n), c = this.getActionByState(l), u = this.getGanttMode(), d = (_) => {
      ...
      if (this._router instanceof Function) if (this._routerParametersFormat === "object") {
        const _ = { entity: u, action: c, data: t, id: n };
        h = this._router(_);
      } else h = this._router(u, c, t, n);
      else if (this._router[u] instanceof Function) h = this._router[u](c, t, n);
      else {
...
See https://docs.dhtmlx.com/gantt/desktop__server_side.html#customrouting and https://docs.dhtmlx.com/gantt/api__gantt_createdataprocessor.html for details.`;
      ...
      if (h) {
        ...
        h.then ? h.then(d).catch((_) => {
          _ && _.action ? d(_) : d({ action: "error", value: _ });
        }) : d(h);
      } else d(null);
      return;
    }
    ...
    i = { callback: (l) => {
      const c = [];
      if (n) c.push(n);
      else if (t) for (const u in t) c.push(u);
    ...
    switch (this._tMode) {
      ...
      case "JSON":
        ...
        for (const c in l) c !== this.action_param && c !== "id" && c !== "gr_id" && (o[c] = l[c]);
...
function Pi(t) {
  ...
  return t.$services.getService("state").registerProvider("batchUpdate", function() {
    ...
    if (e) s(l);
      ...
      var u, d = this._dp && this._dp.updateMode != "off";
      d && (u = this._dp.updateMode, this._dp.setUpdateMode("off"));
      ...
      e = !1, c || this.render(), d && (this._dp.setUpdateMode(u), this._dp.setGanttMode("task"), this._dp.sendData(), this._dp.setGanttMode("link"), this._dp.sendData());
    }
  };
}
function Ri(t) {
...
function Hi(t) {
  const n = /* @__PURE__ */ function(i) {
    return { _needRecalc: !0, reset: function() {
      ...
      let r = a.split("."), o = i.config.root_id;
      for (let s = 0; s < r.length; s++) {
        const l = i.getChildren(o);
        let c = 1 * r[s] - 1;
        if (!i.isTaskExists(l[c])) return null;
        o = l[c];
      }
      return i.isTaskExists(o) ? i.getTask(o) : null;
    }, _calcWBS: function() {
      ...
      i.eachTask(function(r) {
        ...
        const o = this._getPrevNonPlaceholderSibling(r.id);
        if (o !== null) this._increaseWBS(r, o);
        else {
          let s = i.getParent(r.id);
          this._setWBSCode(r, i.getTask(s).$wbs + ".1");
        }
      }, i.config.root_id, this), this._needRecalc = !1;
    }, _increaseWBS: function(a, r) {
      let o = i.getTask(r).$wbs;
      o && (o = o.split("."), o[o.length - 1]++, this._setWBSCode(a, o.join(".")));
    }, _getPrevNonPlaceholderSibling: function(a) {
      let r, o = a;
      do
        r = i.getPrevSibling(o), o = r;
...
function Oi(t) {
  ...
  function r(l, c, u) {
    return Array.isArray(l) ? l.map(function(d) {
      return a(d);
    }).join("_") + `_${c}_${u}` : a(l) + `_${c}_${u}`;
  }
  function o(l, c, u) {
    var d, h = r(c, l, JSON.stringify(u)), _ = {};
    return ht(c, function(f) {
      _[a(f)] = !0;
    }), n[h] ? d = n[h] : (d = n[h] = [], t.eachTask(function(f) {
      if (u) {
        if (!u[t.getTaskType(f)]) return;
      } else if (f.type == t.config.types.project) return;
      l in f && ht(Lt(f[l]) ? f[l] : [f[l]], function(k) {
  ...
  function s(l, c, u) {
    var d = t.config.resource_property, h = [];
  ...
  return { getTaskBy: function(l, c, u) {
    return typeof l == "function" ? (d = l, h = [], t.eachTask(function(_) {
      d(_) && h.push(_);
    }), h) : Lt(c) ? o(l, c, u) : o(l, [c], u);
    var d, h;
  }, getResourceAssignments: function(l, c) {
    var u = [], d = t.config.resource_property;
    return c !== void 0 ? s(l, c, u) : t.getTaskBy(d, l).forEach(function(h) {
      s(l, h.id, u);
    }), u;
  } };
}
function Bi(t) {
  ...
  t.ext.resources = /* @__PURE__ */ function(a) {
    const r = { renderEditableLabel: function(o, s, l, c, u) {
      const d = a.config.readonly ? "" : "contenteditable";
      if (o < l.end_date && s > l.start_date) {
        for (let h = 0; h < u.length; h++) {
          const _ = u[h];
          return "<div " + d + " data-assignment-cell data-assignment-id='" + _.id + "' data-row-id='" + l.id + "' data-task='" + l.$task_id + "' data-start-date='" + a.templates.format_date(o) + "' data-end-date='" + a.templates.format_date(s) + "'>" + _.value + "</div>";
        }
        return "<div " + d + " data-assignment-cell data-empty  data-row-id='" + l.id + "' data-resource-id='" + l.$resource_id + "' data-task='" + l.$task_id + "' data-start-date='" + a.templates.format_date(o) + "''  data-end-date='" + a.templates.format_date(s) + "'>-</div>";
      }
      return "";
    }, renderSummaryLabel: function(o, s, l, c, u) {
      let d = u.reduce(function(h, _) {
        return h + Number(_.value);
      }, 0);
      return d % 1 && (d = Math.round(10 * d) / 10), d ? "<div>" + d + "</div>" : "";
    }, editableResourceCellTemplate: function(o, s, l, c, u) {
      return l.$role === "task" ? r.renderEditableLabel(o, s, l, c, u) : r.renderSummaryLabel(o, s, l, c, u);
    }, editableResourceCellClass: function(o, s, l, c, u) {
      const d = [];
      d.push("resource_marker"), l.$role === "task" ? d.push("task_cell") : d.push("resource_cell");
      const h = u.reduce(function(f, k) {
        return f + Number(k.value);
      }, 0);
      let _ = Number(l.capacity);
      return isNaN(_) && (_ = 8), h <= _ ? d.push("workday_ok") : d.push("workday_over"), d.join(" ");
    }, getSummaryResourceAssignments: function(o) {
      let s;
      const l = a.getDatastore(a.config.resource_store), c = l.getItem(o);
      return c.$role === "task" ? s = a.getResourceAssignments(c.$resource_id, c.$task_id) : (s = a.getResourceAssignments(o), l.eachItem && l.eachItem(function(u) {
      ...
      a.config.resource_render_empty_cells = !0, function() {
        ...
        function c() {
          return l && cancelAnimationFrame(l), l = requestAnimationFrame(function() {
            a.$container && Array.prototype.slice.call(a.$container.querySelectorAll(".resourceTimeline_cell [data-assignment-cell]")).forEach(function(u) {
              u.contentEditable = !0;
            });
          }), !0;
        }
        a.attachEvent("onGanttReady", function() {
          ...
          a.$layout.getCellsByType("viewCell").forEach(function(u) {
            u.$config && u.$config.view === "resourceTimeline" && u.$content && u.$content.attachEvent("onScroll", c);
          });
        });
      }();
      let o = null;
      ...
      a.attachEvent("onGanttReady", function() {
        ...
        a.event(a.$container, "keypress", function(c) {
          var u = c.target.closest(".resourceTimeline_cell [data-assignment-cell]");
          u && (c.keyCode !== 13 && c.keyCode !== 27 || u.blur());
        }), a.event(a.$container, "keydown", function(c) {
          c.key === "Tab" && (o = Ut(a.$container).indexOf(document.activeElement));
          ...
          if (!l) {
            ...
            var u = c.target.closest(".resourceTimeline_cell [data-assignment-cell]");
            if (u) {
              var d = (u.innerText || "").trim();
              d == "-" && (d = "0");
              var h = Number(d), _ = u.getAttribute("data-row-id"), f = u.getAttribute("data-assignment-id"), k = u.getAttribute("data-task"), v = u.getAttribute("data-resource-id"), b = a.templates.parse_date(u.getAttribute("data-start-date")), g = a.templates.parse_date(u.getAttribute("data-end-date")), m = a.getDatastore(a.config.resource_assignment_store);
              if (isNaN(h)) a.getDatastore(a.config.resource_store).refresh(_);
              else {
                var p = a.getTask(k);
                if (a.plugins().undo && a.ext.undo.saveState(k, "task"), f) {
                  ...
                  if (w.start_date.valueOf() === b.valueOf() && w.end_date.valueOf() === g.valueOf()) w.value = h, h ? m.updateItem(w.id) : m.removeItem(w.id);
                    ...
                    if (w.end_date.valueOf() > g.valueOf()) {
                      var y = a.copy(w);
                      y.id = a.uid(), y.start_date = g, y.duration = a.calculateDuration({ start_date: y.start_date, end_date: y.end_date, task: p }), y.delay = a.calculateDuration({ start_date: p.start_date, end_date: y.start_date, task: p }), y.mode = w.mode || "default", y.duration !== 0 && m.addItem(y);
                    }
                  ...
                  a.updateTaskAssignments(p.id), a.updateTask(p.id), s();
                } else if (h) {
                  var w = { task_id: k, resource_id: v, value: h, start_date: b, end_date: g, duration: a.calculateDuration({ start_date: b, end_date: g, task: p }), delay: a.calculateDuration({ start_date: p.start_date, end_date: b, task: p }), mode: "fixedDuration" };
                  m.addItem(w), a.updateTaskAssignments(p.id), a.updateTask(p.id), s();
                }
              }
            }
          }
    ...
    t.$resourcesStore = t.createDatastore({ name: t.config.resource_store, type: s, fetchTasks: r !== void 0 && r, initItem: o }), t.$data.resourcesStore = t.$resourcesStore, t.$resourcesStore.attachEvent("onParse", function() {
      let l, c = function(u) {
        ...
        return u.forEach(function(h) {
          const _ = t.copy(h);
          _.key = h.id, _.label = h.text, d.push(_);
        }), d;
      };
      t.config.resources && t.config.resources.lightbox_resources && (c = t.config.resources.lightbox_resources), t.config.resources && t.config.resources.editable_resource_diagram ? l = c(t.$resourcesStore.getItems().filter((u) => {
        let d = t.getResourceAssignments(u.id);
        if (!t.$resourcesStore.hasChild(u.id) || d && d.length) return !u.$resource_id || !u.$task_id;
...
function zi(t) {
  ...
  var e = "auto", i = "singleValue", a = "valueArray", r = "resourceValueArray", o = "assignmentsArray", s = e, l = "fixedDates", c = "fixedDuration", u = "default";
  function d(p, y) {
    ...
    if (p.mode = p.mode || u, p.mode === c && (isNaN(Number(p.duration)) && (y = y || t.getTask(p.task_id), p.duration = t.calculateDuration({ start_date: p.start_date, end_date: p.end_date, id: y })), x && (y = y || t.getTask(p.task_id), p.delay = t.calculateDuration({ start_date: y.start_date, end_date: p.start_date, id: y }))), p.mode !== l && (y || t.isTaskExists(p.task_id))) {
      var $ = _(p, y = y || t.getTask(p.task_id));
  ...
  var h = t.createDatastore({ name: t.config.resource_assignment_store, initItem: function(p) {
    return p.id || (p.id = t.uid()), d(p), p;
  } });
  function _(p, y) {
    if (p.mode === l) return { start_date: p.start_date, end_date: p.end_date, duration: p.duration };
    var w, x, $ = p.delay ? t.calculateEndDate({ start_date: y.start_date, duration: p.delay, task: y }) : new Date(y.start_date);
    return p.mode === c ? (w = t.calculateEndDate({ start_date: $, duration: p.duration, task: y }), x = p.duration) : (w = new Date(y.end_date), x = y.duration - p.delay), { start_date: $, end_date: w, duration: x };
  }
  function f(p) {
    ...
    let $ = s === e;
    if (t.defined(w) && w) {
      Array.isArray(w) || (w = [w], $ && (s = i, $ = !1));
      const S = {};
      w.forEach(function(T) {
        T.resource_id || (T = { resource_id: T }, $ && (s = a, $ = !1)), $ && (T.id && T.resource_id ? (s = o, $ = !1) : (s = r, $ = !1));
        let E, C = u;
        ...
        Object.keys(T).forEach((M) => {
          M != "$id" && (D[M] = T[M]);
        }), D.start_date && D.start_date.getMonth && D.end_date && D.end_date.getMonth && typeof D.duration == "number" || d(D, p), x.push(D);
      });
    }
    return x;
  }
  function k(p) {
    if (t.isTaskExists(p)) {
      var y = t.getTask(p);
      v(y, t.getTaskAssignments(y.id));
    }
  }
  function v(p, y) {
    y.sort(function(w, x) {
      return w.start_date && x.start_date && w.start_date.valueOf() != x.start_date.valueOf() ? w.start_date - x.start_date : 0;
    }), s == o ? p[t.config.resource_property] = y : s == r && (p[t.config.resource_property] = y.map(function(w) {
    ...
    var w = function(x, $) {
      ...
      if (s == i) {
        ...
        x.forEach(function(A) {
          ...
          M[A.resource_id] = A;
        }), x.concat($).forEach(function(A) {
          if (!I[A.resource_id]) {
            I[A.resource_id] = !0;
            var L = D[A.resource_id], P = M[A.resource_id];
            L && P ? S.inBoth.push({ store: P, task: L }) : L && !P ? S.inTaskNotInStore.push(L) : !L && P && S.inStoreNotInTask.push(P);
          }
        });
      } else s != o && s != r || (D = {}, M = {}, I = {}, x.forEach(function(A) {
        D[A.id || A.$id] = A;
      }), $.forEach(function(A) {
        M[A.id] = A;
      }), x.concat($).forEach(function(A) {
        ...
        if (!I[L]) {
          I[L] = !0;
          var P = D[L], N = M[L];
          P && N ? S.inBoth.push({ store: N, task: P }) : P && !N ? S.inTaskNotInStore.push(P) : !P && N && S.inStoreNotInTask.push(N);
        }
      }));
      return S;
    }(f(p), y);
    ...
    if (t.config.process_resource_assignments) {
      ...
      t.attachEvent("onBeforeBatchUpdate", function() {
        ...
        if (y) {
          ...
          for (var C in t.config.process_resource_assignments && s === "resourceValueArray" && (T = null), w) g(w[C], E[C]);
        }
        y = !1, p = !1, w = {};
      ...
      h.attachEvent("onStoreUpdated", function() {
        ...
        return $ === null && ($ = {}, S = {}, h.eachItem(function(M) {
          $[M.resource_id] || ($[M.resource_id] = []), $[M.resource_id].push(M);
        ...
        const I = [];
        for (const A in M) {
          const L = C.getItem(M[A]);
          L && I.push(L);
        }
        return I;
      }, t.updateTaskAssignments = k;
    }
  }, { once: !0 });
}
function ji(t) {
  ...
  t.config.types.placeholder = "placeholder", t.attachEvent("onDataProcessorReady", n(function(s) {
    s && !s._silencedPlaceholder && (s._silencedPlaceholder = !0, s.attachEvent("onBeforeUpdate", n(function(l, c, u) {
  ...
  t.attachEvent("onGanttReady", function() {
    ...
    for (var l = 0; l < s.commands.length; l++) {
      var c = s.commands[l];
      c.entity === "task" && c.value.type === t.config.types.placeholder && (s.commands.splice(l, 1), l--);
    }
    return !0;
  });
}
function Fi(t) {
  ...
  function e(u, d) {
    ...
    _ !== !1 && t.getTaskType(h) !== _ && (d.$needsUpdate = !0, d[h.id] = { task: h, type: _ });
  }
  function i(u) {
    if (!t.getState().group_mode) {
      ...
      d.$needsUpdate && t.batchUpdate(function() {
        (function(h) {
  ...
  var a;
  function r(u) {
    var d = t.config.types, h = t.hasChild(u.id), _ = t.getTaskType(u.type);
    return h && _ === d.task ? d.project : !h && _ === d.project && d.task;
  }
  var o, s, l = !0;
  function c(u) {
  ...
  t.attachEvent("onParse", n(function() {
    ...
    l && i(u);
  })), t.attachEvent("onBeforeTaskDelete", n(function(u, d) {
    return a = t.getParent(u), !0;
  })), t.attachEvent("onAfterTaskDelete", n(function(u, d) {
    c(a);
  })), t.attachEvent("onRowDragStart", n(function(u, d, h) {
    return o = t.getParent(u), !0;
  })), t.attachEvent("onRowDragEnd", n(function(u, d) {
    c(o), i(u);
  })), t.attachEvent("onBeforeTaskMove", n(function(u, d, h) {
    return s = t.getParent(u), !0;
  })), t.attachEvent("onAfterTaskMove", n(function(u, d, h) {
    document.querySelector(".gantt_drag_marker") || (c(s), i(u));
  }));
}
const oe = class oe {
  constructor(n = null) {
    this.canParse = (e) => {
      ...
      for (let u = 0; u < s.length; u++) {
        const d = s[u], h = this._getValueFromMinutes(o, d, u === c);
        o -= this._getValueInMinutes(h, d), l += `${this._getLabelForConvert(h, d, r)}${u === c ? "" : " "}`;
      }
      return l;
    }, this.parse = (e) => {
      if (this.canParse(e)) {
        ...
        for (let c = 0; c < e.length; c++) {
          const u = e[c];
          l.test(u) ? r = a : a = !0, (r || s === c) && (r || (i += u), o += this._getNumericValue(i), a = r = !1, i = ""), i += u;
        }
        if (o) {
          const c = this._config.store;
          return Math.round(this.transferUnits[c].fromMinutes(Math.ceil(o)));
        }
      }
      return null;
...
}, Vn = function(t, n, e, i, a, r) {
  const o = Ge(t, a), s = ne(t, n, a);
  let l = s.maxHeight, c = e.height, u = c > i, d = e.rowHeight >= i && !s.splitChild && !o;
  (u || d) && (c = i), l < c && (c = l);
  let h = Math.floor((e.rowHeight - c) / 2);
  if (s.splitChild && (h = Math.floor((l - c) / 2)), r || o) {
    let _ = Math.min(e.height, l) - c, f = 2, k = o && a.bar_height >= a.row_height, v = s.splitChild && e.height >= l;
    ...
    t.eachParent(function(r) {
      if (pt(r)) {
        ...
        t.getChildren(r.id).forEach(function(l) {
          const c = t.getTask(l);
          if (c.id == a.id) return;
          const u = c.row_height || t.getLayoutView("timeline").getBarHeight(c.id);
          s = s || u, u > s && (s = u);
  ...
  t.$data.baselineStore = n, t.adjustTaskHeightForBaselines = function(a) {
    ...
    if (c && t.config.show_chart) switch (t.config.baselines.render_mode) {
      ...
      default:
        ...
        break;
      case "separateRow":
        r = c.getBarHeight(a.id), s ? (a.bar_height = a.bar_height || r, a.bar_height > r && (r = a.bar_height), a.row_height = r + l) : a.bar_height && (a.row_height = a.bar_height + 4), i(a);
        break;
      case "individualRow":
        r = c.getBarHeight(a.id), s ? (a.bar_height = a.bar_height || r, a.bar_height > r && (r = a.bar_height), o = l * s, a.row_height = r + o + 2) : a.bar_height && (a.row_height = a.bar_height + 4), i(a);
    }
  }, t.attachEvent("onGanttReady", function() {
    t.config.baselines && (t.attachEvent("onParse", function() {
      n.eachItem(function(a) {
        ...
        if (t.isTaskExists(r)) {
          ...
          s && o.baselines.push(a), J(t) || (pt(o) ? e(o) : t.adjustTaskHeightForBaselines(o));
        }
      });
    }), t.attachEvent("onBeforeTaskUpdate", function(a, r) {
      return function(o) {
        let s = !1;
        const l = {}, c = o.baselines || [], u = t.getTaskBaselines(o.id);
        c.length != u.length && (s = !0), c.forEach(function(d) {
          l[d.id] = !0;
          const h = n.getItem(d.id);
          if (h) {
            const _ = +h.start_date != +d.start_date, f = +h.end_date != +d.end_date;
            (_ || f) && n.updateItem(d.id, d);
          } else n.addItem(d);
        }), u.forEach(function(d) {
          l[d.id] || n.removeItem(d.id);
...
function Un(t) {
  var n = {}, e = {}, i = null, a = -1, r = null, o = /* @__PURE__ */ function(s) {
    ...
    return { resetCache: function() {
      ...
      if (u) for (var d = this._getRowHeight(), h = 0; h < u.fullOrder.length; h++) {
        var _ = u.getItem(u.fullOrder[h]);
        if (_ && _.row_height && _.row_height !== d) {
          c = !1;
          break;
        }
      }
  ...
  return { _resetTopPositionHeight: function() {
    ...
    for (var c = l.getIndexRange(), u = 0, d = 0, h = 0; h < c.length; h++) e[h] = u, u += this.getItemHeight(c[h].id), h < s && (d = u);
    return d;
    ...
    if (this.$config.rowStore) {
      ...
      var l = this.$config.rowStore;
      if (!l) return 0;
      var c = l.getIndexById(s);
      if (c === -1 && l.getParent && l.exists(s)) {
        var u = l.getParent(s);
        if (l.exists(u)) {
          var d = l.getItem(u);
          if (this.$gantt.isSplitTask(d)) return this.getItemTop(u);
        }
      }
      return n[s] = this.getRowTop(c), n[s];
    }
    return 0;
    ...
    if (this.$config.rowStore) {
      ...
      var l = this.getItemHeight.bind(this), c = s.getVisibleItems(), u = 0;
    ...
    if (this.$config.rowStore) {
      ...
      for (var l = this.$config.rowStore, c = 0; c < l.countVisible(); c++) {
        var u = this.getRowTop(c), d = this.getRowTop(c + 1);
        if (!d) {
          var h = l.getIdByIndex(c);
          d = u + this.getItemHeight(h);
        }
        if (s >= u && s < d) return c;
      }
...
class qi {
  ...
  attachEvents() {
    this._dnd.attachEvent("onBeforeDragStart", (n, e) => {
      ...
      if (!this._draggedCell) return;
      const n = this.findColumnsIndexes(), e = n.targetIndex, i = n.draggedIndex, a = this.$grid.$getConfig().columns, r = a[i], o = a[e];
      if (this.$grid.callEvent("onColumnDragMove", [{ draggedColumn: r, targetColumn: o, draggedIndex: i, targetIndex: e }]) === !1) return this.cleanTargetMarker(), void this.$gantt.render();
  ...
  findColumnsIndexes() {
    ...
    let l, c = 0, u = e.length - 1, d = (f, k) => f <= k, h = (f) => ++f;
    this.$gantt.config.rtl && (c = e.length - 1, u = 0, d = (f, k) => f >= k, h = (f) => --f);
    const _ = this._dragX - this.$grid.$grid.getBoundingClientRect().left + this._scrollableGrid.getCorrection();
    for (let f = c; d(f, u) && (i === void 0 || a === void 0); f = h(f)) e[f].hide || (s.startX = s.endX, s.endX += e[f].width, _ >= s.startX && (_ <= s.endX || !d(h(f), u)) && (i = f, r = s.startX, o = s.endX, l = (_ - s.startX) / (s.endX - s.startX)), n === e[f].name && (a = f));
...
var ue = function(t, n, e, i) {
  this.$config = R({}, n || {}), this.$gantt = i, this.$parent = t, _t(this), this.$state = {}, R(this, Un(this));
};
function Yi(t) {
  function n(e) {
  ...
  t.load = function() {
    ...
    for (var a in e) if (e.hasOwnProperty(a)) {
      ...
      for (var s = 0; s < r.length; s++) {
        var l = r[s], c = this.copy(l);
        for (var u in c.key = c.value, l) if (l.hasOwnProperty(u)) {
    ...
    for (var i = {}, a = t.ajax.xpath("//coll_options", e), r = 0; r < a.length; r++) for (var o = i[a[r].getAttribute("for")] = [], s = t.ajax.xpath(".//item", a[r]), l = 0; l < s.length; l++) {
      for (var c = s[l].attributes, u = { key: s[l].getAttribute("value"), label: s[l].getAttribute("label") }, d = 0; d < c.length; d++) {
        var h = c[d];
    ...
    for (o = t.ajax.xpath("//project", i), s = 0; s < o.length; s++)
      (c = t.xml._xmlNodeToJSON(o[s], !0)).id = "project-" + c.id, r.push(c);
    for (s = 0; s < r.length; s++) {
      var c;
      (c = r[s]).start_date = c.startdate || c.est, c.end_date = c.enddate, c.text = c.name, c.duration = c.duration / 8, c.open = 1, c.duration || c.end_date || (c.duration = 1), c.predecessortasks && a.collections.links.push({ target: c.id, source: c.predecessortasks, type: t.config.links.finish_to_start });
    }
    return a;
  }, serialize: function() {
...
ue.prototype = { init: function(t) {
  ...
  var l = function(u, d) {
    var h = { column_before_start: u.bind(function(_, f, k) {
      var v = d.$getConfig(), b = tt(k, v.grid_resizer_column_attribute);
      if (!b || !dt(b, ".gantt_grid_column_resize_wrap")) return !1;
      var g = this.locate(k, v.grid_resizer_column_attribute), m = d.getGridColumns()[g];
      return d.callEvent("onColumnResizeStart", [g, m]) !== !1 && void 0;
    }, u), column_after_start: u.bind(function(_, f, k) {
      var v = d.$getConfig(), b = this.locate(k, v.grid_resizer_column_attribute);
      _.config.marker.innerHTML = "", _.config.marker.className += " gantt_grid_resize_area", _.config.marker.style.height = d.$grid.offsetHeight + "px", _.config.marker.style.top = "0px", _.config.drag_index = b;
    }, u), column_drag_move: u.bind(function(_, f, k) {
      var v = d.$getConfig(), b = _.config, g = d.getGridColumns(), m = parseInt(b.drag_index, 10), p = g[m], y = K(d.$grid_scale), w = parseInt(b.marker.style.left, 10), x = p.min_width ? p.min_width : v.min_grid_column_width, $ = d.$grid_data.offsetWidth, S = 0, T = 0;
      ...
      if (w < x && (w = x), v.keep_grid_width) {
        ...
        C && (x = d.$grid_data.offsetWidth - C), w < x && (w = x), w > $ && (w = $);
      } else if (!d.$config.scrollable) {
        var D = w, M = u.$container.offsetWidth, I = 0;
        if (d.$grid_data.offsetWidth <= M - 25) for (E = m + 1; E < g.length; E++) I += g[E].width;
        else {
        ...
        if (A && A.$config.mode == "y") {
          var L = A.$lastSize.x;
          M = Math.min(M, L - (A.$cells.length - 1));
        }
        D + I > M && (w = M - I);
      }
      return b.left = w - 1, T = Math.abs(w - S), p.max_width && T > p.max_width && (T = p.max_width), v.rtl && (S = y.width - S + 2 - T), b.marker.style.top = y.y + "px", b.marker.style.left = y.x - 1 + S + "px", b.marker.style.width = T + "px", d.callEvent("onColumnResize", [m, g[m], T - 1]), !0;
    }, u), column_drag_end: u.bind(function(_, f, k) {
      for (var v = d.$getConfig(), b = d.getGridColumns(), g = 0, m = parseInt(_.config.drag_index, 10), p = b[m], y = 0; y < m; y++) g += b[y].width;
      var w = p.min_width && _.config.left - g < p.min_width ? p.min_width : _.config.left - g;
      if (p.max_width && p.max_width < w && (w = p.max_width), d.callEvent("onColumnResizeEnd", [m, p, w]) !== !1 && p.width != w) {
        if (p.width = w, v.keep_grid_width) g = v.grid_width;
        else {
          y = m;
          for (var x = b.length; y < x; y++) g += b[y].width;
        }
        d.callEvent("onColumnResizeComplete", [b, d._setColumnsWidth(g, m)]), d.$config.scrollable || u.$layout._syncCellSizes(d.$config.group, { value: v.grid_width, isGravity: !1 }), this.render();
      }
    }, u) };
    return { init: function() {
      var _ = u.$services.getService("dnd"), f = d.$getConfig(), k = new _(d.$grid_scale, { updates_per_second: 60 });
      ...
      for (var _ = d.getGridColumns(), f = d.$getConfig(), k = 0, v = d.$config.width, b = f.scale_height, g = 0; g < _.length; g++) {
        var m, p = _[g];
        if (k += p.width, m = f.rtl ? v - k : k, p.resize && g != _.length - 1) {
          var y = document.createElement("div");
          y.className = "gantt_grid_column_resize_wrap", y.style.top = "0px", y.style.height = b + "px", y.innerHTML = "<div class='gantt_grid_column_resize'></div>", y.setAttribute(f.grid_resizer_column_attribute, g), y.setAttribute("column_index", g), u._waiAria.gridSeparatorAttr(y), d.$grid_scale.appendChild(y), y.style.left = Math.max(0, m) + "px";
        }
      }
    } };
  }(n, this);
  ...
  var c = function(u, d) {
    var h = { row_before_start: u.bind(function(_, f, k) {
      var v = d.$getConfig(), b = d.$config.rowStore;
      if (!tt(k, v.task_grid_row_resizer_attribute)) return !1;
      var g = this.locate(k, v.task_grid_row_resizer_attribute), m = b.getItem(g);
      return d.callEvent("onBeforeRowResize", [m]) !== !1 && void 0;
    }, u), row_after_start: u.bind(function(_, f, k) {
      var v = d.$getConfig(), b = this.locate(k, v.task_grid_row_resizer_attribute);
      _.config.marker.innerHTML = "", _.config.marker.className += " gantt_row_grid_resize_area", _.config.marker.style.width = d.$grid.offsetWidth + "px", _.config.drag_id = b;
    }, u), row_drag_move: u.bind(function(_, f, k) {
      var v = d.$config.rowStore, b = d.$getConfig(), g = _.config, m = g.drag_id, p = d.getItemHeight(m), y = d.getItemTop(m) - f.scrollTop, w = K(d.$grid_data), x = parseInt(g.marker.style.top, 10), $ = y + w.y, S = 0, T = b.min_task_grid_row_height;
      return (S = x - $) < T && (S = T), g.marker.style.left = w.x + "px", g.marker.style.top = $ - 1 + "px", g.marker.style.height = Math.abs(S) + 1 + "px", g.marker_height = S, d.callEvent("onRowResize", [m, v.getItem(m), S + p]), !0;
    }, u), row_drag_end: u.bind(function(_, f, k) {
      var v = d.$config.rowStore, b = _.config, g = b.drag_id, m = v.getItem(g), p = d.getItemHeight(g), y = b.marker_height;
      d.callEvent("onBeforeRowResizeEnd", [g, m, y]) !== !1 && m.row_height != y && (m.row_height = y, v.updateItem(g), d.callEvent("onAfterRowResize", [g, m, p, y]), this.render());
    }, u) };
    return { init: function() {
      var _ = u.$services.getService("dnd"), f = d.$getConfig(), k = new _(d.$grid_data, { updates_per_second: 60 });
      u.defined(f.dnd_sensitivity) && (k.config.sensitivity = f.dnd_sensitivity), k.attachEvent("onBeforeDragStart", function(v, b) {
  ...
  c.init(), this._addLayers(this.$gantt), this._initEvents(), r && (this._columnDND = new qi(n, this), this._columnDND.init()), this.callEvent("onReady", []);
  ...
  for (var e, i = this.getGridColumns(), a = 0, r = (c = this.$getConfig()).grid_elastic_columns, o = 0, s = i.length; o < s; o++) this._validateColumnWidth(i[o], "min_width"), this._validateColumnWidth(i[o], "max_width"), this._validateColumnWidth(i[o], "width"), a += 1 * i[o].width;
  if (!isNaN(a) && this.$config.scrollable || (a = e = this._setColumnsWidth(t + 1)), this.$config.scrollable && r && !isNaN(a)) {
    let d = "width";
    r == "min_width" && (d = "min_width");
    let h = 0;
    i.forEach(function(_) {
      h += _[d] || c.min_grid_column_width;
    });
    var l = Math.max(h, t);
  ...
  var u = Math.max(this.$state.height - c.scale_height, 0);
  ...
  var l = r - a, c = 0;
  for (o = 0; o < n + 1; o++) c += i[o].width;
  for (a -= c, o = n + 1; o < i.length; o++) {
    var u = i[o], d = Math.round(l * (u.width / a));
    l < 0 ? u.min_width && u.width + d < u.min_width ? d = u.min_width - u.width : !u.min_width && e.min_grid_column_width && u.width + d < e.min_grid_column_width && (d = e.min_grid_column_width - u.width) : u.max_width && u.width + d > u.max_width && (d = u.max_width - u.width), a -= u.width, u.width += d, l -= d;
  }
  ...
  if (t.autofit || i.length) {
    ...
    if (t.autofit && !t.grid_elastic_columns) for (r = 0; r < a.length; r++) {
      ...
      a[r] += c, (u = this._getColumnWidth(n[r], t, a[r])) != a[r] && (c = u - a[r], a[r] = u), l -= c;
    }
    else if (i.length) for (r = 0; r < i.length; r++) {
      c = Math.round(l / (i.length - r));
      var u, d = i[r];
      a[d] += c, (u = this._getColumnWidth(n[d], t, a[d])) != a[d] && (c = u - a[d], a[d] = u), l -= c;
    }
  ...
  n.rtl && (a = a.reverse());
  var r = [], o = 0, s = e.labels, l = n.scale_height - 1;
  const c = {};
  for (var u = 0; u < a.length; u++) {
    var d = u == a.length - 1, h = a[u];
    h.name || (h.name = t.uid() + "");
    var _ = 1 * h.width, f = this._getGridWidth();
    d && f > o + _ && (h.width = _ = f - o), o += _;
    var k = t._sort && h.name == t._sort.name ? `<div data-column-id="${h.name}" class="gantt_sort gantt_${t._sort.direction}"></div>` : "", v = ["gantt_grid_head_cell", "gantt_grid_head_" + h.name, d ? "gantt_last_cell" : "", i.grid_header_class(h.name, h)].join(" "), b = "width:" + (_ - (d ? 1 : 0)) + "px;", g = h.label || s["column_" + h.name] || s[h.name];
    typeof g == "function" && (g = g.call(t, h.name, h)), g = g || "";
    let p = !1;
    t.config.external_render && t.config.external_render.isElement(g) && (p = !0, c[h.name] = g);
    var m = "<div class='" + v + "' style='" + b + "' " + t._waiAria.gridScaleCellAttrString(h, g) + " data-column-id='" + h.name + "' column_id='" + h.name + "' data-column-name='" + h.name + "' data-column-index='" + u + "'>" + (p ? "<div data-component-container></div>" : g) + k + "</div>";
    r.push(m);
  }
...
var Gn = function(t) {
  return { getWorkHoursArguments: function() {
    var n = arguments[0];
    if (!Et((n = at(n) ? { date: n } : R({}, n)).date)) throw t.assert(!1, "Invalid date argument for getWorkHours method"), new Error("Invalid date argument for getWorkHours method");
    return n;
    ...
    var r, o = arguments[0];
    if (o instanceof ke) return o;
    if (r = at(o) ? new ke(arguments[0], arguments[1], arguments[2], void 0, arguments[3], void 0, arguments[4]) : new ke(o.start_date, o.duration, o.unit, o.step, o.task, null, o.calendar), o.id !== null && o.id !== void 0 && (r.task = o, r.unit = null, r.step = null), r.unit = r.unit || t.config.duration_unit, r.step = r.step || t.config.duration_step, !Et(r.start_date)) throw t.assert(!1, "Invalid start_date argument for calculateEndDate method"), new Error("Invalid start_date argument for calculateEndDate method");
    return r;
  } };
};
function qn() {
}
qn.prototype = { _getIntervals: function(t) {
  ...
  var o = { hours: this._toHoursArray(this._mergeHoursConfig(a.hours, r.hours)), dates: {}, customWeeks: {} };
  const s = (c, u) => {
    for (let d in c.dates) {
      ...
      for (const _ in u.dates) {
        const f = u.dates[_];
        if (_ == d && (o.dates[d] = !(!h || !f)), Array.isArray(h)) {
          const k = Array.isArray(f) ? f : u.hours;
          o.dates[d] = this._toHoursArray(this._mergeHoursConfig(h, k));
        }
      }
    }
  };
  if (s(a, r), s(r, a), a.customWeeks) for (var l in a.customWeeks) o.customWeeks[l] = a.customWeeks[l];
  if (r.customWeeks) for (var l in r.customWeeks) o.customWeeks[l] ? o.customWeeks[l + "_second"] = r.customWeeks[l] : o.customWeeks[l] = r.customWeeks[l];
  return o;
} };
class Ki {
  constructor() {
...
class Qi {
  ...
  getEndDate(n, e, i, a, r) {
    ...
    if (o.endDates[l] === void 0) {
      const u = r(), d = u.valueOf();
      o.endDates[l] = d, o.durations[this._durationCacheKey(s, d)] = e, c = u;
    } else c = new Date(o.endDates[l]);
    return c;
  }
  getDuration(n, e, i, a, r) {
    const o = this._getCacheObject(n, i, a), s = n.valueOf(), l = e.valueOf(), c = this._durationCacheKey(s, l);
    let u;
    if (o.durations[c] === void 0) {
      const d = r();
      o.durations[c] = d.valueOf(), u = d;
    } else u = o.durations[c];
    return u;
  }
  clear() {
    this._cache = {};
...
Pe.prototype = { units: ["year", "month", "week", "day", "hour", "minute"], _clearCaches: function() {
  ...
  (s = a[e + "_start"](new Date(r))).valueOf() != r.valueOf() && (d = !0);
  var h = !1;
  (l = a[e + "_start"](new Date(n))).valueOf() != n.valueOf() && (h = !0);
  for (var _ = !1; r.valueOf() < o.valueOf(); ) {
    if (_ = (u = this._nextDate(r, e, i)).valueOf() > o.valueOf(), this._isWorkTime(r, e)) (d || h && _) && (s = a[e + "_start"](new Date(r)), l = a.add(s, i, e)), d ? (d = !1, u = this._nextDate(s, e, i), c += (l.valueOf() - r.valueOf()) / (l.valueOf() - s.valueOf())) : h && _ ? (h = !1, c += (o.valueOf() - r.valueOf()) / (l.valueOf() - s.valueOf())) : c++;
    else {
  ...
  for (a = B(e == "minute" ? this._getMinutesPerDay : this.getHoursPerDay, this); o.valueOf() < s.valueOf(); ) if (s - o > 27648e5 && o.getDate() === 0) {
    var l = this._largeUnitsCache.getMinutesPerMonth(o);
    e == "hour" && (l /= 60), r += l, o = this.$gantt.date.add(o, 1, "month");
  } else {
    if (s - o > 13824e5) {
      var c = this.$gantt.date.week_start(new Date(o));
      if (o.valueOf() === c.valueOf()) {
  ...
  var l = this.$gantt.date.day_start(new Date(r)), c = r, u = this._getMinutesBetweenSingleDay(o, s), d = this._getMinutesBetweenSingleDay(l, c);
  return u + this._getWorkUnitsForRange(s, l, e, i) + d;
}, _getHoursBetween: function(t, n, e, i) {
  var a = new Date(t), r = new Date(n);
  i = i || 1;
  var o = new Date(a), s = this.$gantt.date.add(this.$gantt.date.day_start(new Date(a)), 1, "day");
  if (r.valueOf() <= s.valueOf()) return Math.round(this._getMinutesBetweenSingleDay(t, n) / 60);
  var l = this.$gantt.date.day_start(new Date(r)), c = r, u = this._getMinutesBetweenSingleDay(o, s, e, i) / 60, d = this._getMinutesBetweenSingleDay(l, c, e, i) / 60, h = u + this._getWorkUnitsForRange(s, l, e, i) + d;
  return Math.round(h);
}, getConfig: function() {
  ...
  if (Array.isArray(t)) {
    ...
    t.forEach(function(s) {
      typeof s == "number" ? n.push(60 * s * 60) : typeof s == "string" && s.split("-").map(function(l) {
        ...
        var c = l.split(":").map(function(d) {
          return d.trim();
        }), u = parseInt(60 * c[0] * 60);
        c[1] && (u += parseInt(60 * c[1])), c[2] && (u += parseInt(c[2])), n.push(u);
      });
    });
  ...
  for (var r = 0, o = t, s = !1; r < n && (!a || !a(o)); ) {
    ...
    i > 0 && (c = new Date(l.valueOf() - 1)), this._isWorkTime(c, e) && !s && r++, o = l;
  }
  ...
  for (var e = this.$gantt.date.add(this.$gantt.date.day_start(new Date(t)), 1, "day"), i = 0, a = n, r = this._getIntervalTimestamp(t, e), o = this._getWorkHours(t), s = 0; s < o.length && i < n; s++) {
    ...
    if (r.end >= l.start && r.start <= l.end) {
      var c = Math.max(l.start, r.start), u = Math.min(l.end, r.end), d = (u - c) / 3600;
      d > a && (d = a, u = c + 60 * a * 60);
  ...
  for (var e = this.$gantt.date.add(this.$gantt.date.hour_start(new Date(t)), 1, "hour"), i = 0, a = n, r = this._getIntervalTimestamp(t, e), o = this._getWorkHours(t), s = 0; s < o.length && i < n; s++) {
    var l = o[s];
    if (r.end >= l.start && r.start <= l.end) {
      var c = Math.max(l.start, r.start), u = Math.min(l.end, r.end), d = (u - c) / 60;
      d > a && (d = a, u = c + 60 * a);
  ...
  for (var e = this.$gantt.date.hour_start(new Date(t)), i = 0, a = n, r = 60 * e.getHours() * 60 + 60 * e.getMinutes() + e.getSeconds(), o = 60 * t.getHours() * 60 + 60 * t.getMinutes() + t.getSeconds(), s = this._getWorkHours(t), l = s.length - 1; l >= 0 && i < n; l--) {
    var c = s[l];
    if (o > c.start && r <= c.end) {
      var u = Math.min(o, c.end), d = Math.max(r, c.start), h = (u - d) / 60;
      h > a && (h = a, d = u - 60 * a);
      var _ = Math.abs(Math.round((u - d) / 60));
      a -= _, i += _, o = d;
    }
  }
  var f = e;
  ...
  a += o.added, i = o.end;
  for (var s = 0, l = [], c = 0; a < n; ) {
    var u = this.$gantt.date.day_start(new Date(i)), d = !1;
    ...
    var _ = n - a, f = this._getTimeOfDayStamp(i, d);
  ...
  for (var o = 0, s = [], l = 0, c = this._isMinutePrecision(i); a < n; ) {
    ...
    if (s.length && l) if (s[0].start >= h && d >= l) {
      ...
      if (_) if (h === f.start && d >= f.durationMinutes) a += f.durationMinutes, i = this.$gantt.date.add(i, f.durationMinutes, "minute");
      else if (d <= f.endMinute - h / 60) a += d, i = this.$gantt.date.add(i, d, "minute");
      else {
        var v = this._getMinutesPerHour(i);
        v <= d ? (a += v, i = c ? this.$gantt.date.add(i, v, "minute") : this._nextDate(i, "hour", e)) : (a += (r = this._addMinutesUntilHourEnd(i, d)).added, i = r.end);
      }
...
const zt = { isLegacyResourceCalendarFormat: function(t) {
  ...
  return null;
} }, ta = (Kt = {}, { getCalendarIdFromMultipleResources: function(t, n) {
  var e = function(a) {
...
function Jn(t) {
  this.$gantt = t.$gantt, this.argumentsHelper = Gn(this.$gantt), this.calendarManager = t, this.$disabledCalendar = new Re(this.$gantt, this.argumentsHelper);
}
Yn.prototype = { _calendars: {}, _convertWorkTimeSettings: function(t) {
  var n = t.days;
  if (n && !t.dates) {
    t.dates = t.dates || {};
  ...
  var n = new Pe(this.$gantt, Gn(this.$gantt));
  n.id = String(ut());
  ...
  if (n.resource_calendars) {
    var e;
    if (e = this._legacyConfig === !1 ? n.resource_property : zt.getResourceProperty(n), Array.isArray(t[e]) && t[e].length) n.dynamic_resource_calendars ? i = ta.getCalendarIdFromMultipleResources(t[e], this) : a = this.getResourceCalendar(t[e]);
...
} }, Jn.prototype = { _getCalendar: function(t) {
  var n;
...
function na(t) {
  ...
  var n = function(o, s) {
    var l = !!(s && s != t.config.root_id && t.isTaskExists(s)) && t.getTask(s), c = null;
    if (l) if (t.config.schedule_from_end) c = t.calculateEndDate({ start_date: l.end_date, duration: -t.config.duration_step, task: o });
    else {
      if (!l.start_date) return n(l, t.getParent(l));
      c = l.start_date;
    }
    else if (t.config.schedule_from_end) c = t.calculateEndDate({ start_date: t._getProjectEnd(), duration: -t.config.duration_step, task: o });
    else {
      const u = t.getTaskByIndex(0), d = t.config.start_date || t.getState().min_date;
      c = u ? u.start_date ? u.start_date : u.end_date ? t.calculateEndDate({ start_date: u.end_date, duration: -t.config.duration_step, task: o }) : d : d;
    }
  ...
  t._set_default_task_timing = function(o) {
    ...
    if (o = o || {}, t.defined(o.id) || (o.id = t.uid()), o.start_date || (o.start_date = n(o, s)), o.text === void 0 && (o.text = t.locale.labels.new_task), o.duration === void 0 && (o.duration = 1), this.isTaskExists(s)) {
      ...
      c.$open = !0, this.config.details_on_create || this.callEvent("onAfterParentExpand", [s, c]);
    }
  ...
  var e = function(o, s) {
    var l = t.getTaskType(o.type), c = { type: l, $no_start: !1, $no_end: !1, scheduled_summary: !1 };
    return l === t.config.types.project && o.auto_scheduling === !1 && (c.scheduled_summary = !0), s || l != o.$rendered_type ? (l == t.config.types.project ? c.$no_end = c.$no_start = !0 : l != t.config.types.milestone && (c.$no_end = !(o.end_date || o.duration), c.$no_start = !o.start_date, t._isAllowedUnscheduledTask(o) && (c.$no_end = c.$no_start = !1)), c) : (c.$no_start = o.$no_start, c.$no_end = o.$no_end, c);
  };
  function i(o) {
  ...
  function a(o, s, l, c) {
    const u = { start: "start_date", end: "end_date" }, d = { start: "$auto_start_date", end: "$auto_end_date" };
    let h;
    h = o.type === t.config.types.project && o.auto_scheduling === !1 ? d : u, s.$no_start && (o[h.start] = l ? new Date(l) : n(o, this.getParent(o))), s.$no_end && (o[h.end] = c ? new Date(c) : this.calculateEndDate({ start_date: o[h.start], duration: this.config.duration_step, task: o })), (s.$no_start || s.$no_end) && this._init_task_timing(o);
  }
  function r(o) {
    var s = null, l = null, c = o !== void 0 ? o : t.config.root_id, u = [];
    return t.eachTask(function(d) {
      const h = t.getTaskType(d.type) == t.config.types.project && d.auto_scheduling === !1;
      t.getTaskType(d.type) == t.config.types.project && !h || t.isUnscheduledTask(d) || (d.rollup && u.push(d.id), !d.start_date || d.$no_start && !h || s && !(s > d.start_date.valueOf()) || (s = d.start_date.valueOf()), !d.end_date || d.$no_end && !h || l && !(l < d.end_date.valueOf()) || (l = d.end_date.valueOf()));
    }, c), { start_date: s ? new Date(s) : null, end_date: l ? new Date(l) : null, rollup: u };
  }
  t._init_task_timing = function(o) {
    var s = e(o, !0), l = o.$rendered_type != s.type, c = s.type;
    l && (o.$no_start = s.$no_start, o.$no_end = s.$no_end, o.$rendered_type = s.type), l && c != this.config.types.milestone && c == this.config.types.project && (this._set_default_task_timing(o), o.$calculate_duration = !1), c == this.config.types.milestone && (o.end_date = o.start_date), o.start_date && o.end_date && o.$calculate_duration !== !1 && (o.duration = this.calculateDuration(o)), o.$calculate_duration || (o.$calculate_duration = !0), o.end_date || (o.end_date = o.start_date), o.duration = o.duration || 0, this.config.min_duration === 0 && o.duration === 0 && (o.$no_end = !1);
    var u = this.getTaskCalendar(o);
    o.$effective_calendar && o.$effective_calendar !== u.id && (i(o), this.config.inherit_calendar && this.isSummaryTask(o) && this.eachTask(function(d) {
      i(d);
    ...
    return this.eachTask(function(c) {
      this.getTaskType(c.type) == t.config.types.project || this.isUnscheduledTask(c) || (s += c.duration);
    }, l), s;
    ...
    if (o) {
      ...
      c.rollup && (l = !0);
      var u = this.getParent(c), d = e(c), h = !0;
      if (l || c.start_date && c.end_date && (d.$no_start || d.$no_end)) {
        const k = c.$auto_start_date ? "$auto_start_date" : "start_date", v = c.$auto_end_date ? "$auto_end_date" : "end_date";
        var _ = c[k].valueOf(), f = c[v].valueOf();
        t.resetProjectDates(c), l || _ != c[k].valueOf() || f != c[v].valueOf() || (h = !1), h && !s && this.refreshTask(c.id, !0), d.scheduled_summary && (h = !0);
      }
    ...
    var l, c, u, d = o.date, h = o.step, _ = o.unit;
    if (!s) return d;
    if (_ == s.unit && h == s.step && +d >= +s.min_date && +d <= +s.max_date) u = Math.floor(t.columnIndexByDate(d)), s.trace_x[u] || (u -= 1, s.rtl && (u = 0)), c = new Date(s.trace_x[u]), l = t.date.add(c, h, _);
    else {
      for (u = Math.floor(t.columnIndexByDate(d)), l = t.date[_ + "_start"](new Date(s.min_date)), s.trace_x[u] && (l = t.date[_ + "_start"](s.trace_x[u])); +l < +d; ) {
    ...
    return o.dir && o.dir == "future" ? l : o.dir && o.dir == "past" || Math.abs(d - c) < Math.abs(l - d) ? c : l;
...
function Ye() {
  ...
  const t = typeof location < "u" ? location.hostname : window.location.hostname, n = ["ZGh0bWx4LmNvbQ==", "ZGh0bWx4Y29kZS5jb20=", "d2ViaXhjb2RlLmNvbQ==", "d2ViaXguaW8=", "cmVwbC5jbw==", "Y3NiLmFwcA==", "cmVwbGl0LmRldg=="];
  for (let e = 0; e < n.length; e++) {
...
const ia = (t) => {
  Ye() || setTimeout(() => {
    ...
    if (!(typeof 1749883503000 > "u")) {
      var e, i;
      setInterval(() => {
        !t.$destroyed && Date.now() - 1749883503000 > 5184e6 && t.message({ type: "error", text: n, expire: -1, id: "evaluation-warning" });
...
function aa(t) {
  ...
  var i = (function() {
    ...
    J(t) || (this.$root.innerHTML = "", this.$root.gantt = this, Ae(this), this.config.layout.id = "main", this.$layout = this.$ui.createView("layout", this.$root, this.config.layout), this.$layout.attachEvent("onBeforeResize", function() {
      ...
      o.className = "gantt_container_resize_watcher", o.tabIndex = -1, r.config.wai_aria_attributes && (o.setAttribute("role", "none"), o.setAttribute("aria-hidden", !0)), r.env.isSalesforce && (r.config.container_resize_method = "timeout"), r.$root.appendChild(o), o.contentWindow ? fn(r, o.contentWindow) : (r.$root.removeChild(o), fn(r, window));
    }(this));
  }).bind(t);
  t.resetLayout = function() {
    i(), a(), this.render();
  }, t._reinit = function(r) {
    this.callEvent("onBeforeGanttReady", []), this._update_flags(), this.$services.getService("templateLoader").initTemplates(this), i(), this.$root = null, r && (this.$root = We(r), a(), this.$mouseEvents.reset(this.$root), function(o) {
...
https://docs.dhtmlx.com/gantt/faq.html#theganttchartisntrenderedcorrectly`);
    ...
    if (!t.isReadonly(o)) {
      ...
      t._delete_task_confirm({ task: o, message: s, title: l, callback: function() {
    ...
    if (Ae(this), J(t)) t.refreshData();
      ...
      if (this.config.preserve_scroll = !1, this.$layout.resize(), this.config.preserve_scroll = l, this.config.preserve_scroll && o) {
        if (s || o.y) {
          var c = t.getScrollState();
          if (+r != +t.dateFromPos(c.x) || c.y != o.y) {
            s = null;
            var u = null;
            r && (s = Math.max(t.posFromDate(r) - t.config.task_scroll_offset, 0)), o.y && (u = o.y), t.scrollTo(s, u);
          }
        }
        this.$layout.getScrollbarsInfo().forEach((d) => {
          const h = t.$ui.getView(d.id), _ = t.utils.dom.isChildOf(h.$view, t.$container);
          d.boundViews.forEach((f) => {
            const k = t.$ui.getView(f);
            d.y && d.y != o.y && k && !_ && k.scrollTo(void 0, 0), d.x_pos && d.x_pos != o.x && k && _ && k.scrollTo(d.x_pos, void 0);
          });
        });
      }
    }
    ...
    var c = null;
    if (!(c = typeof r == "object" ? r : { source: r, target: o, type: this._get_link_type(s, l) }) || !(c.source && c.target && c.type) || c.source == c.target) return !1;
    var u = !0;
    return this.checkEvent("onLinkValidation") && (u = this.callEvent("onLinkValidation", [c])), u;
...
function Ba() {
  this.constants = xi, this.version = "9.0.11", this.license = "evaluation", this.templates = {}, this.ext = {}, this.keys = { edit_save: this.constants.KEY_CODES.ENTER, edit_cancel: this.constants.KEY_CODES.ESC };
}
function za(t) {
  var n = new Ba(), e = new Bn(t), i = {};
  n.plugins = function(l) {
    for (var c in l) if (l[c] && !i[c]) {
      var u = e.getExtension(c);
      u && (u(n), i[c] = !0);
    }
    return i;
  }, n.$services = /* @__PURE__ */ function() {
    var l = {};
    return { services: {}, setService: function(c, u) {
      l[c] = u;
    ...
    return { cache: !0, method: "get", parse: function(c) {
      return typeof c != "string" ? c : (c = c.replace(/^[\s]+/, ""), typeof DOMParser > "u" || bt.isIE ? Y.ActiveXObject !== void 0 && ((u = new Y.ActiveXObject("Microsoft.XMLDOM")).async = "false", u.loadXML(c)) : u = new DOMParser().parseFromString(c, "text/xml"), u);
      var u;
    }, xmltop: function(c, u, d) {
      ...
      return d !== -1 && l.callEvent("onLoadXMLError", ["Incorrect XML", arguments[1], d]), document.createElement("DIV");
    }, xpath: function(c, u) {
      if (u.nodeName || (u = u.responseXML || u), bt.isIE) return u.selectNodes(c) || [];
      for (var d, h = [], _ = (u.ownerDocument || u).evaluate(c, u, null, XPathResult.ANY_TYPE, null); d = _.iterateNext(); ) h.push(d);
      return h;
    }, query: function(c) {
      ...
      return this.query(h);
    }, getSync: function(c, u) {
      var d = Mt("GET", arguments);
      return d.async = !1, this.query(d);
    }, put: function(c, u, d, h) {
      var _ = Mt("PUT", arguments);
      return this.query(_);
    }, del: function(c, u, d) {
      var h = Mt("DELETE", arguments);
      return this.query(h);
    }, post: function(c, u, d, h) {
      arguments.length == 1 ? u = "" : arguments.length == 2 && typeof u == "function" && (d = u, u = "");
      var _ = Mt("POST", arguments);
      return this.query(_);
    }, postSync: function(c, u, d) {
      u = u === null ? "" : String(u);
      var h = Mt("POST", arguments);
      return h.async = !1, this.query(h);
    }, _call: function(c, u, d, h, _, f) {
      return new l.Promise(function(k, v) {
        ...
        h && (b.onreadystatechange = function() {
          if (b.readyState == 4 || g && b.readyState == 3) {
            ...
            setTimeout(function() {
              typeof _ == "function" && _.apply(Y, [{ xmlDoc: b, filePath: u }]), k(b), typeof _ == "function" && (_ = null, b = null);
            }, 0);
          }
        });
        var m = !this || !this.cache;
        if (c == "GET" && m && (u += (u.indexOf("?") >= 0 ? "&" : "?") + "dhxr" + (/* @__PURE__ */ new Date()).getTime() + "=1"), b.open(c, u, h), f) for (var p in f) b.setRequestHeader(p, f[p]);
        else c.toUpperCase() == "POST" || c == "PUT" || c == "DELETE" ? b.setRequestHeader("Content-Type", "application/x-www-form-urlencoded") : c == "GET" && (d = null);
        if (b.setRequestHeader("X-Requested-With", "XMLHttpRequest"), b.send(d), !h) return { xmlDoc: b, filePath: u };
      });
    }, urlSeparator: function(c) {
      return c.indexOf("?") != -1 ? "&" : "?";
    } };
  }(n), n.date = wi(n), n.RemoteEvents = Ti;
  var a = function(l) {
    function c(d) {
      return { target: d.target || d.srcElement, pageX: d.pageX, pageY: d.pageY, clientX: d.clientX, clientY: d.clientY, metaKey: d.metaKey, shiftKey: d.shiftKey, ctrlKey: d.ctrlKey, altKey: d.altKey };
    }
    function u(d, h) {
      this._obj = d, this._settings = h || {}, _t(this);
      var _ = this.getInputMethods();
      this._drag_start_timer = null, l.attachEvent("onGanttScroll", B(function(v, b) {
        this.clearDragTimer();
      }, this));
      for (var f = { passive: !1 }, k = 0; k < _.length; k++) B(function(v) {
        l.event(d, v.down, B(function(g) {
          v.accessor(g) && (g.button !== void 0 && g.button !== 0 || (h.preventDefault && h.selector && dt(g.target, h.selector) && g.preventDefault(), l.config.touch && g.timeStamp && g.timeStamp - 0 < 300 || (this._settings.original_target = c(g), this._settings.original_element_sizes = { ...ct(g, In(d)), width: g.target.offsetWidth, height: g.target.offsetHeight }, l.config.touch ? (this.clearDragTimer(), this._drag_start_timer = setTimeout(B(function() {
            l.getState().lightbox || this.dragStart(d, g, v);
          }, this), l.config.touch_drag)) : this.dragStart(d, g, v))));
        }, this), f);
        var b = document.body;
        l.event(b, v.up, B(function(g) {
          v.accessor(g) && this.clearDragTimer();
        }, this), f);
      }, this)(_[k]);
    }
    return u.prototype = { traceDragEvents: function(d, h) {
      var _ = B(function(m) {
        return this.dragMove(d, m, h.accessor);
      }, this);
      B(function(m) {
        return this.dragScroll(d, m);
      }, this);
      var f = B(function(m) {
      ...
      }, this), k = Dt(l.$root), v = this.config.mousemoveContainer || Dt(l.$root), b = { passive: !1 }, g = B(function(m) {
        return l.eventRemove(v, h.move, f), l.eventRemove(k, h.up, g, b), this.dragEnd(d);
      }, this);
      l.event(v, h.move, f, b), l.event(k, h.up, g, b);
    }, checkPositionChange: function(d) {
      var h = d.x - this.config.pos.x, _ = d.y - this.config.pos.y;
      return Math.sqrt(Math.pow(Math.abs(h), 2) + Math.pow(Math.abs(_), 2)) > this.config.sensitivity;
    }, initDnDMarker: function() {
      var d = this.config.marker = document.createElement("div");
      d.className = "gantt_drag_marker", d.innerHTML = "", document.body.appendChild(d);
    }, backupEventTarget: function(d, h) {
      if (l.config.touch) {
        var _ = h(d), f = _.target || _.srcElement, k = f.cloneNode(!0);
        this.config.original_target = c(_), this.config.original_target.target = k, this.config.backup_element = f, f.parentNode.appendChild(k), f.style.display = "none", (this.config.mousemoveContainer || document.body).appendChild(f);
      }
    }, getInputMethods: function() {
      var d = [];
      if (d.push({ move: "mousemove", down: "mousedown", up: "mouseup", accessor: function(_) {
        return _;
      } }), l.config.touch) {
        var h = !0;
        try {
        ...
        h ? d.push({ move: "touchmove", down: "touchstart", up: "touchend", accessor: function(_) {
        ...
        } }) : Y.navigator.pointerEnabled ? d.push({ move: "pointermove", down: "pointerdown", up: "pointerup", accessor: function(_) {
          return _.pointerType == "mouse" ? null : _;
        } }) : Y.navigator.msPointerEnabled && d.push({ move: "MSPointerMove", down: "MSPointerDown", up: "MSPointerUp", accessor: function(_) {
    ...
    }, dragStart: function(d, h, _) {
      this.config && this.config.started || (this.config = { obj: d, marker: null, started: !1, pos: this.getPosition(h), sensitivity: 4 }, this._settings && R(this.config, this._settings, !0), this.traceDragEvents(d, _), l._prevent_touch_scroll = !0, h.target.closest(".gantt_row") && !l.config.order_branch && (l._prevent_touch_scroll = !1), document.body.classList.add("gantt_noselect"), l.config.touch && this.dragMove(d, h, _.accessor));
    }, dragMove: function(d, h, _) {
      var f = _(h);
      if (!f) return !1;
      if (!this.config.marker && !this.config.started) {
        var k = this.getPosition(f);
        if (l.config.touch || this.checkPositionChange(k)) {
          if (this.config.started = !0, this.config.ignore = !1, l._touch_drag = !0, this.callEvent("onBeforeDragStart", [d, this.config.original_target]) === !1) return this.config.ignore = !0, !1;
          this.backupEventTarget(h, _), this.initDnDMarker(), l._touch_feedback(), this.callEvent("onAfterDragStart", [d, this.config.original_target]);
        } else this.config.ignore = !0;
      }
      return this.config.ignore ? !1 : h.targetTouches && !f.target ? void 0 : (f.pos = this.getPosition(f), this.config.marker.style.left = f.pos.x + "px", this.config.marker.style.top = f.pos.y + "px", this.callEvent("onDragMove", [d, f]), !0);
    }, dragEnd: function(d) {
      ...
      var h = 0, _ = 0;
      return d.pageX || d.pageY ? (h = d.pageX, _ = d.pageY) : (d.clientX || d.clientY) && (h = d.clientX + document.body.scrollLeft + document.documentElement.scrollLeft, _ = d.clientY + document.body.scrollTop + document.documentElement.scrollTop), { x: h, y: _ };
    } }, u;
  }(n);
  ...
  var r = /* @__PURE__ */ function(l) {
    ...
    function u(d, h, _) {
      ...
      l.config[d] && c[_] != f[d] && (h && k[_] || (k[_] = l.date.date_to_str(f[d]), c[_] = f[d]));
    }
    return { initTemplates: function() {
      var d = l.date, h = d.date_to_str, _ = l.config, f = h(_.xml_date || _.date_format, _.server_utc), k = d.str_to_date(_.xml_date || _.date_format, _.server_utc);
      u("date_scale", !0, void 0, l.config, l.templates), u("date_grid", !0, "grid_date_format", l.config, l.templates), u("task_date", !0, void 0, l.config, l.templates), l.mixin(l.templates, { xml_format: void 0, format_date: f, xml_date: void 0, parse_date: k, progress_text: function(v, b, g) {
        return "";
        ...
        v = l.getTask(v);
        var p = l.locale.labels, y = "<b>" + v.text + "</b> " + (b ? p.link_start : p.link_end) + "<br/>";
        return g && (y += "<b> " + (g = l.getTask(g)).text + "</b> " + (m ? p.link_start : p.link_end) + "<br/>"), y;
      ...
      }, tooltip_date_format: d.date_to_str("%Y-%m-%d"), tooltip_text: function(v, b, g) {
  ...
  o.registerProvider("global", function() {
    ...
    return o;
  }), R(n, On), n.Promise = Ci, n.env = bt, function(l) {
    var c = Ai.create();
    R(l, c);
    var u, d = l.createDatastore({ name: "task", type: "treeDatastore", rootId: function() {
      return l.config.root_id;
    }, initItem: B(function(g) {
      ...
      return l.config;
    } }), h = l.createDatastore({ name: "link", initItem: B(function(g) {
    ...
    l.attachEvent("onDestroy", function() {
      ...
      return !0;
    }), d.attachEvent("onBeforeRefreshAll", function() {
      if (!d._skipTaskRecalculation) for (var g = d.getVisibleItems(), m = 0; m < g.length; m++) {
        var p = g[m];
        p.$index = m, p.$local_index = l.getTaskIndex(p.id), l.resetProjectDates(p);
      }
    }), d.attachEvent("onFilterItem", function(g, m) {
      ...
      return !0;
    }), d.attachEvent("onIdChange", function(g, m) {
      l._update_flags(g, m);
      var p = l.getTask(m);
      d.isSilent() || (p.$split_subtask || p.rollup) && l.eachParent(function(y) {
        l.refreshTask(y.id);
      }, m);
    }), d.attachEvent("onAfterUpdate", function(g) {
    ...
    }), d.attachEvent("onBeforeItemMove", function(g, m, p) {
      return !Gt(g, l, d) || (console.log("The placeholder task cannot be moved to another position."), !1);
    }), d.attachEvent("onAfterItemMove", function(g, m, p) {
      var y = l.getTask(g);
    ...
    }), d.attachEvent("onStoreUpdated", function(g, m, p) {
      ...
      return u[g] = Me.getSubtreeLinks(l, g), !0;
    ...
    }), l.attachEvent("onParse", v), b({ source: h, target: l, events: { onItemLoading: "onLinkLoading", onBeforeAdd: "onBeforeLinkAdd", onAfterAdd: "onAfterLinkAdd", onBeforeUpdate: "onBeforeLinkUpdate", onAfterUpdate: "onAfterLinkUpdate", onBeforeDelete: "onBeforeLinkDelete", onAfterDelete: "onAfterLinkDelete", onIdChange: "onLinkIdChange" } }), b({ source: d, target: l, events: { onItemLoading: "onTaskLoading", onBeforeAdd: "onBeforeTaskAdd", onAfterAdd: "onAfterTaskAdd", onBeforeUpdate: "onBeforeTaskUpdate", onAfterUpdate: "onAfterTaskUpdate", onBeforeDelete: "onBeforeTaskDelete", onAfterDelete: "onAfterTaskDelete", onIdChange: "onTaskIdChange", onBeforeItemMove: "onBeforeTaskMove", onAfterItemMove: "onAfterTaskMove", onFilterItem: "onBeforeTaskDisplay", onItemOpen: "onTaskOpened", onItemClose: "onTaskClosed", onBeforeSelect: "onBeforeTaskSelected", onAfterSelect: "onTaskSelected", onAfterUnselect: "onTaskUnselected" } }), l.$data = { tasksStore: d, linksStore: h };
  }(n), n.dataProcessor = Li, n.createDataProcessor = Ni, function(l) {
    l.ext || (l.ext = {});
    for (var c = [Ri, Hi, Bi, zi, ji, Fi, Wi, Vi, Ui], u = 0; u < c.length; u++) c[u] && c[u](l);
  }(n), function(l) {
    (function(c) {
      c.getGridColumn = function(u) {
        for (var d = c.config.columns, h = 0; h < d.length; h++) if (d[h].name == u) return d[h];
        return null;
      }, c.getGridColumns = function() {
        return c.config.columns.slice();
      };
    })(l), ue.prototype.getGridColumns = function() {
      for (var c = this.$getConfig().columns, u = [], d = 0; d < c.length; d++) c[d].hide || u.push(c[d]);
      return u;
    };
  }(n), function(l) {
    ...
    var c = new Yn(l), u = new Jn(c);
    R(l, ea(c, u));
  }(n), na(n), ia(n), function(l) {
    l.getTaskType = function(c) {
      var u = c;
      for (var d in c && typeof c == "object" && (u = c.type), this.config.types) if (this.config.types[d] == u) return u;
    ...
    l._cached_functions = { cache: {}, mode: !1, critical_path_mode: !1, wrap_methods: function(d, h) {
      if (h._prefetch_originals) for (var _ in h._prefetch_originals) h[_] = h._prefetch_originals[_];
      for (h._prefetch_originals = {}, _ = 0; _ < d.length; _++) this.prefetch(d[_], h);
    }, prefetch: function(d, h) {
      var _ = h[d];
      if (_) {
        var f = this;
        h._prefetch_originals[d] = _, h[d] = function() {
      ...
      return _;
    }, cache_value: function(d, h, _) {
      this.is_date(_) && (_ = new Date(_)), d[h] = _;
    }, has_cached_value: function(d, h) {
      return d.hasOwnProperty(h);
    }, get_cached_value: function(d, h) {
      var _ = d[h];
      return this.is_date(_) && (_ = new Date(_)), _;
    }, is_date: function(d) {
      return d && d.getUTCDate;
    }, get_arguments_hash: function(d) {
      for (var h = [], _ = 0; _ < d.length; _++) h.push(this.stringify_argument(d[_]));
      return "(" + h.join(";") + ")";
    }, stringify_argument: function(d) {
      return (d.id ? d.id : this.is_date(d) ? d.valueOf() : d) + "";
    }, activate: function() {
      ...
      this.mode == "auto" ? d.config.highlight_critical_path && (h = _) : this.mode === !0 && (h = _), this.wrap_methods(h, d);
    }, update_if_changed: function(d) {
      (this.critical_path_mode != d.config.highlight_critical_path || this.mode !== d.config.optimize_render) && (this.critical_path_mode = d.config.highlight_critical_path, this.mode = d.config.optimize_render, this.setup(d));
    ...
    l.destructor = function() {
      for (var c in this.clearAll(), this.callEvent("onDestroy", []), this._getDatastores().forEach(function(u) {
        u.destructor();
  ...
  var s = new Ta({ en: ga, ar: sa, be: oa, ca: la, cn: ca, cs: da, da: ua, de: ha, el: _a, es: fa, fa: pa, fi: ma, fr: va, he: ka, hr: ya, hu: ba, id: $a, it: xa, jp: wa, kr: Sa, nb: Ea, nl: Ca, no: Da, pl: Aa, pt: Ma, ro: Ia, ru: La, si: Na, sk: Pa, sv: Ra, tr: Ha, ua: Oa });
  return n.i18n = { addLocale: s.addLocale, setLocale: function(l) {
    if (typeof l == "string") {
      var c = s.getLocale(l);
      c || (c = s.getLocale("en")), n.locale = c;
    } else if (l) if (n.locale) for (var u in l) l[u] && typeof l[u] == "object" ? (n.locale[u] || (n.locale[u] = {}), n.mixin(n.locale[u], l[u], !0)) : n.locale[u] = l[u];
    else n.locale = l;
    const d = n.locale.labels;
    d.gantt_save_btn = d.gantt_save_btn || d.icon_save, d.gantt_cancel_btn = d.gantt_cancel_btn || d.icon_cancel, d.gantt_delete_btn = d.gantt_delete_btn || d.icon_delete;
...
function ja(t) {
  ...
  function f(g, m, p, y) {
    return typeof g != "object" && (g = { text: g, type: m, expire: p, id: y }), g.id = g.id || ut(), g.expire = g.expire || v.expire, g;
  }
  ...
  return t.attachEvent("onMessagePopup", function(g) {
    ...
    var g = _.apply(this, arguments);
    return g.type = g.type || "alert", d(g);
...
const Wa = function(t) {
  ...
  function i(a, r, o, s) {
    ...
    var c = new l.create(r, o, this, t);
    return l.configure && l.configure(c), Fa(c, s), c.$id || (c.$id = o.id || t.uid()), c.$parent || typeof r != "object" || (c.$parent = r), c.$config || (c.$config = o), e[c.$id] && (c.$id = t.uid()), e[c.$id] = c, c;
  }
...
var Va = /* @__PURE__ */ function(t) {
  return function(n) {
    ...
    function d(h) {
      u.detachAll(), h && (u.attach(h, "click", a), u.attach(h, "dblclick", l), u.attach(h, "mousemove", c), u.attach(h, "contextmenu", r));
    }
    return { reset: d, global: function(h, _, f) {
      i(h, _, f, null);
...
function Q(t, n, e, i, a) {
  ...
  const c = [];
  if (t.config.keyboard_navigation && t.getSelectedId() && t.getTask(t.getSelectedId()).$expanded_branch && c.push(t.getSelectedId()), t.$ui.getView("grid") && t.ext.inlineEditors && t.ext.inlineEditors.getState().id) {
    let u = t.ext.inlineEditors.getState().id;
    i.exists(u) && c.push(u);
  }
  return { start: s, end: l, ids: c };
}
var Ga = function(t) {
  var n = /* @__PURE__ */ function(e) {
    ...
    function o(s, l, c) {
      ...
      var u = null, d = null, h = null, _ = null, f = null;
      typeof l.renderer == "function" ? (u = l.renderer, h = ie) : (u = l.renderer.render, d = l.renderer.update, _ = l.renderer.onrender, l.renderer.isInViewPort ? f = l.renderer.isInViewPort : h = l.renderer.getRectangle, h || h === null || (h = ie));
      var k = l.filter;
      return c && c.setAttribute(e.config.layer_attribute, !0), a[s] = { render_item: function(v, b, g, m, p) {
        if (b = b || c, !k || k(v)) {
          var y = m || r(l), w = p || (y ? y.$getConfig() : null), x = g;
          !x && w && w.smart_rendering && (x = y.getViewPort());
          var $ = null;
          !Wt(e) && (h || f) && x ? (f ? f(v, x, y, w, e) : vn(x, h(v, y, w, e))) && ($ = u.call(e, v, y, w, x)) : $ = u.call(e, v, y, w, x), this.append(v, $, b);
          var S = b.nodeType == 11;
          _ && !S && $ && _.call(e, v, $, y);
        ...
        if (g && g.$getConfig().smart_rendering && !Wt(e) && this.rendered && (h || f)) {
          ...
          for (var T = {}, E = (S = 0, v.length); S < E; S++) {
            ...
            $[C.id] = !1, D && D.parentNode ? (f ? f(C, y, g, m, e) : vn(y, h(C, g, m, e))) ? (d && d.call(e, C, D, g, m, y), this.restore(C, p)) : $[C.id] = !0 : (T[v[S].id] = !0, this.render_item(v[S], p, y, g, m));
          }
          for (var S in $) $[S] && this.hide(S);
          if (p.childNodes.length && b.appendChild(p, b), _) {
            var M = {};
            for (var S in this.rendered) x[S] && !T[S] || (M[S] = this.rendered[S], _.call(e, w[S], this.rendered[S], g));
          }
        }
      ...
      }, rendered: i[s], node: c, destructor: function() {
  ...
  return { createGroup: function(e, i, a, r) {
    var o = { tempCollection: [], renderers: {}, container: e, filters: [], getLayers: function() {
      ...
      s && (s.id = s.id || ut(), this.tempCollection.push(s));
      const l = this.container(), c = this.tempCollection;
      for (let u = 0; u < c.length; u++) {
        if (s = c[u], !(this.container() || s && s.container && s.container.isConnected)) continue;
        let d = s.container, h = s.id, _ = s.topmost;
        if (!d.parentNode) if (_) l.appendChild(d);
        else {
          let f = i ? i() : l.firstChild;
          f && f.parentNode == l ? l.insertBefore(d, f) : l.appendChild(d);
        }
        this.renderers[h] = n.getRenderer(h, s, d), r && r(s, t), this.tempCollection.splice(u, 1), u--;
      }
    }, addLayer: function(s) {
      if (s) {
        ...
        s.requestUpdate = function() {
          t.config.smart_rendering && !Wt(t) && l.renderers[s.id] && l.onUpdateRequest(l.renderers[s.id]);
        };
      }
...
function Kn() {
  ...
  function i(r, o, s) {
    o.$getConfig(), r.getVisibleItems().forEach(function(l) {
      var c = function(u, d, h, _) {
        if (!_.isTaskExists(u.source) || !_.isTaskExists(u.target)) return null;
        var f = yn(_.getTask(u.source), d), k = yn(_.getTask(u.target), d);
        if (!f || !k) return null;
  ...
  return function(r, o, s, l, c) {
    ...
    for (var u = [], d = 0; d < t.length; d++) {
      var h = t[d], _ = h.rec;
      _.right < c.x || _.left < c.x_end && _.right > c.x && _.top < c.y_end && _.bottom > c.y && u.push(h.id);
    }
    return { ids: u };
  };
}
function Xn(t, n, e, i, a) {
  var r = e.$gantt.getTask(t.source), o = e.$gantt.getTask(t.target), s = e.getItemTop(r.id), l = e.getItemHeight(r.id), c = e.getItemTop(o.id), u = e.getItemHeight(o.id);
  if (n.y > s + l && n.y > c + u || n.y_end < c && n.y_end < s) return !1;
  var d = e.posFromDate(r.start_date), h = e.posFromDate(r.end_date), _ = e.posFromDate(o.start_date), f = e.posFromDate(o.end_date);
  if (d > h) {
    var k = h;
    h = d, d = k;
  }
  return _ > f && (k = f, f = _, _ = k), d += -100, h += 100, _ += -100, f += 100, !(n.x > h && n.x > f) && !(n.x_end < d && n.x_end < _);
}
function qa(t, n) {
  if (t.view) {
    var e = t.view;
...
var wt = function() {
  function t(n, e, i, a) {
    n && (this.$container = We(n), this.$parent = n), this.$config = R(e, { headerHeight: 33 }), this.$gantt = a, this.$domEvents = a._createDomEventScope(), this.$id = e.id || "c" + ut(), this.$name = "cell", this.$factory = i, this.$externalComponent = null, _t(this);
  }
...
var Zn = function(t) {
  ...
  return W(n, t), n.prototype.destructor = function() {
    ...
    for (var u, d = 0; d < i.length; d++) e[(u = i[d]).$config.scroll] ? c(u) : u.shouldHide() ? s.push(u) : u.shouldShow() ? l(u) : u.isVisible() ? r.push(u) : o.push(u);
    var h = {};
    for (d = 0; d < r.length; d++) r[d].$config.group && (h[r[d].$config.group] = !0);
    for (s.forEach(function(f) {
      f.$config.group && h[f.$config.group] || c(f);
    }), d = 0; d < o.length; d++) if ((u = o[d]).$config.group && h[u.$config.group]) {
      l(u);
    ...
    return e.forEach((a) => {
      let r = {};
      const { visible: o, direction: s, size: l, scrollSize: c, position: u } = a.getScrollState();
      let d = a._getLinkedViews().map((h) => h.$config.id);
      r.id = a.$id, r.visible = o, r.boundViews = d, s === "x" ? (r.x = l, r.x_inner = c, r.x_pos = u || 0) : (r.y = l, r.y_inner = c, r.y_pos = u || 0), i.push(r);
    }), i;
    ...
    if (e.length) for (var a = e[0].$parent._xLayout ? "width" : "height", r = e[0].$parent.getNextSibling(e[0].$id) ? 1 : -1, o = i.value, s = i.isGravity, l = 0; l < e.length; l++) {
      var c = e[l].getSize(), u = r > 0 ? e[l].$parent.getNextSibling(e[l].$id) : e[l].$parent.getPrevSibling(e[l].$id);
      u.$name == "resizer" && (u = r > 0 ? u.$parent.getNextSibling(u.$id) : u.$parent.getPrevSibling(u.$id));
      var d = u.getSize();
      if (s) e[l].$config.gravity = o;
      else if (u[a]) {
        var h = c.gravity + d.gravity, _ = c[a] + d[a], f = h / _;
    ...
    if (this.$root && !this._resizeInProgress && (this.callEvent("onBeforeResize", []), i = !0, this._resizeInProgress = !0), t.prototype.resize.call(this, !0), t.prototype.resize.call(this, !1), i) {
      ...
      if (this.$config.autosize && (this.autosize(this.$config.autosize), a.forEach(function(c) {
        const u = c.$parent, d = u.getContentSize(s);
        s.x && (u.$config.$originalWidthStored || (u.$config.$originalWidthStored = !0, u.$config.$originalWidth = u.$config.width), u.$config.width = d.width), s.y && (u.$config.$originalHeightStored || (u.$config.$originalHeightStored = !0, u.$config.$originalHeight = u.$config.height), u.$config.height = d.height);
      }), l = !0), l)
    ...
    for (var e = 0, i = 0, a = 1e11, r = 0, o = 1e11, s = 0, l = 0; l < this.$cells.length; l++) {
      ...
      this.$cells[l].$config.hidden || (this._xLayout ? (!c.width && c.minWidth ? e += c.minWidth : e += c.width, a += c.maxWidth, i += c.minWidth, r = Math.max(r, c.height), o = Math.min(o, c.maxHeight), s = Math.max(s, c.minHeight)) : (!c.height && c.minHeight ? r += c.minHeight : r += c.height, o += c.maxHeight, s += c.minHeight, e = Math.max(e, c.width), a = Math.min(a, c.maxWidth), i = Math.max(i, c.minWidth)));
    }
    ...
    for (this._visibleCells = a, s = 0; s < o.length; s++) {
      var l = (r = o[s]).$cells, c = !0;
      l.forEach(function(u) {
        u.$config.hidden || u.$config.resizer || (c = !1);
      }), r.$config.hidden = c;
    }
    ...
    for (var s = 0; s < this._sizes.length; s++) if (!(a = this.$cells[s]).$config.hidden) {
      ...
      var c = a.$view, u = this._xLayout ? "marginRight" : "marginBottom";
      s !== this.$cells.length - 1 && (c.style[u] = l + "px", this._free -= l), r = this._sizes[s], this._xLayout ? r.width || (this._gravity += r.gravity) : r.height || (this._gravity += r.gravity);
    }
  ...
  function n(e, i, a) {
    ...
    function l(c) {
      var u = c.pageX, d = c.pageY;
      return c.touches && (u = c.touches[0].pageX, d = c.touches[0].pageY), { x: u, y: d };
    }
    return s._moveHandler = function(c) {
      s._moveResizer(s._resizer, l(c).x, l(c).y);
    }, s._upHandler = function(c) {
      var u = s._getNewSizes();
      s.callEvent("onResizeEnd", [r, o, u ? u.back : 0, u ? u.front : 0]) !== !1 && s._setSizes(), s._setBackground(!1), s._clearResizer(), s._clearListeneres(), c.touches && (s.$gantt._prevent_touch_scroll = !1);
    ...
    }, s._startDND = function(c) {
      if (this._callStartDNDEvent() !== !1) {
        var u = !1;
        this._eachGroupItem(function(d) {
          d._getSiblings(), d._callStartDNDEvent() === !1 && (u = !0);
        }), u || (s._moveHandler(c), s.$domEvents.attach(document, "mousemove", s._moveHandler), s.$domEvents.attach(document, "mouseup", s._upHandler));
      }
    }, s._cancelDND = function() {
      s._setBackground(!1), s._clearResizer(), s._clearListeneres();
    }, s._startOnMove = function(c) {
      c.touches && (s.$gantt._touch_drag = !0, s.$gantt._prevent_touch_scroll = !0), s._isPosChanged(c) && (s._clearListeneres(), s._startDND(c));
    }, s._downHandler = function(c) {
      s._getSiblings(), s._behind.$config.collapsed || s._front.$config.collapsed || (s._setBackground(!0), s._resizer = s._setResizer(), s._positions = { x: l(c).x, y: l(c).y, timestamp: Date.now() }, s.$domEvents.attach(document, "mousemove", s._startOnMove), s.$domEvents.attach(document, "mouseup", s._cancelDND));
    }, s.$name = "resizer", s;
  }
  return W(n, t), n.prototype.init = function() {
    ...
    var c, u, d = this._getDirection(e, i);
    if (d === -1) {
      if (u = s - a, c = r - Math.abs(a), s - a > this._front.$config.maxWidth) return;
      Math.abs(a) >= r && (a = -Math.abs(r - 2)), r - Math.abs(a) <= o && (a = -Math.abs(r - o));
    } else u = s - Math.abs(a), c = r + a, r + a > this._behind.$config.maxWidth && (a = this._behind.$config.maxWidth - r), Math.abs(a) >= s && (a = s - 2), s - Math.abs(a) <= l && (a = Math.abs(s - l));
    return d === -1 ? (u = s - a, c = r - Math.abs(a)) : (u = s - Math.abs(a), c = r + a), { size: a, newFrontSide: u, newBehindSide: c };
    ...
    this._eachGroupItem(function(l) {
      ...
      this._xMode ? c.x += l._behind.$config.width - this._behind.$config.width : c.y += l._behind.$config.height - this._behind.$config.height, l._positions = c, o.push(l._getResizePosition(e, i));
    });
  ...
  function e(a, r, o, s) {
    ...
    this.$config = R(r, { scroll: "x" }), l._scrollHorizontalHandler = B(l._scrollHorizontalHandler, l), l._scrollVerticalHandler = B(l._scrollVerticalHandler, l), l._outerScrollVerticalHandler = B(l._outerScrollVerticalHandler, l), l._outerScrollHorizontalHandler = B(l._outerScrollHorizontalHandler, l), l._mouseWheelHandler = B(l._mouseWheelHandler, l), this.$config.hidden = !0;
    var c = s.config.scroll_size;
    return s.env.isIE && (c += 1), this._isHorizontal() ? (l.$config.height = c, l.$parent.$config.height = c) : (l.$config.width = c, l.$parent.$config.width = c), this.$config.scrollPosition = 0, l.$name = "scroller", l;
  }
  function i(a, r) {
  ...
  return W(e, t), e.prototype.init = function(a) {
    ...
    for (var a = this._getLinkedViews(), r = this._isVertical() ? "gantt_layout_outer_scroll gantt_layout_outer_scroll_vertical" : "gantt_layout_outer_scroll gantt_layout_outer_scroll_horizontal", o = 0; o < a.length; o++) xt(a[o].$view || a[o].getNode(), r);
    ...
    for (var a, r = 0, o = 0, s = this._isHorizontal(), l = this._getLinkedViews(), c = s ? "scrollWidth" : "scrollHeight", u = s ? "contentX" : "contentY", d = s ? "x" : "y", h = this._getScrollOffset(), _ = 0; _ < l.length; _++) if ((a = l[_]) && a.$content && a.$content.getSize && !a.$config.hidden) {
      var f, k = a.$content.getSize();
      if (f = k.hasOwnProperty(c) ? k[c] : k[u], h) k[u] > k[d] && k[u] > r && f > k[d] - h + 2 && (r = f + (s ? 0 : 2), o = k[d]);
      else {
        var v = Math.max(k[u] - f, 0);
        (f += v) > Math.max(k[d] - v, 0) && f > r && (r = f, o = k[d]);
      }
    }
    ...
    if (this._isVertical()) {
      ...
      var l = o[s].$parent.$cells, c = l[l.length - 1];
    ...
    s ? this._isVertical() ? (this.$config.outerSize = this.$config.height - s + 3, o.style.height = this.$config.outerSize + "px", o.style.top = s - 1 + "px", xt(o, this.$parent._borders.top), xt(o.parentNode, "gantt_task_vscroll")) : (this.$config.outerSize = this.$config.width - s + 1, o.style.width = this.$config.outerSize + "px") : (o.style.top = "auto", Nt(o, this.$parent._borders.top), Nt(o.parentNode, "gantt_task_vscroll"), this.$config.outerSize = this.$config.height), l.style[r] = a + "px";
    ...
    if (this._checkWheelTarget(r)) {
      ...
      var c = bt.isFF, u = c ? a.deltaX : a.wheelDeltaX, d = c ? a.deltaY : a.wheelDelta, h = -20;
      if (c) {
        const y = parseInt(navigator.userAgent.split("Firefox/")[1]);
        h = y <= 87 ? a.deltaMode !== 0 ? -40 : -10 : y <= 90 ? -3 : y <= 96 ? -1.5 : -1;
      }
      var _ = c ? u * h * s.x : 2 * u * s.x, f = c ? d * h * s.y : d * s.y, k = this.$gantt.config.horizontal_scroll_key;
  ...
  function a(d) {
    var h = /^rgba?\(([\d]{1,3}), *([\d]{1,3}), *([\d]{1,3}) *(,( *[\d.]+ *))?\)$/i.exec(d);
  ...
  function o(d, h, _) {
    return (d + "" + h + _.bottomBorderColor + _.rightBorderColor).replace(/[^\w\d]/g, "");
  }
  function s(d, h) {
    e[d] = h;
  }
  function l(d, h, _) {
    var f = Math.floor(500 / d) || 1, k = Math.floor(500 / h) || 1, v = document.createElement("canvas");
    v.height = h * k, v.width = d * f;
    var b = v.getContext("2d");
    return function(m, p, y, w, x, $) {
      var S = x.createImageData(p * w, m * y);
      S.imageSmoothingEnabled = !1;
      for (var T = 1 * $.rightBorderWidth, E = a($.rightBorderColor), C = 0, D = 0, M = 0, I = 1; I <= w; I++) for (C = I * p - 1, M = 0; M < T; M++) for (D = 0; D < m * y; D++) g(C - M, D, E, S);
      var A = 1 * $.bottomBorderWidth, L = a($.bottomBorderColor);
      D = 0;
      for (var P = 1; P <= y; P++) for (D = P * m - 1, M = 0; M < A; M++) for (C = 0; C < p * w; C++) g(C, D - M, L, S);
      x.putImageData(S, 0, 0);
    }(h, d, k, f, b, _), v.toDataURL();
    function g(m, p, y, w) {
      var x = 4 * (p * (d * f) + m);
  ...
  function c(d) {
    return "gantt-static-bg-" + d;
  }
  function u() {
    var d = document.createElement("div");
    d.className = "gantt_task_cell";
    var h = document.createElement("div");
    return h.className = "gantt_task_row", h.appendChild(d), h;
  }
  return { render: function(d, h, _, f, k) {
    if ((h.static_background || h.timeline_placeholder) && document.createElement("canvas").getContext) {
      ...
      var v = function(p) {
      ...
      }(d), b = function(p, y, w, x) {
        var $ = {}, S = function(A) {
          for (var L = A.width, P = {}, N = 0; N < L.length; N++) 1 * L[N] && (P[L[N]] = !0);
          return P;
        }(w), T = x, E = "";
        for (var C in S) {
          var D = 1 * C, M = o(D, T, p);
          if (!r(M)) {
            var I = l(D, T, p);
            s(M, I), E += "." + c(M) + "{ background-image: url('" + I + "');}";
          }
          $[C] = c(M);
        }
        return E && (function() {
        ...
        if (n.isIE) {
          ...
          I.indexOf("Windows NT 6.2") == -1 && I.indexOf("Windows NT 6.1") == -1 && I.indexOf("Windows NT 6.0") == -1 || (M = 2e4);
        }
        for (var A = 0; A < C.length; A++) {
          var L = C[A];
          if (L != S && S !== void 0 || A == C.length - 1 || E > M) {
            for (var P = x, N = 0, z = Math.floor(M / y.row_height) * y.row_height, O = E; P > 0; ) {
              var H = Math.min(P, z);
              P -= z, ($ = document.createElement("div")).style.height = H + "px", $.style.position = "absolute", $.style.top = N + "px", $.style.left = D + "px", $.style.pointerEvents = "none", $.style.whiteSpace = "no-wrap", $.className = p[S || L], A == C.length - 1 && (O = L + O - 1), $.style.width = O + "px", T.push($), N += H;
            }
            E = 0, D += O;
          }
          L && (E += L, S = L);
...
var Je = function(t, n, e, i) {
  this.$config = R({}, n || {}), this.$scaleHelper = new Fn(i), this.$gantt = i, this._posFromDateCache = {}, this._timelineDragScroll = null, R(this, Un(this)), _t(this);
};
function ye(t) {
  ...
  return { init: function() {
    ...
    a.$config.fetchTasks && a.silent(function() {
      ...
      for (var l in a.eachItem(function(c) {
        if (c.$role != "task") {
          var u = i.getResourceAssignments(c.id), d = {};
          u.sort(function(h, _) {
            const f = a.pull, k = f[`${h.task_id}_${h.resource_id}`], v = f[`${_.task_id}_${_.resource_id}`];
            return k && v ? k.$local_index - v.$local_index : 0;
          }), u.forEach(function(h) {
            if (!d[h.task_id] && i.isTaskExists(h.task_id)) {
              d[h.task_id] = !0;
              var _, f = i.getTask(h.task_id);
              (_ = a.$config.copyOnParse ? i.copy(f) : Object.create(f)).id = f.id + "_" + c.id, _.$task_id = f.id, _.$resource_id = c.id, _[a.$parentProperty] = c.id, _.$role = "task", r.push(_), o[_.id] = !0;
            }
          });
        } else s[c.id] = !0;
...
Je.prototype = { init: function(t) {
    ...
    if (i === null && t.isVisible()) {
      ...
      if (o.static_background || o.timeline_placeholder) {
        ...
        if (l || ((l = document.createElement("div")).className = "gantt_task_bg", t.$task_bg_static = l, t.$task_bg.nextSibling ? t.$task_data.insertBefore(l, t.$task_bg.nextSibling) : t.$task_data.appendChild(l)), s) {
          ...
          o.timeline_placeholder && (c = o.timeline_placeholder.height || t.$task_data.offsetHeight || 99999), n.render(l, o, t.getScale(), c, t.getItemHeight(a ? a.id : null));
        }
  ...
  if (this.isVisible()) {
    ...
    var l = r.prepareConfigs(o, t.min_column_width, s, i - 1, a.min_date, a.max_date, t.rtl), c = this._tasks = l[l.length - 1];
  ...
  var a = this.$getConfig(), r = this.$gantt.templates, o = [], s = null, l = null, c = t.format || t.template || t.date;
  typeof c == "string" && (c = this.$gantt.date.date_to_str(c));
  var u = 0, d = t.count;
  !a.smart_scales || isNaN(n) || isNaN(e) || (u = Bt(t.left, n), d = Bt(t.left, e) + 1), l = t.css || function() {
  }, !t.css && a.inherit_scale_class && (l = r.scale_cell_class);
  for (var h = u; h < d && t.trace_x[h]; h++) {
    s = new Date(t.trace_x[h]);
    var _ = c.call(this, s), f = t.width[h];
    t.height;
  ...
  var r = Bt(e, n), o = this._getClosestVisibleColumn(r, e, i), s = e[o], l = this._tasks.trace_index_transition;
  if (!s) return l ? l[0] : 0;
  var c = (t - e[o]) / this._getColumnDuration(this._tasks, e[o]);
  return l ? l[o] + (1 - c) : o + c;
...
const nr = { init: function(t, n) {
  ...
  r.attachEvent("onBeforeFocus", function(o) {
    ...
    if (clearTimeout(a), s) {
      var l = s.columnName, c = s.id, u = e.getState();
      if (e.isVisible() && u.id == c && u.columnName === l) return !1;
    }
    return !0;
    ...
    var l = t.getState(), c = t.locateCell(s.target);
    ...
    var l = t.locateCell(s.target), c = !!l && t.getEditorConfig(l.columnName), u = t.getState(), d = i.constants.KEY_CODES, h = s.keyCode, _ = !1;
    switch (h) {
      case d.ENTER:
        t.isVisible() ? (t.save(), s.preventDefault(), _ = !0) : c && !(s.ctrlKey || s.metaKey || s.shiftKey) && (e.startEdit(l.id, l.columnName), s.preventDefault(), _ = !0);
        break;
      case d.ESC:
      ...
      case d.LEFT:
      case d.RIGHT:
        (c && t.isVisible() || u.editorType === "date") && (_ = !0);
        break;
      case d.SPACE:
        t.isVisible() && (_ = !0), c && !t.isVisible() && (e.startEdit(l.id, l.columnName), s.preventDefault(), _ = !0);
        break;
      case d.DELETE:
      ...
      default:
        if (t.isVisible()) _ = !0;
          ...
          k.alt || v && r.getCommandHandler(o, "taskCell") || c && !t.isVisible() && (e.startEdit(l.id, l.columnName), _ = !0);
        }
    }
    return !_;
  });
...
function rr(t) {
  ...
  return W(e, n), R(e.prototype, { show: function(i, a, r, o) {
    var s = r.min || 0, l = r.max || 100, c = `<div role='cell'><input type='number' min='${s}' max='${l}' name='${a.name}' title='${a.name}'></div>`;
    o.innerHTML = c, o.oninput = function(u) {
...
function sr(t) {
  ...
  return W(e, n), R(e.prototype, { show: function(i, a, r, o) {
    for (var s = `<div role='cell'><select name='${a.name}' title='${a.name}'>`, l = [], c = r.options || [], u = 0; u < c.length; u++) l.push("<option value='" + r.options[u].key + "'>" + c[u].label + "</option>");
...
function or(t) {
  ...
  return W(r, n), R(r.prototype, { show: function(o, s, l, c) {
    i || (i = t.date.date_to_str(e)), a || (a = t.date.str_to_date(e));
    var u = null, d = null;
    u = typeof l.min == "function" ? l.min(o, s) : l.min, d = typeof l.max == "function" ? l.max(o, s) : l.max;
    var h = `<div style='width:140px' role='cell'><input type='date' ${u ? " min='" + i(u) + "' " : ""} ${d ? " max='" + i(d) + "' " : ""} name='${s.name}' title='${s.name}'></div>`;
    c.innerHTML = h, c.oninput = function(_) {
      _.target.value && (u || d) && (+t.date.str_to_date("%Y-%m-%d")(_.target.value) < +u && (_.target.value = t.date.date_to_str("%Y-%m-%d")(u)), +t.date.str_to_date("%Y-%m-%d")(_.target.value) > +d && (_.target.value = t.date.date_to_str("%Y-%m-%d")(d)));
    };
  }, set_value: function(o, s, l, c) {
    o && o.getFullYear ? this.get_input(c).value = i(o) : this.get_input(c).value = o;
  }, is_valid: function(o, s, l, c) {
...
function lr(t) {
  ...
  function a(l, c) {
    for (var u = (l || "").split(c.delimiter || ","), d = 0; d < u.length; d++) {
      var h = u[d].trim();
      h ? u[d] = h : (u.splice(d, 1), d--);
    }
    return u.sort(), u;
  }
  function r(l, c, u) {
    for (var d = l.$target, h = [], _ = 0; _ < d.length; _++) {
      var f = u.getLink(d[_]);
      h.push(i(c).format(f));
    }
  ...
  return W(e, n), R(e.prototype, { show: function(l, c, u, d) {
    var h = `<div role='cell'><input type='text' name='${c.name}' title='${c.name}'></div>`;
    d.innerHTML = h;
  }, hide: function() {
  }, set_value: function(l, c, u, d) {
    this.get_input(d).value = r(l, u.editor, t);
  }, get_value: function(l, c, u) {
    return a(this.get_input(u).value || "", c.editor);
  }, save: function(l, c, u) {
    var d = s(t.getTask(l), this.get_value(l, c, u), c.editor);
    (d.add.length || d.remove.length) && t.batchUpdate(function() {
      d.add.forEach(function(h) {
        t.addLink(h);
      }), d.remove.forEach(function(h) {
        t.deleteLink(h);
      }), t.autoSchedule && t.autoSchedule();
    });
  }, is_changed: function(l, c, u, d) {
    var h = this.get_value(c, u, d), _ = a(r(l, u.editor, t), u.editor);
...
function ur(t) {
  t.config.editor_types = { text: new (ar())(), number: new (rr())(), select: new (sr())(), date: new (or(t))(), predecessor: new (lr(t))(), duration: new (cr(t))() };
}
function hr(t) {
  ...
  var i = { init: ur, createEditors: function(a) {
    function r(d, h) {
      ...
      }(d, h), k = document.createElement("div");
      k.className = "gantt_grid_editor_placeholder", k.setAttribute(a.$config.item_attribute, d), k.setAttribute(a.$config.bind + "_id", d), k.setAttribute("data-column-name", h);
      var v = function(b, g) {
    ...
    var o = dr(t), s = [], l = [], c = null, u = { _itemId: null, _columnName: null, _editor: null, _editorType: null, _placeholder: null, locateCell: function(d) {
      if (!et(d, a.$grid)) return null;
      var h = tt(d, a.$config.item_attribute), _ = tt(d, "data-column-name");
      if (h && _) {
        var f = _.getAttribute("data-column-name");
        return { id: h.getAttribute(a.$config.item_attribute), columnName: f };
      }
      return null;
    }, getEditorConfig: function(d) {
      return a.getColumn(d).editor;
    }, init: function() {
      var d = n.getMapping();
      d.init && d.init(this, a), c = a.$gantt.getDatastore(a.$config.bind);
      var h = this;
      s.push(c.attachEvent("onIdChange", function(_, f) {
        h._itemId == _ && (h._itemId = f);
      })), s.push(c.attachEvent("onStoreUpdated", function() {
    ...
    }, startEdit: function(d, h) {
      if (this.isVisible() && this.save(), !c.exists(d)) return;
      var _ = { id: d, columnName: h };
      if (t.isReadonly(c.getItem(d))) return void this.callEvent("onEditPrevent", [_]);
      ...
      var _ = { id: d, columnName: h }, f = a.getColumn(_.columnName), k = this.getEditorConfig(f.name);
      if (!k) return;
      var v = a.$getConfig().editor_types[k.type], b = r(_.id, _.columnName);
      a.$grid_data.appendChild(b);
      const g = (function() {
        this._editor = v, this._placeholder = b, this._itemId = _.id, this._columnName = _.columnName, this._editorType = k.type;
        var p = n.getMapping();
        p.onShow && p.onShow(this, b, a), b._onReMount = (function() {
          this.setValue();
        }).bind(this);
      }).bind(this), m = v.show(_.id, f, k, b);
      if (m && m.then) return m.then(() => {
        g();
      });
      g();
    }, setValue: function() {
      var d = this.getState(), h = d.id, _ = d.columnName, f = a.getColumn(_), k = c.getItem(h), v = this.getEditorConfig(_);
      if (v) {
        var b = k[v.map_to];
      ...
      var d = a.getColumn(this._columnName);
      return this._editor.get_value(this._itemId, d, this._placeholder);
    }, _getItemValue: function() {
      var d = this.getEditorConfig(this._columnName);
      if (d) {
        var h = t.getTask(this._itemId)[d.map_to];
        return d.map_to == "auto" && (h = c.getItem(this._itemId)), h;
      }
    }, isChanged: function() {
      var d = a.getColumn(this._columnName), h = this._getItemValue();
      return this._editor.is_changed(h, this._itemId, d, this._placeholder);
    }, hide: function() {
      if (this._itemId) {
        var d = this._itemId, h = this._columnName, _ = n.getMapping();
        _.onHide && _.onHide(this, this._placeholder, a), this._itemId = null, this._columnName = null, this._editorType = null, this._placeholder && (this._editor && this._editor.hide && this._editor.hide(this._placeholder), this._editor = null, this._placeholder.parentNode && this._placeholder.parentNode.removeChild(this._placeholder), this._placeholder = null, this.callEvent("onEditEnd", [{ id: d, columnName: h }]));
      }
    }, save: function() {
      if (this.isVisible() && c.exists(this._itemId) && this.isChanged()) {
        var d = this._itemId, h = this._columnName;
        if (c.exists(d)) {
          var _ = c.getItem(d), f = this.getEditorConfig(h), k = { id: d, columnName: h, newValue: this.getValue(), oldValue: this._getItemValue() };
          if (this.callEvent("onBeforeSave", [k]) !== !1 && (!this._editor.is_valid || this._editor.is_valid(k.newValue, k.id, a.getColumn(h), this._placeholder))) {
            ...
            v != "auto" ? (_[v] = b, o(_, v, t.config.inline_editors_date_processing), c.updateItem(d)) : this._editor.save(d, a.getColumn(h), this._placeholder), this.callEvent("onSave", [k]);
          }
          this.hide();
        }
      } else this.hide();
    }, _findEditableCell: function(d, h) {
      var _ = d, f = a.getGridColumns()[_], k = f ? f.name : null;
      if (k) {
        for (; k && !this.getEditorConfig(k); ) k = this._findEditableCell(d + h, h);
        return k;
      }
      return null;
    }, getNextCell: function(d) {
      return this._findEditableCell(a.getColumnIndex(this._columnName, !0) + d, d);
    }, getFirstCell: function() {
      return this._findEditableCell(0, 1);
    }, getLastCell: function() {
      return this._findEditableCell(a.getGridColumns().length - 1, -1);
    }, editNextCell: function(d) {
      var h = this.getNextCell(1);
      if (h) {
        var _ = this.getNextCell(1);
        _ && this.getEditorConfig(_) && this.startEdit(this._itemId, _);
      } else if (d && this.moveRow(1)) {
        var f = this.moveRow(1);
        (h = this.getFirstCell()) && this.getEditorConfig(h) && this.startEdit(f, h);
      }
    }, editPrevCell: function(d) {
      var h = this.getNextCell(-1);
      if (h) {
        var _ = this.getNextCell(-1);
        _ && this.getEditorConfig(_) && this.startEdit(this._itemId, _);
      ...
      for (var h = d > 0 ? t.getNext : t.getPrev, _ = (h = t.bind(h, t))(this._itemId); t.isTaskExists(_) && t.isReadonly(t.getTask(_)); ) _ = h(_);
      return _;
    }, editNextRow: function(d) {
      var h = this.getState().id;
      if (t.isTaskExists(h)) {
        var _ = null;
        _ = d ? this.moveRow(1) : t.getNext(h), t.isTaskExists(_) && this.startEdit(_, this._columnName);
      }
    }, editPrevRow: function(d) {
      var h = this.getState().id;
      if (t.isTaskExists(h)) {
        var _ = null;
        _ = d ? this.moveRow(-1) : t.getPrev(h), t.isTaskExists(_) && this.startEdit(_, this._columnName);
      }
...
function It(t, n, e, i, a) {
  ...
  var s = e.posFromDate(t.start_date), l = e.posFromDate(t.end_date), c = Math.min(s, l) - 200, u = Math.max(s, l) + 200;
  return !(c > n.x_end || u < n.x);
}
function be(t) {
  function n(r, o, s) {
    if (t._isAllowedUnscheduledTask(r) || !t._isTaskInTimelineLimits(r)) return;
    var l = o.getItemPosition(r), c = s, u = o.$getTemplates(), d = t.getTaskType(r.type), h = o.getBarHeight(r.id, d == c.types.milestone), _ = 0;
    d == c.types.milestone && (_ = (h - l.height) / 2);
    var f = Math.floor((o.getItemHeight(r.id) - h) / 2);
    const k = t.config.baselines && r.baselines && r.baselines.length, v = t.config.baselines && (t.config.baselines.render_mode == "separateRow" || t.config.baselines.render_mode == "individualRow");
    if (k && v && r.bar_height !== "full" && r.bar_height < r.row_height) if (d === c.types.milestone) {
      let T = o.getBarHeight(r.id, !0), E = Math.sqrt(2 * T * T);
      f = Math.floor((E - h) / 2) + 2;
    } else f = 2;
    d == c.types.milestone && (l.left -= Math.round(h / 2), l.width = h);
    var b = document.createElement("div"), g = Math.round(l.width);
    o.$config.item_attribute && (b.setAttribute(o.$config.item_attribute, r.id), b.setAttribute(o.$config.bind + "_id", r.id)), c.show_progress && d != c.types.milestone && function(T, E, C, D, M) {
      var I = 1 * T.progress || 0;
      C = Math.max(C, 0);
      var A = document.createElement("div"), L = Math.round(C * I);
      L = Math.min(C, L), A.style.width = L + "px", A.className = "gantt_task_progress", A.innerHTML = M.progress_text(T.start_date, T.end_date, T), D.rtl && (A.style.position = "absolute", A.style.right = "0px");
      var P = document.createElement("div");
      P.className = "gantt_task_progress_wrapper", P.appendChild(A), E.appendChild(P);
      const N = !t.isReadonly(T), z = t.ext.dragTimeline && t.ext.dragTimeline._isDragInProgress();
      if (t.config.drag_progress && (N || z)) {
        var O = document.createElement("div"), H = L;
        D.rtl && (H = C - L), O.style.left = H + "px", O.className = "gantt_task_progress_drag", O.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="12" height="9" viewBox="0 0 12 9" fill="none">
<path d="M5.58397 1.52543C5.78189 1.22856 6.21811 1.22856 6.41602 1.52543L10.5475 7.72265C10.769 8.05493 10.5308 8.5 10.1315 8.5L1.86852 8.5C1.46917 8.5 1.23097 8.05493 1.45249 7.72265L5.58397 1.52543Z" fill="var(--dhx-gantt-progress-handle-background)" stroke="var(--dhx-gantt-progress-handle-border)"/>
</svg>`, A.appendChild(O), E.appendChild(O);
      }
    }(r, b, g, c, u);
    ...
    var p = function(T, E, C, D) {
      ...
      E && I.push(E);
      var A = t.getState(), L = t.getTask(C);
      if (t.getTaskType(L.type) == M.types.milestone ? I.push("gantt_milestone") : t.getTaskType(L.type) == M.types.project && I.push("gantt_project"), I.push("gantt_bar_" + t.getTaskType(L.type)), t.isSummaryTask(L) && I.push("gantt_dependent_task"), t.isSplitTask(L) && (M.open_split_tasks && !L.$open || !M.open_split_tasks) && I.push("gantt_split_parent"), M.select_task && t.isSelectedTask(C) && I.push("gantt_selected"), C == A.drag_id && (I.push("gantt_drag_" + A.drag_mode), A.touch_drag && I.push("gantt_touch_" + A.drag_mode)), A.link_source_id == C && (I.push("gantt_link_source"), A.link_from_start ? I.push("gantt_link_from_start") : I.push("gantt_link_from_end")), A.link_target_id == C && I.push("gantt_link_target"), M.highlight_critical_path && t.isCriticalTask && t.isCriticalTask(L) && I.push("gantt_critical_task"), A.link_landing_area && A.link_target_id && A.link_source_id && A.link_target_id != A.link_source_id && (A.link_target_id == C || A.link_source_id == C)) {
        var P = A.link_source_id, N = A.link_from_start, z = A.link_to_start, O = "";
        O = t.isLinkAllowed(P, C, N, z) ? z ? "link_start_allow" : "link_finish_allow" : z ? "link_start_deny" : "link_finish_deny", I.push(O);
      }
      return I.join(" ");
    ...
    var w = function(T, E, C, D) {
      ...
      return D && (I = { type: "marginRight", value: D }), e(T, C.leftside_text, M, I);
    }(r, c, u, _);
    w && b.appendChild(w), w = function(T, E, C, D) {
      var M = "gantt_right " + i(!!E.rtl, T), I = null;
      return D && (I = { type: "marginLeft", value: D }), e(T, C.rightside_text, M, I);
    }(r, c, u, _), w && b.appendChild(w), t._waiAria.setTaskBarAttr(r, b);
    var x = t.getState();
    const $ = !t.isReadonly(r), S = t.ext.dragTimeline && t.ext.dragTimeline._isDragInProgress();
    return ($ || S) && (c.drag_resize && !t.isSummaryTask(r) && d != c.types.milestone && a(b, "gantt_task_drag", r, function(T) {
      var E = document.createElement("div");
  ...
  function e(r, o, s, l) {
    if (!o) return null;
    var c = o(r.start_date, r.end_date, r);
    if (!c) return null;
    var u = document.createElement("div");
    return u.className = "gantt_side_content " + s, u.innerHTML = c, l && (u.style[l.type] = Math.abs(l.value) + "px"), u;
  }
  function i(r, o) {
    ...
    for (var l in s) for (var c = o[l], u = 0; u < c.length; u++) for (var d = t.getLink(c[u]), h = 0; h < s[l].length; h++) if (d.type == s[l][h]) return "gantt_link_crossing";
    return "";
  }
  function a(r, o, s, l, c, u) {
    var d, h = t.getState();
    +s.start_date >= +h.min_date && ((d = l([o, c.rtl ? "task_right" : "task_left", "task_start_date"].join(" "))).setAttribute("data-bind-property", "start_date"), u && (d.style.marginLeft = u + "px"), r.appendChild(d)), +s.end_date <= +h.max_date && ((d = l([o, c.rtl ? "task_left" : "task_right", "task_end_date"].join(" "))).setAttribute("data-bind-property", "end_date"), u && (d.style.marginRight = u + "px"), r.appendChild(d));
  }
  return function(r, o, s) {
    var l = (s = o.$getConfig()).type_renderers[t.getTaskType(r.type)], c = n;
    return l ? l.call(t, r, function(u) {
      return c.call(t, u, o, s);
    }, o) : c.call(t, r, o, s);
  };
}
function _r(t, n, e, i, a) {
  ...
  var l = Math.min(...s) - 200, c = Math.max(...s) + 200;
...
function gr(t) {
  function n(r, o, s) {
    ...
    y.classList.add("gantt_task_line_actual", "gantt_task_line", "gantt_project"), y.style.cssText = ["position:absolute", "left:" + w.left + "px", "top:16px", "height:8px", "width:" + w.width + "px"].join(";"), b.appendChild(y), c.show_progress && d != c.types.milestone && function(T, E, C, D, M) {
      var I = 1 * T.progress || 0;
      C = Math.max(C - 2, 0);
      var A = document.createElement("div"), L = Math.round(C * I);
      L = Math.min(C, L), T.progressColor && (A.style.backgroundColor = T.progressColor, A.style.opacity = 1), A.style.width = L + "px", A.className = "gantt_task_progress", A.innerHTML = M.progress_text(T.start_date, T.end_date, T), D.rtl && (A.style.position = "absolute", A.style.right = "0px");
      var P = document.createElement("div");
      if (P.className = "gantt_task_progress_wrapper", P.appendChild(A), E.appendChild(P), t.config.drag_progress && !t.isReadonly(T)) {
        var N = document.createElement("div"), z = L;
        D.rtl && (z = C - L), N.style.left = z + "px", N.className = "gantt_task_progress_drag", A.appendChild(N), E.appendChild(N);
      }
    }(r, y, g, c, u);
    ...
    $ && m.appendChild($), $ = function(T, E, C, D) {
      var M = "gantt_right " + i(!!E.rtl, T), I = null;
      return D && (I = { type: "marginLeft", value: D }), e(T, C.rightside_text, M, I);
    }(r, c, u, _), $ && m.appendChild($), t._waiAria.setTaskBarAttr(r, b);
    var S = t.getState();
...
function mr(t) {
  var n = { current_pos: null, dirs: { left: "left", right: "right", up: "up", down: "down" }, path: [], clear: function() {
    ...
    var s, l = r.$getConfig().link_wrapper_width, c = a.y - l / 2;
    switch (a.direction) {
      case this.dirs.left:
        s = { top: c, height: l, lineHeight: l, left: a.x - a.size - l / 2, width: a.size + l };
        break;
      case this.dirs.right:
        s = { top: c, lineHeight: l, height: l, left: a.x - l / 2, width: a.size + l };
        break;
      case this.dirs.up:
        s = { top: c - a.size, lineHeight: a.size + l, height: a.size + l, left: a.x - l / 2, width: l };
        break;
      case this.dirs.down:
        s = { top: c, lineHeight: a.size + l, height: a.size + l, left: a.x - l / 2, width: l };
    }
    return s;
  }, get_line_sizes: function(a, r) {
    var o, s = r.$getConfig(), l = s.link_line_width, c = s.link_wrapper_width, u = a.size + l;
    switch (a.direction) {
      case this.dirs.left:
      case this.dirs.right:
        o = { height: l, width: u, marginTop: (c - l) / 2, marginLeft: (c - l) / 2 };
        break;
      case this.dirs.up:
      case this.dirs.down:
        o = { height: u, width: l, marginTop: (c - l) / 2, marginLeft: (c - l) / 2 };
    }
    return o;
  }, render_line: function(a, r, o, s) {
    var l = this.get_wrapper_sizes(a, o, s), c = document.createElement("div");
    c.style.cssText = ["top:" + l.top + "px", "left:" + l.left + "px", "height:" + l.height + "px", "width:" + l.width + "px"].join(";"), c.className = "gantt_line_wrapper";
    var u = this.get_line_sizes(a, o), d = document.createElement("div");
    return d.style.cssText = ["height:" + u.height + "px", "width:" + u.width + "px", "margin-top:" + u.marginTop + "px", "margin-left:" + u.marginLeft + "px"].join(";"), d.className = "gantt_link_line_" + a.direction, c.appendChild(d), c;
  }, render_corner: function(a, r) {
    const o = a.radius, s = r.$getConfig(), l = s.link_line_width || 2, c = document.createElement("div");
    let u, d;
    return c.classList.add("gantt_link_corner"), c.classList.add(`gantt_link_corner_${a.direction.from}_${a.direction.to}`), c.style.width = `${o}px`, c.style.height = `${o}px`, a.direction.from === "right" && a.direction.to === "down" ? (u = "Right", d = "Top", c.style.left = a.x - s.link_line_width / 2 + "px", c.style.top = `${a.y}px`) : a.direction.from === "down" && a.direction.to === "right" ? (u = "Left", d = "Bottom", c.style.left = a.x - s.link_line_width / 2 + "px", c.style.top = `${a.y}px`) : a.direction.from === "right" && a.direction.to === "up" ? (u = "Right", d = "Bottom", c.style.left = a.x - s.link_line_width / 2 + "px", c.style.top = a.y - o + "px") : a.direction.from === "up" && a.direction.to === "right" ? (u = "Left", d = "Top", c.style.left = a.x - s.link_line_width / 2 + "px", c.style.top = a.y - o + "px") : a.direction.from === "left" && a.direction.to === "down" ? (u = "Left", d = "Top", c.style.left = a.x - o - s.link_line_width / 2 + "px", c.style.top = `${a.y}px`) : a.direction.from === "down" && a.direction.to === "left" ? (u = "Right", d = "Bottom", c.style.left = a.x - o - s.link_line_width / 2 + "px", c.style.top = `${a.y}px`) : a.direction.from === "left" && a.direction.to === "up" ? (u = "Left", d = "Bottom", c.style.left = a.x - o - s.link_line_width / 2 + "px", c.style.top = a.y - o + "px") : a.direction.from === "up" && a.direction.to === "left" && (u = "Right", d = "Top", c.style.left = a.x - o - s.link_line_width / 2 + "px", c.style.top = a.y - o + "px"), c.style[`border${d}Width`] = `${l}px`, c.style[`border${u}Width`] = `${l}px`, c.style[`border${u}Style`] = "solid", c.style[`border${d}Style`] = "solid", c.style[`border${d}${u}Radius`] = `${o}px`, c;
  }, render_arrow(a, r) {
    var o = document.createElement("div"), s = a.y, l = a.x, c = r.link_arrow_size;
    o.style.setProperty("--dhx-gantt-icon-size", `${c}px`);
    ...
    var l = this.get_endpoint(a, r, o, s), c = t.config, u = l.e_y - l.y, d = l.e_x - l.x, h = n.dirs, _ = r.getItemHeight(a.source);
    this.clear(), this.point({ x: l.x, y: l.y });
    var f = 2 * c.link_arrow_size, k = this.get_line_type(a, r.$getConfig()), v = l.e_x > l.x;
    if (k.from_start && k.to_start) this.point_to(h.left, f), v ? (this.point_to(h.down, u), this.point_to(h.right, d)) : (this.point_to(h.right, d), this.point_to(h.down, u)), this.point_to(h.right, f);
    else if (!k.from_start && k.to_start) if (u !== 0 && (v = l.e_x > l.x + 2 * f), this.point_to(h.right, f), v) d -= f, this.point_to(h.down, u), this.point_to(h.right, d);
    else {
      d -= 2 * f;
      var b = u > 0 ? 1 : -1;
      this.point_to(h.down, b * (_ / 2)), this.point_to(h.right, d), this.point_to(h.down, b * (Math.abs(u) - _ / 2)), this.point_to(h.right, f);
    }
    else k.from_start || k.to_start ? k.from_start && !k.to_start && (u !== 0 && (v = l.e_x > l.x - 2 * f), this.point_to(h.left, f), v ? (d += 2 * f, b = u > 0 ? 1 : -1, this.point_to(h.down, b * (_ / 2)), this.point_to(h.right, d), this.point_to(h.down, b * (Math.abs(u) - _ / 2)), this.point_to(h.left, f)) : (d += f, this.point_to(h.down, u), this.point_to(h.right, d))) : (this.point_to(h.right, f), v ? (this.point_to(h.right, d), this.point_to(h.down, u)) : (this.point_to(h.down, u), this.point_to(h.right, d)), this.point_to(h.left, f));
    return this.path;
    ...
    var l = r.$getConfig(), c = this.get_line_type(a, l), u = c.from_start, d = c.to_start, h = i(o, r, l), _ = i(s, r, l);
    return { x: u ? h.left : h.left + h.width, e_x: d ? _.left : _.left + _.width, y: h.top + h.rowHeight / 2 - 1, e_y: _.top + _.rowHeight / 2 - 1 };
  } };
  function i(a, r, o) {
    var s = r.getItemPosition(a);
    let l = ne(t, r, a), c = l.maxHeight, u = l.splitChild;
    const d = t.config.baselines && (t.config.baselines.render_mode == "separateRow" || t.config.baselines.render_mode == "individualRow") && a.baselines && a.baselines.length;
    let h;
    l.shrinkHeight && (s.rowHeight = c);
    let _ = t.getTaskType(a.type) == o.types.milestone;
    if (_) {
      let f = r.getBarHeight(a.id, !0);
      h = Math.sqrt(2 * f * f), l.shrinkHeight && c < f && (f = c, h = c), s.left -= h / 2, s.width = h;
    }
    if (u) if (c >= s.height) {
      const f = Ft(t, a.parent);
      d || f ? _ ? (s.rowHeight = s.height + 4, s.left += (s.width - s.rowHeight + 4) / 2, s.width = s.rowHeight - 3) : s.rowHeight = s.height + 6 : _ && (s.left += (h - s.height) / 2);
    } else s.rowHeight = c + 2, _ && (s.left += (s.width - s.rowHeight + 4) / 2, s.width = s.rowHeight - 3);
    else d && (s.rowHeight = s.height + 4);
    return s;
  }
  return { render: function(a, r, o) {
    ...
    var c = e.get_endpoint(a, r, s, l), u = c.e_y - c.y;
    if (!(c.e_x - c.x) && !u) return null;
    var d = e.get_points(a, r, s, l);
    const h = function(v, b) {
      ...
      return p;
    }(n.get_lines(d, r).filter((v) => v.size > 0), o), _ = function(v, b, g, m) {
  ...
  }, update: null, isInViewPort: Xn, getVisibleRange: Kn() };
}
...
function ti(t) {
  ...
  return t.$data.tasksStore.attachEvent("onStoreUpdated", function() {
    ...
    let s;
    return s = n[o] ? n[o] : n[o] = function(l, c, u, d) {
      let h, _ = !1, f = {};
      t.config.process_resource_assignments && c === t.config.resource_property ? (h = l.$role == "task" ? t.getResourceAssignments(l.$resource_id, l.$task_id) : t.getResourceAssignments(l.id), _ = !0) : h = l.$role == "task" ? [] : t.getTaskBy(c, l.id), f = function($, S, T) {
        ...
        for (let I = 0; I < $.length; I++) {
          const A = $[I];
          let L = A;
          if (T && (L = t.getTask(A.task_id)), L.unscheduled) continue;
          let P = A.start_date || L.start_date, N = A.end_date || L.end_date;
          T && (A.start_date && (P = new Date(Math.max(A.start_date.valueOf(), L.start_date.valueOf()))), A.end_date && (N = new Date(Math.min(A.end_date.valueOf(), L.end_date.valueOf()))), A.mode && A.mode == "fixedDates" && (P = A.start_date, N = A.end_date));
          let z = Bt(S.trace_x, P.valueOf()), O = new Date(S.trace_x[z] || t.date[E + "_start"](new Date(P))), H = new Date(Math.min(P.valueOf(), O.valueOf())), F = t.config.work_time ? t.getTaskCalendar(L) : t;
          for (M[F.id] = {}; H < N; ) {
            const V = M[F.id], j = H.valueOf();
            H = t.date.add(H, C, E), V[j] !== !1 && (D[j] || (D[j] = { tasks: [], assignments: [] }), D[j].tasks.push(L), T && D[j].assignments.push(A));
          }
        }
        return D;
      }(h, u, _);
...
function br(t) {
  ...
  function r(l, c) {
    var u = e[l];
    u && u[c] && u[c].parentNode && u[c].parentNode.removeChild(u[c]);
  }
  function o(l, c, u, d, h, _, f) {
    var k = a[l.id];
    k && k.parentNode && k.parentNode.removeChild(k);
    var v = function(b, g, m, p) {
      for (var y = g.getScale(), w = document.createElement("div"), x = Ct(y, p), $ = x.start; $ <= x.end; $++) {
        var S = y.trace_x[$], T = y.trace_x[$ + 1] || t.date.add(S, y.step, y.unit), E = y.trace_x[$].valueOf(), C = Math.min(b[E] / m, 1) || 0;
        if (C < 0) return null;
        var D = Math.min(b[T.valueOf()] / m, 1) || 0, M = kr(C, S, T, g);
        M && w.appendChild(M);
        var I = yr(C, D, g.posFromDate(T));
        I && w.appendChild(I);
      }
      return w;
    }(u, h, _, f);
  ...
  function s(l, c, u, d, h, _, f) {
    ...
    if (k || v) {
      ...
      return m.className = ["gantt_histogram_cell", k].join(" "), m.setAttribute(f.$config.item_attribute, l.id), m.style.cssText = ["left:" + c.left + "px", "width:" + c.width + "px", "height:" + g + "px", "line-height:" + g + "px", "top:" + (c.top + 1) + "px"].join(";"), v && (v = "<div class='gantt_histogram_label'>" + v + "</div>"), b && (v = "<div class='gantt_histogram_fill' style='height:" + 100 * Math.min(b / u || 0, 1) + "%;'></div>" + v), v && (m.innerHTML = v), m;
    }
    return null;
  }
  return { render: function(l, c, u, d) {
    var h = c.$getTemplates(), _ = c.getScale(), f = n(l, u.resource_property, _, c), k = [], v = {}, b = l.capacity || c.$config.capacity || 24;
    e[l.id] = {}, i[l.id] = null, a[l.id] = null;
    for (var g = !!d, m = Ct(_, d), p = m.start; p <= m.end; p++) {
      var y = f[p];
      if (y && (!g || Vt(p, _, d, t))) {
        var w = h.histogram_cell_capacity(y.start_date, y.end_date, l, y.tasks, y.assignments);
        v[y.start_date.valueOf()] = w || 0;
        var x = c.getItemPosition(l, y.start_date, y.end_date), $ = s(l, x, b, 0, h, y, c);
        $ && (k.push($), e[l.id][p] = $);
      }
    }
    var S = null;
    if (k.length) {
      ...
      var E = o(l, x, v, 0, c, b, d);
      E && (S.appendChild(E), a[l.id] = E), i[l.id] = S;
    }
    return S;
  }, update: function(l, c, u, d, h) {
    var _ = u.$getTemplates(), f = u.getScale(), k = n(l, d.resource_property, f, u), v = l.capacity || u.$config.capacity || 24, b = {}, g = !!h, m = Ct(f, h), p = {};
...
function Xe(t, n, e, i, a, r) {
  const o = { id: t.id, parent: t.id };
  function s(u) {
    if (!(u[r.start_date] && u[r.end_date])) return !1;
    for (let d = 0; d < r.length; d++) if (!u[r[d]]) return !1;
    return !0;
  }
  const l = s(t);
  let c = !1;
  return l && (o.start_date = t[r.start_date], o.end_date = t[r.end_date]), t.render == "split" && a.eachTask(function(u) {
    s(u) && (c = !0, o.start_date = o.start_date || u[r.start_date], o.end_date = o.end_date || u[r.end_date], o.start_date < u[r.start_date] && (o.start_date = u[r.start_date]), o.end_date > u[r.end_date] && (o.end_date = u[r.end_date]));
  }), !(!l && !c) && It(o, n, e);
}
function $r(t, n, e, i, a) {
  return a.config.auto_scheduling && a.config.auto_scheduling.show_constraints !== !1 ? Xe(t, n, e, 0, a, { start_date: "constraint_date", end_date: "constraint_date", additional_properties: ["constraint_type"] }) : !1;
}
function xr(t) {
  const n = {};
  ...
  function e(i, a, r) {
    ...
    let { height: c, marginTop: u } = Vn(t, a, l, 30, i, r), d = c, h = 0;
    switch (o) {
      case "snet":
      case "fnet":
      case "mso":
        h = t.config.rtl ? 1 : -d - 1;
        break;
      case "snlt":
      case "fnlt":
      case "mfo":
        h = t.config.rtl ? -d - 1 : 1;
    }
    switch (i.type === t.config.types.milestone && (u -= 1), s.style.height = c + "px", s.style.width = d + "px", s.style.left = l.left + "px", s.style.top = l.top + "px", s.style.marginLeft = h + "px", s.style.marginTop = u + "px", s.className = "gantt_constraint_marker gantt_constraint_marker_" + o, o) {
      case "snet":
      case "snlt":
      case "fnet":
      case "fnlt":
...
function wr(t, n, e, i, a) {
  return Xe(t, n, e, 0, a, { start_date: "deadline", end_date: "deadline" });
}
...
function bn(t, n, e, i, a, r) {
  for (var o = 0; o < t.baselines.length; o++)
    if (Xe({ id: t.id, parent: t.parent, start_date: t.baselines[o].start_date, end_date: t.baselines[o].end_date }, n, e, 0, a, r)) return !0;
}
const Tr = { init: function(t, n) {
  ...
  if (n.$config.bind && t.getDatastore(n.$config.bind)) {
    ...
    t.defined(n.$getConfig().dnd_sensitivity) && (i.config.sensitivity = n.$getConfig().dnd_sensitivity), i.attachEvent("onBeforeDragStart", t.bind(function(s, l) {
      ...
      var u = c.getAttribute(n.$config.item_attribute);
      if (o(u)) return !1;
      var d = r().getItem(u);
      return !t.isReadonly(d) && (i.config.initial_open_state = d.$open, !!t.callEvent("onRowDragStart", [u, l.target || l.srcElement, l]) && void 0);
      ...
      u && (u.style.position = "static"), i.config.id = c.getAttribute(n.$config.item_attribute);
      var d = r(), h = d.getItem(i.config.id);
      i.config.index = d.getBranchIndex(i.config.id), i.config.parent = h.parent, h.$open = !1, h.$transparent = !0, this.refreshData();
    }, t)), i.lastTaskOfLevel = function(s) {
      for (var l = null, c = r().getItems(), u = 0, d = c.length; u < d; u++) c[u].$level == s && (l = c[u]);
      return l ? l.id : null;
    }, i._getGridPos = t.bind(function(s) {
      var l = K(n.$grid_data), c = l.x + n.$grid.scrollLeft, u = s.pos.y - 10, d = n.getItemHeight(i.config.id);
      u < l.y && (u = l.y);
      var h = n.getTotalHeight();
      u > l.y + h - d && (u = l.y + h - d);
      const _ = l.y + l.height;
      return u > _ - d && (u = _ - d), l.x = c, l.y = u, l;
    }, t), i._getTargetY = t.bind(function(s) {
      var l = K(n.$grid_data), c = n.$state.scrollTop || 0, u = t.$grid_data.getBoundingClientRect().height + c, d = s.pageY - l.y + c;
      return d > u ? d = u : d < c && (d = c), d;
    }, t), i._getTaskByY = t.bind(function(s, l) {
      var c = r();
      s = s || 0;
      var u = n.getItemIndexByTopPosition(s);
      return (u = l < u ? u - 1 : u) > c.countVisible() - 1 ? null : c.getIdByIndex(u);
    }, t), i.attachEvent("onDragMove", t.bind(function(s, l) {
      var c = t.$grid_data.getBoundingClientRect(), u = c.height + c.y + (n.$state.scrollTop || 0) + window.scrollY, d = i.config, h = i._getGridPos(l);
      t._waiAria.reorderMarkerAttr(d.marker);
      var _ = n.$getConfig(), f = r();
      h.y < u ? d.marker.style.top = h.y + "px" : d.marker.style.top = u + "px", d.marker.style.left = h.x + 10 + "px";
      const k = K(t.$root);
      h.width > k.width && (d.marker.style.width = k.width - 10 - 2 + "px", d.marker.style.overflow = "hidden");
      var v = f.getItem(i.config.id), b = i._getTargetY(l), g = i._getTaskByY(b, f.getIndexById(v.id));
      function m(D, M) {
        return !f.isChildOf(p.id, M.id) && (D.$level == M.$level || _.order_branch_free);
      }
...
function xn(t, n, e, i, a, r) {
  ...
  if (n !== a.$getRootId()) {
    ...
    if (l === r) {
      ...
      e < 0.5 && !c ? o = nt.prevSiblingTarget(t, n, a) : (e < 0.5 && (n = c), o = nt.nextSiblingTarget(t, n, a));
    } else if (l > r) a.eachParent(function(f) {
      a.calculateItemLevel(f) === r && (n = f.id);
    }, s), o = $e(t, n, r, a);
    else {
      var u = $e(t, n, r, a), d = $n(t, n, r, a);
      o = e < 0.5 ? u : d;
    }
  } else {
    var h = a.$getRootId(), _ = a.getChildren(h);
    o = nt.createDropTargetObject(), o = _.length && i >= 0 ? $e(t, function(f) {
...
const Sn = { removeLineHighlight: function(t) {
  ...
  var i = function(r, o) {
    var s = K(o.$grid_data), l = ct(r, o.$grid_data), c = s.x + o.$grid.scrollLeft, u = l.y - 10, d = o.getItemHeight(r.targetId);
    u < s.y && (u = s.y);
    var h = o.getTotalHeight();
    return u > s.y + h - d && (u = s.y + h - d), s.x = c, s.y = u, s.width = xe(o.$gantt, s.width, 9), s;
  }(t, e);
  ...
  a || ((a = document.createElement("div")).className = "gantt_drag_marker gantt_grid_dnd_marker", a.innerHTML = "<div class='gantt_grid_dnd_marker_line'></div>", a.style.pointerEvents = "none"), t.child ? function(r, o, s) {
    var l = r.targetParent, c = wn({ x: 0, y: s.getItemTop(l) }, s), u = s.$grid_data.getBoundingClientRect().bottom + window.scrollY;
    let d = xe(s.$gantt, s.$grid_data.offsetWidth);
    o.innerHTML = "<div class='gantt_grid_dnd_marker_folder'></div>", o.style.width = d + "px", o.style.top = c.y + "px", o.style.left = c.x + "px", o.style.height = s.getItemHeight(l) + "px", c.y > u && (o.style.top = u + "px");
  }(t, a, e) : function(r, o, s) {
    var l = function(d, h) {
      var _ = h.$config.rowStore, f = { x: 0, y: 0 }, k = h.$grid_data.querySelector(".gantt_tree_indent"), v = 15, b = 0;
      k && (v = k.offsetWidth);
      var g = 40;
      if (d.targetId !== _.$getRootId()) {
        var m = h.getItemTop(d.targetId), p = h.getItemHeight(d.targetId);
        if (b = _.exists(d.targetId) ? _.calculateItemLevel(_.getItem(d.targetId)) : 0, d.prevSibling) f.y = m;
        else if (d.nextSibling) {
          var y = 0;
          _.eachItem(function(w) {
            _.getIndexById(w.id) !== -1 && y++;
          }, d.targetId), f.y = m + p + y * p;
        } else f.y = m + p, b += 1;
      }
      return f.x = g + b * v, f.width = xe(h.$gantt, Math.max(h.$grid_data.offsetWidth - f.x, 0), f.x), wn(f, h);
    }(r, s), c = s.$grid_data.getBoundingClientRect().bottom + window.scrollY;
    o.innerHTML = "<div class='gantt_grid_dnd_marker_line'></div>", o.style.left = l.x + "px", o.style.height = "4px";
    var u = l.y - 2;
    o.style.top = u + "px", o.style.width = l.width + "px", u > c && (o.style.top = c + "px");
  ...
  if (n.$config.bind && t.getDatastore(n.$config.bind)) {
    ...
    t.defined(n.$getConfig().dnd_sensitivity) && (i.config.sensitivity = n.$getConfig().dnd_sensitivity), i.attachEvent("onBeforeDragStart", t.bind(function(s, l) {
      var c = a(l);
      if (!c || (t.hideQuickInfo && t.hideQuickInfo(), dt(l.target, ".gantt_grid_editor_placeholder"))) return !1;
      var u = c.getAttribute(n.$config.item_attribute), d = n.$config.rowStore.getItem(u);
      return !t.isReadonly(d) && !r(u) && (i.config.initial_open_state = d.$open, !!t.callEvent("onRowDragStart", [u, l.target || l.srcElement, l]) && void 0);
    }, t)), i.attachEvent("onAfterDragStart", t.bind(function(s, l) {
      var c = a(l);
      i.config.marker.innerHTML = c.outerHTML;
      var u = i.config.marker.firstChild;
      u && (i.config.marker.style.opacity = 0.4, u.style.position = "static", u.style.pointerEvents = "none"), i.config.id = c.getAttribute(n.$config.item_attribute);
      var d = n.$config.rowStore, h = d.getItem(i.config.id);
      i.config.level = d.calculateItemLevel(h), i.config.drop_target = nt.createDropTargetObject({ targetParent: d.getParent(h.id), targetIndex: d.getBranchIndex(h.id), targetId: h.id, nextSibling: !0 }), h.$open = !1, h.$transparent = !0, this.refreshData();
    }, t)), i.attachEvent("onDragMove", t.bind(function(s, l) {
      var c = o(l);
      return c && t.callEvent("onBeforeRowDragMove", [i.config.id, c.targetParent, c.targetIndex]) !== !1 || (c = nt.createDropTargetObject(i.config.drop_target)), Sn.highlightPosition(c, i.config, n), i.config.drop_target = c, t._waiAria.reorderMarkerAttr(i.config.marker), this.callEvent("onRowDragMove", [i.config.id, c.targetParent, c.targetIndex]), !0;
    }, t)), i.attachEvent("onDragEnd", t.bind(function() {
      ...
      this.callEvent("onBeforeRowDragEnd", [i.config.id, c.targetParent, c.targetIndex]) === !1 ? l.$drop_target = null : (s.move(i.config.id, c.targetIndex, c.targetParent), t.render(), this.callEvent("onRowDragEnd", [i.config.id, c.targetParent, c.targetIndex])), s.refresh(l.id);
    }, t));
  }
  function a(s) {
    return tt(s, n.$config.item_attribute);
  ...
  function r(s) {
    return Gt(s, t, t.getDatastore(n.$config.bind));
  }
  function o(s) {
    var l, c = function(f) {
    ...
    }(s), u = null, d = n.$config.rowStore, h = !n.$getConfig().order_branch_free, _ = ct(s, n.$grid_data).y;
    return document.doctype || (_ += window.scrollY), c !== d.$getRootId() && (u = (_ - n.getItemTop(c)) / n.getItemHeight(c)), h ? (l = xn(i.config.id, c, u, _, d, i.config.level)) && l.targetParent && r(l.targetParent) && (c = d.getPrevSibling(l.targetParent), l = xn(i.config.id, c, u, _, d, i.config.level)) : l = function(f, k, v, b, g) {
      var m;
      ...
      return m;
    }(i.config.id, c, u, _, d), l;
  }
} };
var Cr = function(t) {
  return { onCreated: function(n) {
    n.$config = R(n.$config, { bind: "task" }), n.$config.id == "grid" && (this.extendGantt(n), t.ext.inlineEditors = t.ext._inlineEditors.createEditors(n), t.ext.inlineEditors.init()), this._mouseDelegates = qe(t);
    ...
    this._mouseDelegates.delegate("click", "gantt_row", e.bind(function(i, a, r) {
      const o = n.$getConfig();
      if (a !== null) {
        const s = this.getTask(a);
        if (o.scroll_on_click) {
          const l = !e._is_icon_open_click(i), c = e.$ui.getView("timeline");
          l && c && this.showDate(s.start_date);
        }
      ...
      if (e.callEvent("onGridHeaderClick", [o, i])) {
        ...
        if (o != "add") {
          if (s.sort && o) {
            for (var l, c = o, u = 0; u < s.columns.length; u++) if (s.columns[u].name == o) {
              l = s.columns[u];
              break;
            }
            if (l && l.sort !== void 0 && l.sort !== !0 && !(c = l.sort)) return;
            var d = this._sort && this._sort.direction && this._sort.name == o ? this._sort.direction : "desc";
            d = d == "desc" ? "asc" : "desc", this._sort = { name: o, direction: d }, this.sort(c, d == "desc");
          }
  ...
  return { extend: function(n) {
    ...
    return t = function(i, a) {
      ...
      return { drag: null, dragMultiple: {}, _events: { before_start: {}, before_finish: {}, after_finish: {} }, _handlers: {}, init: function() {
        ...
        this.set_actions(), r.getService("state").registerProvider("tasksDnd", B(function() {
        ...
        for (var l in this._events) for (var c in o) this._events[l][c] = s[l];
        ...
        var c = i.$getConfig(), u = this._drag_task_coords(o, l);
        ...
        var d = this._calculateMinDuration(c.min_duration, c.duration_unit);
        o.end_date - o.start_date < c.min_duration && (l.left ? o.start_date = a.calculateEndDate(o.end_date, -d, c.duration_unit, o) : o.end_date = a.calculateEndDate(o.start_date, d, c.duration_unit, o)), a._init_task_timing(o);
        ...
        var c = this._drag_task_coords(o, l), u = i.$getConfig().rtl ? c.start - l.pos.x : l.pos.x - c.start, d = Math.max(0, u);
        o.progress = Math.min(1, d / Math.abs(c.end - c.start));
      }, _find_max_shift: function(o, s) {
        var l;
        for (var c in o) {
          var u = o[c], d = a.getTask(u.id);
          if (!d.unscheduled) {
            var h = this._drag_task_coords(d, u), _ = a.posFromDate(new Date(a.getState().min_date)), f = a.posFromDate(new Date(a.getState().max_date));
            if (h.end + s > f) {
              var k = f - h.end;
              (k < l || l === void 0) && (l = k);
            } else if (h.start + s < _) {
        ...
        return l;
      }, _move: function(o, s, l, c) {
        var u = this._drag_task_coords(o, l), d = null, h = null;
        c ? (d = new Date(+l.obj.start_date + c), h = new Date(+l.obj.end_date + c)) : (d = a.dateFromPos(u.start + s), h = a.dateFromPos(u.end + s)), d ? h ? (o.start_date = d, o.end_date = h) : (o.end_date = new Date(a.getState().max_date), o.start_date = a.dateFromPos(a.posFromDate(o.end_date) - (u.end - u.start))) : (o.start_date = new Date(a.getState().min_date), o.end_date = a.dateFromPos(a.posFromDate(o.start_date) + (u.end - u.start)));
        ...
        var l = o.x - s.x, c = o.y - s.y;
        return Math.sqrt(l * l + c * c);
      }, _is_number: function(o) {
        return !isNaN(parseFloat(o)) && isFinite(o);
      }, on_mouse_move: function(o) {
        if (this.drag.start_drag) {
          var s = ct(o, a.$task_data), l = this.drag.start_drag.start_x, c = this.drag.start_drag.start_y;
          (Date.now() - this.drag.timestamp > 50 || this._is_number(l) && this._is_number(c) && this._mouse_position_change({ x: l, y: c }, s) > 20) && this._start_dnd(o);
        }
        if (this.drag.mode) {
          if (!zn(this, 40)) return;
          this._update_on_move(o);
        }
      }, _update_item_on_move: function(o, s, l, c, u, d) {
        var h = a.getTask(s), _ = a.mixin({}, h), f = a.mixin({}, h);
        this._handlers[l].apply(this, [f, o, c, d]), a.mixin(h, f, !0), a.callEvent("onTaskDrag", [h.id, l, f, _, u]), a.mixin(h, f, !0), a.refreshTask(s);
      }, _update_on_move: function(o) {
        var s = this.drag, l = i.$getConfig();
        if (s.mode) {
          var c = ct(o, i.$task_data);
          if (s.pos && s.pos.x == c.x) return;
          s.pos = c;
          var u = a.dateFromPos(c.x);
          if (!u || isNaN(u.getTime())) return;
          var d = c.x - s.start_x, h = a.getTask(s.id);
          if (this._handlers[s.mode]) {
            if (s.mode === l.drag_mode.move) {
              ...
              var v = this._find_max_shift(_, d);
              let m;
              if (v !== void 0 && (d = v), this._update_item_on_move(d, s.id, s.mode, s, o), v === void 0) {
                const p = a.posFromDate(s.obj.start_date), y = a.posFromDate(s.obj.end_date);
                if (s.handle_offset === void 0) {
                  const x = y - p, $ = s.start_x - p;
                  s.handle_offset = $ / x;
                }
                let w = p + Math.abs(y - p) * s.handle_offset;
                m = u - a.dateFromPos(w);
              }
              for (var b in _) {
                var g = _[b];
                f && g.id != s.id && (a._bulk_dnd = !0), this._update_item_on_move(d, g.id, g.mode, g, o, m);
              }
              a._bulk_dnd = !1;
            } else this._update_item_on_move(d, s.id, s.mode, s, o);
            a._update_parents(s.id);
          }
        }
      }, on_mouse_down: function(o, s) {
        if (o.button != 2 || o.button === void 0) {
          var l = i.$getConfig(), c = a.locate(o), u = null;
          if (a.isTaskExists(c) && (u = a.getTask(c)), !a.isReadonly(u) && !this.drag.mode) {
            this.clear_drag_state();
            var d = it(s = s || o.target || o.srcElement), h = this._get_drag_mode(d, s);
            if (!d || !h) return s.parentNode ? this.on_mouse_down(o, s.parentNode) : void 0;
            if (h) if (h.mode && h.mode != l.drag_mode.ignore && l["drag_" + h.mode]) {
              if (c = a.locate(s), u = a.copy(a.getTask(c) || {}), a.isReadonly(u)) return this.clear_drag_state(), !1;
            ...
            else if (a.checkEvent("onMouseDown") && a.callEvent("onMouseDown", [d.split(" ")[0]]) && s.parentNode) return this.on_mouse_down(o, s.parentNode);
          }
        }
      }, _fix_dnd_scale_time: function(o, s) {
        var l = i.$getConfig(), c = a.getScale().unit, u = a.getScale().step;
        function d(h) {
        ...
        l.round_dnd_dates || (c = "minute", u = l.time_step), s.mode == l.drag_mode.resize ? s.left ? (o.start_date = a.roundDate({ date: o.start_date, unit: c, step: u }), d(o)) : (o.end_date = a.roundDate({ date: o.end_date, unit: c, step: u }), function(h) {
          if (a.config.correct_work_time) {
            var _ = i.$getConfig();
        ...
        }(o)) : s.mode == l.drag_mode.move && (o.start_date = a.roundDate({ date: o.start_date, unit: c, step: u }), d(o), o.end_date = a.calculateEndDate(o));
      ...
      }, _finalize_mouse_up: function(o, s, l, c) {
        var u = a.getTask(o);
        if (s.work_time && s.correct_work_time && this._fix_working_times(u, l), this._fix_dnd_scale_time(u, l), this._fireEvent("before_finish", l.mode, [o, l.mode, a.copy(l.obj), c])) {
          var d = o;
          a._init_task_timing(u), this.clear_drag_state(), a.updateTask(u.id), this._fireEvent("after_finish", l.mode, [d, l.mode, c]);
        } else if (this.clear_drag_state(), o == l.id && (l.obj._dhx_changed = !1, a.mixin(u, l.obj, !0)), a.refreshTask(u.id), u.$level > 100) {
          let h = !1;
        ...
        var s = this.drag;
        if (s.mode && s.id) {
          var l = i.$getConfig(), c = a.getTask(s.id), u = this.dragMultiple, d = !1, h = 0;
          s.mode === l.drag_mode.move && (a.isSummaryTask(c) && l.drag_project || this._isMultiselect()) && (d = !0, h = Object.keys(u).length);
          var _ = function() {
            if (d) for (var f in u) u[f].id != s.id && this._finalize_mouse_up(u[f].id, l, u[f], o);
            this._finalize_mouse_up(s.id, l, s, o);
          };
          d && h > 10 ? a.batchUpdate((function() {
            _.call(this);
        ...
        delete s.start_drag;
        var l = i.$getConfig(), c = s.id;
        if (l["drag_" + s.mode] && a.callEvent("onBeforeDrag", [c, s.mode, o]) && this._fireEvent("before_start", s.mode, [c, s.mode, o])) {
          delete s.start_drag;
          var u = a.getTask(c);
          if (a.isReadonly(u)) return void this.clear_drag_state();
          if (this._isMultiselect()) {
            var d = a.getSelectedTasks();
            d.indexOf(s.id) >= 0 && ht(d, a.bind(function(h) {
              var _ = a.getTask(h);
        ...
        var c = this._events[o][s];
        return a.assert(c, "Unknown after drop mode:{" + s + "}"), a.assert(l, "Invalid event arguments"), !a.checkEvent(c) || a.callEvent(c, l);
...
var Ar = function(t, n) {
  ...
  var l = n.$services, c = l.getService("state"), u = l.getService("dnd");
  c.registerProvider("linksDnD", s);
  var d = "gantt_link_point", h = "gantt_link_control", _ = new u(t.$task_bars, { sensitivity: 0, updates_per_second: 60, mousemoveContainer: n.$root, selector: "." + d, preventDefault: !0 });
  function f(m, p) {
    var y, w = _.getPosition(m), x = function(M) {
      var I = 0, A = 0;
  ...
  function b(m, p, y, w) {
    ...
    if (T = Math.max(0, T - 3)) {
      ...
      var D = Math.sin(C), M = Math.cos(C), I = Math.round(p), A = Math.round(m), L = ["-webkit-transform: rotate(" + C + "rad)", "-moz-transform: rotate(" + C + "rad)", "-ms-transform: rotate(" + C + "rad)", "-o-transform: rotate(" + C + "rad)", "transform: rotate(" + C + "rad)", "width:" + Math.round(T) + "px"];
      if (window.navigator.userAgent.indexOf("MSIE 8.0") != -1) {
        L.push('-ms-filter: "' + function(z, O) {
          return "progid:DXImageTransform.Microsoft.Matrix(M11 = " + O + ",M12 = -" + z + ",M21 = " + z + ",M22 = " + O + ",SizingMethod = 'auto expand')";
        }(D, M) + '"');
      ...
      L.push("top:" + I + "px"), L.push("left:" + A + "px"), x.style.cssText = L.join(";");
    }
  }
  function g(m, p, y, w) {
  ...
  _.attachEvent("onBeforeDragStart", n.bind(function(m, p) {
    ...
    if (et(At(p), n.$root) || (x = !1, E = null), x && (C = !yt(p, "task_end_date"), x = !!E), i = E, e = x, a = C, x) {
      const M = yt(p, h).querySelector(`.${d}`);
      if (M) {
        const I = Mn(M, t.$task_bg);
...
var Ir = function(t) {
  ...
  return { onCreated: function(e) {
    var i = e.$config;
    i.bind = G(i.bind) ? i.bind : "task", i.bindLinks = G(i.bindLinks) ? i.bindLinks : "link", e._linksDnD = Mr(), e._tasksDnD = Dr.createTaskDND(), e._tasksDnD.extend(e), this._mouseDelegates = qe(t);
    ...
    function i(a, r) {
      if (a && this.callEvent("onLinkDblClick", [a, r])) {
        ...
        window.setTimeout(function() {
          e._delete_link_confirm({ link: o, message: s, title: "", callback: function() {
            e.deleteLink(a);
          } });
    ...
    this._mouseDelegates.delegate("click", "gantt_task_link", e.bind(function(a, r) {
      ...
      var o = ct(a, e.$task_data), s = e.dateFromPos(o.x), l = Math.floor(e.columnIndexByDate(s)), c = e.getScale().trace_x[l];
  ...
  return { getVerticalScrollbar: function() {
        ...
        if (e && !o && r !== !1 && (t.config.grid_width = r), a.$config.hidden = a.$parent.$config.hidden = o, !a.$config.hidden) {
          ...
          if (s[0] && t.config.grid_width < s[0] && (t.config.grid_width = s[0]), s[1] && t.config.grid_width > s[1] && (t.config.grid_width = s[1]), i && t.config.show_chart) {
            if (a.$config.width = t.config.grid_width - 1, !a.$config.scrollable && a.$config.scrollY && t.$root.offsetWidth) {
              var l = a.$gantt.$layout.$container.offsetWidth, c = t.$ui.getView(a.$config.scrollY).$config.width, u = l - (a.$config.width + c) - 4;
    ...
    if (i.resizer) {
      ...
      o.attachEvent("onResizeStart", function(s, l) {
        var c = t.$ui.getView("grid"), u = c ? c.$parent : null;
        if (u) {
          var d = c._getGridWidthLimits();
          c.$config.scrollable || (u.$config.minWidth = d[0]), u.$config.maxWidth = d[1];
        }
        return a = r ? s : l, t.callEvent("onGridResizeStart", [a]);
      }), o.attachEvent("onResize", function(s, l) {
        var c = r ? s : l;
        return t.callEvent("onGridResize", [a, c]);
      }), o.attachEvent("onResizeEnd", function(s, l, c, u) {
        var d = r ? s : l, h = r ? c : u, _ = t.$ui.getView("grid"), f = _ ? _.$parent : null;
        f && (f.$config.minWidth = void 0);
        var k = t.callEvent("onGridResizeEnd", [d, h]);
...
const Nr = { init: function(t) {
  ...
  var e = Wa(t);
  e.registerView("cell", wt), e.registerView("resizer", Ka), e.registerView("scrollbar", Xa), e.registerView("layout", Zn, function(r) {
    (r.$config ? r.$config.id : null) === "main" && n(r, Lr);
  }), e.registerView("viewcell", Ja), e.registerView("multiview", Ya), e.registerView("timeline", Je, function(r) {
  ...
  }), e.registerView("resourceGrid", tr), e.registerView("resourceTimeline", Qn), e.registerView("resourceHistogram", er);
  var i = function(r) {
    var o = Ga(r);
    return { getDataRender: function(s) {
      return r.$services.getService("layer:" + s) || null;
    }, createDataRender: function(s) {
      var l = s.name, c = s.defaultContainer, u = s.defaultContainerSibling, d = o.createGroup(c, u, function(h, _) {
        if (!d.filters) return !0;
        for (var f = 0; f < d.filters.length; f++) if (d.filters[f](h, _) === !1) return !1;
      }, qa);
      ...
      return { addTaskLayer: function(c) {
        const u = Q;
        return typeof c == "function" ? c = { renderer: { render: c, getVisibleRange: u } } : c.renderer && !c.renderer.getVisibleRange && (c.renderer.getVisibleRange = u), c.view = "timeline", s.addLayer(c);
        ...
        const u = Kn();
        return typeof c == "function" ? c = { renderer: { render: c, getVisibleRange: u } } : c.renderer && !c.renderer.getVisibleRange && (c.renderer.getVisibleRange = u), c.view = "timeline", c && c.renderer && (c.renderer.getRectangle || c.renderer.isInViewPort || (c.renderer.isInViewPort = Xn)), l.addLayer(c);
  ...
  return t.ext.inlineEditors = a, t.ext._inlineEditors = a, a.init(t), { factory: e, mouseEvents: Ua.init(t), layersApi: i.init(), render: { gridLine: function() {
    return /* @__PURE__ */ function(r) {
      return { render: function(o, s, l, c) {
        for (var u = s.getGridColumns(), d = s.$getTemplates(), h = s.$config.rowStore, _ = [], f = 0; f < u.length; f++) {
          var k, v, b, g = f == u.length - 1, m = u[f];
          m.name == "add" ? (v = "<div " + (S = r._waiAria.gridAddButtonAttrString(m)) + " class='gantt_add'></div>", b = "") : (at(v = m.template ? m.template(o) : o[m.name]) && (v = d.date_grid(v, o, m.name)), v == null && (v = ""), b = v, v = "<div class='gantt_tree_content'>" + v + "</div>");
          var p = "gantt_cell" + (g ? " gantt_last_cell" : ""), y = [];
          if (m.tree) {
            p += " gantt_cell_tree";
            for (var w = 0; w < o.$level; w++) y.push(d.grid_indent(o));
            !h.hasChild(o.id) || r.isSplitTask(o) && !r.config.open_split_tasks ? (y.push(d.grid_blank(o)), y.push(d.grid_file(o))) : (y.push(d.grid_open(o)), y.push(d.grid_folder(o)));
          }
        ...
        if (p += o.$transparent ? " gantt_transparent" : "", p += o.$dataprocessor_class ? " " + o.$dataprocessor_class : "", d.grid_row_class) {
          var T = d.grid_row_class.call(r, o.start_date, o.end_date, o);
          T && (p += " " + T);
        }
      ...
      }, update: null, getRectangle: ie, isInViewPort: vr, getVisibleRange: Q, onrender: function(o, s, l) {
        for (var c = l.getGridColumns(), u = 0; u < c.length; u++) {
          var d = c[u];
          if (d.onrender) {
            var h = s.querySelector(`[data-column-name="${d.name}"]`);
            if (h) {
              var _ = d.onrender(o, h);
    ...
    return /* @__PURE__ */ function(r) {
      ...
      return { render: function(_, f, k, v) {
        ...
        var p, y = document.createElement("div"), w = u(f), x = d(f);
        if (p = v && k.smart_rendering && !Wt(r) ? Ct(g, v.x) : { start: 0, end: m - 1 }, k.show_task_cells) {
          o[_.id] = {}, s[_.id] = {};
        ...
        if (T.isSelected(_.id) && (D += " gantt_selected"), y.className = D, k.smart_rendering ? (y.style.position = "absolute", y.style.top = f.getItemTop(_.id) + "px", y.style.width = "100%") : y.style.position = "relative", y.style.height = f.getItemHeight(_.id) + "px", _.id == "timeline_placeholder_task") {
          var M = 0;
          _.lastTaskId && (M = f.getItemTop(_.lastTaskId) + f.getItemHeight(_.lastTaskId));
          var I = (_.row_height || f.$task_data.offsetHeight) - M;
          I < 0 && (I = 0), k.smart_rendering && (y.style.top = M + "px"), y.style.height = I + "px";
        }
        ...
        var g = k.getScale(), m = g.count, p = u(k), y = d(k);
      ...
      }, getRectangle: He, getVisibleRange: Q, prepareData: pr };
    }(t);
  }, taskBar: function() {
    return function(r) {
    ...
    return function(r) {
      const o = be(r), s = {};
      function l(d, h, _, f, k) {
        let v = !0;
        return f.smart_rendering && (v = It(d, h, _)), v;
      }
      function c(d, h, _, f) {
        const k = r.copy(r.getTask(h.id));
        if (k.$rendered_at = d.id, r.callEvent("onBeforeRollupTaskDisplay", [k.id, k, d.id]) === !1) return;
        const v = o(k, _);
        if (!v) return;
        const b = _.getBarHeight(d.id, h.type == r.config.types.milestone), g = Math.floor((_.getItemHeight(d.id) - b) / 2);
        return v.style.top = f.top + g + "px", v.classList.add("gantt_rollup_child"), v.setAttribute("data-rollup-parent-id", d.id), v;
      }
      function u(d, h) {
        return d + "_" + h;
      }
      return { render: function(d, h, _, f) {
        if (d.rollup !== !1 && d.$rollup && d.$rollup.length) {
          const k = document.createElement("div"), v = r.getTaskPosition(d);
          return f && (f.y = 0, f.y_end = r.$task_bg.scrollHeight), d.$rollup.forEach(function(b) {
            if (!r.isTaskExists(b)) return;
            const g = r.getTask(b);
            if (!l(g, f, h, _)) return;
            const m = c(d, g, h, v);
            m ? (s[u(g.id, d.id)] = m, k.appendChild(m)) : s[u(g.id, d.id)] = !1;
          }), k;
        }
        return !1;
      }, update: function(d, h, _, f, k) {
        const v = document.createElement("div"), b = r.getTaskPosition(d);
        k.y = 0, k.y_end = r.$task_bg.scrollHeight, d.$rollup.forEach(function(g) {
          const m = r.getTask(g), p = u(m.id, d.id);
          let y = l(m, k, _, f);
          if (y !== !!s[p]) if (y) {
            const w = c(d, m, _, b);
            s[p] = w || !1;
    ...
    return /* @__PURE__ */ function(r) {
      function o(s, l, c) {
        const u = document.createElement("div"), d = r.getTaskPosition(s, s.deadline, s.deadline), { height: h, marginTop: _ } = Vn(r, l, d, 20, s, c);
        let f = h;
        return r.config.rtl && (d.left += f), u.style.left = d.left - f + "px", u.style.top = d.top + "px", u.style.marginTop = _ + "px", u.style.width = f + "px", u.style.height = h + "px", u.style.fontSize = h + "px", u.className = "gantt_task_deadline", u.setAttribute("data-task-id", s.id), u;
      }
      return { render: function(s, l, c, u) {
        const d = document.createElement("div");
        if (d.className = "gantt_deadline_nodes", d.setAttribute("data-task-row-id", s.id), s.deadline) {
          const h = o(s, l);
          d.appendChild(h);
        }
        if (pt(s)) {
    ...
    return /* @__PURE__ */ function(r) {
      function o(s, l, c, u, d) {
        ...
        let g = ne(r, u, s, k.rowHeight).maxHeight, m = k.top + 1 + b, p = u.getBarHeight(s.id, s.type);
        ...
        switch (r.config.baselines.render_mode) {
          case "separateRow":
            ...
            break;
          case "individualRow":
            x = y * c, m += k.height + x + (y - w) / 2, p = w;
            break;
          default:
            $ = 1, d ? (g = ne(r, u, s).maxHeight, b ? g >= v ? $ = (g - p) / 2 - 1 - b : (p = _n(g), m = k.top, $ = Math.abs(p - g) / 2, b = 0) : (g > p && ($ = (g - p) / 2 - 1), $ -= b), s.bar_height || ($ -= 1)) : (s.bar_height && k.rowHeight >= s.bar_height && ($ = (k.rowHeight - s.bar_height) / 2 - 1), $ += b, s.bar_height || ($ += 2), f && ($ += 1));
        }
        let S = k.top + g + 1 - b;
        return !(m + p > S && (p -= m + p - S, p <= 0)) && (h.style.left = k.left + "px", h.style.width = k.width + "px", h.style.top = m + "px", h.style.height = Math.floor(p) + "px", $ && (h.style.marginTop = $ + "px"), h.className = `gantt_task_baseline gantt_task_baseline_${c} ${l.className || ""}`, f ? (h.className += "gantt_milestone_baseline", h.style.width = h.style.height = p + "px", h.style.marginLeft = Math.floor(-p / 2) + "px") : h.innerHTML = r.templates.baseline_text(s, l, c), h.setAttribute("data-task-id", s.id), h.setAttribute("data-baseline-id", l.id), h);
      }
      return { render: function(s, l, c, u) {
        if (!r.config.baselines.render_mode) return;
        const d = document.createElement("div");
        return d.className = "gantt_baseline_nodes", d.setAttribute("data-task-row-id", s.id), s.baselines && s.baselines.length && s.baselines.forEach(function(h, _) {
          const f = o(s, h, _, l);
          f && d.appendChild(f);
        ...
        }, s.id), d.childNodes.length ? d : void 0;
    ...
    return function(r) {
      var o = ti(r), s = {};
      function l(u, d, h, _, f) {
        var k = h.resource_cell_class(d.start_date, d.end_date, u, d.tasks, d.assignments), v = h.resource_cell_value(d.start_date, d.end_date, u, d.tasks, d.assignments), b = f.getItemHeight(u.id) - 1;
        if (k || v) {
          var g = f.getItemPosition(u, d.start_date, d.end_date), m = document.createElement("div");
      ...
      function c(u, d) {
        s[u] && s[u][d] && s[u][d].parentNode && s[u][d].parentNode.removeChild(s[u][d]);
      }
      return { render: function(u, d, h, _) {
        var f = d.$getTemplates(), k = d.getScale(), v = o(u, h.resource_property, d.getScale(), d), b = !!_, g = [];
        s[u.id] = {};
        for (var m = Ct(k, _), p = m.start; p <= m.end; p++) {
          var y = v[p];
          if (y && (!b || Vt(p, k, _, r))) {
            var w = l(u, y, f, 0, d);
        ...
        return x;
      }, update: function(u, d, h, _, f) {
        var k = h.$getTemplates(), v = h.getScale(), b = o(u, _.resource_property, h.getScale(), h), g = Ct(v, f), m = {};
        if (s && s[u.id]) for (var p in s[u.id]) m[p] = p;
        for (var y = g.start; y <= g.end; y++) {
          var w = b[y];
          if (m[y] = !1, w) if (Vt(y, v, f, r)) if (s[u.id] && s[u.id][y]) s[u.id] && s[u.id][y] && !s[u.id][y].parentNode && d.appendChild(s[u.id][y]);
          else {
            var x = l(u, w, k, 0, h);
            x && (d.appendChild(x), s[u.id][y] = x);
          }
          else c(u.id, y);
        }
        for (var p in m) m[p] !== !1 && c(u.id, p);
      }, getRectangle: He, getVisibleRange: Q };
    }(t);
    ...
    return /* @__PURE__ */ function(r) {
      return { render: function(o, s, l) {
        var c = s.$getConfig(), u = document.createElement("div");
        return u.className = "gantt_task_grid_row_resize_wrap", u.style.top = s.getItemTop(o.id) + s.getItemHeight(o.id) + "px", u.innerHTML = "<div class='gantt_task_grid_row_resize' role='cell'></div>", u.setAttribute(c.task_grid_row_resizer_attribute, o.id), r._waiAria.rowResizerAttr(u), u;
...
function we(t, n) {
  ...
  for (u[1] && !n.defined(u[1].width) && (u[1].width = c._second_column_width), u[2] && !n.defined(u[2].width) && (u[2].width = c._third_column_width), s = 0; s < u.length; s++) {
    var d = u[s];
    d.name == "add" && (d.width || (d.width = 44), n.defined(d.min_width) && n.defined(d.max_width) || (d.min_width = d.min_width || d.width, d.max_width = d.max_width || d.width), d.min_width && (d.min_width = +d.min_width), d.max_width && (d.max_width = +d.max_width), d.width && (d.width = +d.width, d.width = d.min_width && d.min_width > d.width ? d.min_width : d.width, d.width = d.max_width && d.max_width < d.width ? d.max_width : d.width));
  }
...
function Pr(t) {
  ...
  function s() {
    var _ = !!document.querySelector(".gantt_drag_marker"), f = !!document.querySelector(".gantt_drag_marker.gantt_grid_resize_area") || !!document.querySelector(".gantt_drag_marker.gantt_row_grid_resize_area"), k = !!document.querySelector(".gantt_link_direction"), v = t.getState(), b = v.autoscroll;
    return e = _ && !f && !k, !(!v.drag_mode && !_ || f) || b;
  }
  function l(_) {
    if (i && (clearTimeout(i), i = null), _) {
      var f = t.config.autoscroll_speed;
      f && f < 10 && (f = 10), i = setTimeout(function() {
        n = setInterval(d, f || 50);
  ...
  function d() {
    if (!s()) return c(!1), !1;
    var _ = o(t.$task) ? t.$task : o(t.$grid) ? t.$grid : t.$root;
    if (_) {
      var f = !1;
      ...
      var k = K(_), v = r.x - k.x, b = r.y - k.y + window.scrollY, g = e ? 0 : h(v, k.width, a.x - k.x), m = h(b, k.height, a.y - k.y + window.scrollY), p = t.getScrollState(), y = p.y, w = p.inner_height, x = p.height, $ = p.x, S = p.inner_width, T = p.width;
      (m && !w || m < 0 && !y || m > 0 && y + w >= x + 2) && (m = 0), (g && !S || g < 0 && !$ || g > 0 && $ + S >= T) && (g = 0);
  ...
  function h(_, f, k) {
    return _ - 50 < 0 && _ < k ? -1 : _ > f - 50 && _ > k ? 1 : 0;
  }
  t.attachEvent("onGanttReady", function() {
    if (!J(t)) {
...
var Se, Te;
window.jQuery && (Se = window.jQuery, Te = [], Se.fn.dhx_gantt = function(t) {
...
const Rr = ["ctrlKey", "altKey", "shiftKey", "metaKey"], Hr = [[{ unit: "month", date: "%M", step: 1 }, { unit: "day", date: "%d", step: 1 }], [{ unit: "day", date: "%d %M", step: 1 }], [{ unit: "day", date: "%d %M", step: 1 }, { unit: "hour", date: "%H:00", step: 8 }], [{ unit: "day", date: "%d %M", step: 1 }, { unit: "hour", date: "%H:00", step: 1 }]];
class Or {
  constructor(n) {
    this.zoomIn = () => {
      ...
      a = typeof e.element == "function" ? e.element() : e.element, a && this._domEvents.attach(a, i, this.$gantt.bind(function(r) {
        if (this._useKey && (Rr.indexOf(this._useKey) < 0 || !r[this._useKey]))
          return !1;
...
function Br(t) {
  ...
  t._touch_events = function(r, o, s) {
    var l, c = 0, u = !1, d = !1, h = null, _ = null, f = null, k = [], v = null;
    let b = {};
      ...
      if (!s(p) && u) {
        ...
        if (!t._prevent_touch_scroll) {
          if (w && h) {
            ...
            if (!d && (Math.abs(x) > 5 || Math.abs($) > 5) && (d = !0, c = 0, l = v ? e(v) : t.getScrollState()), d) {
              var S, T = l.x + x, E = l.y + $;
              if (v ? (function(C, D, M) {
    ...
    for (a.push([this.$container, "contextmenu", function(p) {
              ...
              if (T) {
                ...
                for (let C = 0; C < $.length; C++) {
                  ...
                  if (D && D.childNodes && (M = D.querySelector(`[${t.config.task_attribute}="${S.id}"]`)), M) {
                    ...
                    M.parentNode.appendChild(I), t.$task_bars.appendChild(M), M.style.display = "none", k.push(M), M = null;
                  }
                }
              }
            }
      ...
      if (document && document.body && document.body.classList.remove("gantt_touch_active"), !s(p)) {
        ...
        }), t._touch_feedback())), u = d = !1, k = [], f = null, h && c) {
...
function Ot(t, n, e) {
  return n = n || [], "<" + t + ni(e || []) + ">" + (n.innerHTML || "") + "</" + t + ">";
}
function ni(t) {
  var n = "";
...
function zr(t) {
  ...
  function i(a, r) {
    var o = [], s = [];
    r && (o = t.getTaskByTime(), a.allow_root && o.unshift({ id: t.config.root_id, text: a.root_label || "" }), o = function(d, h, _) {
      ...
      for (var k = 0; k < d.length; k++) {
        var v = d[k];
        (v.id == _ || t.isChildOf(v.id, _) || f(v.id, v) === !1) && (d.splice(k, 1), k--);
      }
      return d;
    }(o, a, r), a.sort && o.sort(a.sort));
    for (var l = a.template || t.templates.task_text, c = 0; c < o.length; c++) {
      var u = l.apply(t, [o[c].start_date, o[c].end_date, o[c]]);
      u === void 0 && (u = ""), s.push({ key: o[c].id, label: u });
    }
  ...
  return W(e, n), e.prototype.render = function(a) {
    ...
    r === 0 && (r = "0"), !o.id && t.getState().lightbox && (o.id = t.getLightboxValues().id);
    var l = document.createElement("div");
    l.innerHTML = i(s, o.id);
    var c = l.removeChild(l.firstChild);
    return a.onselect = null, a.parentNode.replaceChild(c, a), t.form_blocks.select.set_value.apply(t, [c, r, o, s]);
  }, e;
}
function jr(t) {
  const n = ft();
  ...
  function a(l, c, u, d) {
    var h, _ = "";
    if (l) return h = [{ key: "data-item-id", value: l.key }, { key: "data-assignment-id", value: d || "" }, { key: "class", value: "gantt_resource_amount_input" }], u && h.push({ key: "disabled", value: "disabled" }), l.options ? _ += ae.getHtmlSelect(l.options, h, c) : (h[h.length] = { key: "value", value: c || "" }, _ += ae.getHtmlInput(h)), _;
  }
  function r(l) {
  ...
  return t.attachEvent("onAfterLightbox", function() {
    ...
    var u = t.locale.labels.resources_filter_placeholder || l.filter_placeholder || "type to filter", d = t.locale.labels.resources_filter_label || "hide empty";
    return c = "<div" + (isNaN(l.height) ? "" : " style='height: " + l.height + "px;'") + " class='gantt_section_" + l.name + "'>", c += "<div class='gantt_cal_ltext gantt_resources_filter'><input type='text' class='gantt_resources_filter_input' placeholder='" + u + "'> <label><input class='switch_unsetted' type='checkbox'><span class='matherial_checkbox_icon'></span>" + d + "</label></div>", c += "<div class='gantt_cal_ltext gantt_resources' data-name='" + l.name + "'></div>", c += "</div>";
  }, i.prototype.set_value = function(l, c, u, d) {
    ...
    (function(k, v) {
      ...
      e.filter[v.id];
    })(l, d), function(k, v, b, g) {
      if (e.eventsInitialized[b.id]) return;
      var m = function(S) {
        var T, E, C, D, M;
        y(b, k);
        var I = s(b);
        M = I.checkbox, D = I.input, C = M.checked, E = D.value.trim(), I.filterApplied = !!E, t.getState().lightbox && (v = t.getLightboxValues()), T = function(L, P, N, z) {
          var O, H;
          if (z) {
            var F = P[L.map_to] || [];
        ...
        var A = v[b.map_to];
        g.form_blocks.resources.set_value(k, A, v, T);
      };
      function p(S) {
        var T, E = S.target;
        if (S.target.type === "checkbox") {
          (T = E.parentNode.querySelector(r())).disabled = !E.checked;
          var C = T.getAttribute("data-item-id"), D = yt(S, "gantt_resource_row"), M = D.querySelector(".gantt_resource_amount_input");
          if (D.setAttribute("data-checked", E.checked), E.checked) {
            ...
            var I = C, A = b.default_value;
            b.options.forEach(function(L) {
              L.key == I && L.default_value && (A = L.default_value);
            }), M && !M.value && A !== void 0 && (M.value = A, y(b, this)), M.select ? M.select() : M.focus && M.focus();
      ...
      function y(S, T) {
        ...
        for (var D = 0; D < C.length; D++) {
          var M = C[D].getAttribute("data-item-id"), I = C[D].getAttribute("data-assignment-id");
          C[D].disabled ? delete e.resourcesValues[S.id][M] : e.resourcesValues[S.id][M] = { value: C[D].value, id: I };
        }
      }
      ...
      var w, x, $;
    }(l, u, d, this), ht(d.options, function(k, v) {
      d.unassigned_value != k.key && (h = function(b, g, m) {
        var p = {};
        if (g) {
          var y;
          Lt(g) ? y = Ee(g, function(w) {
  ...
  var n = function() {
    ...
    return W(p, m), p.prototype.render = function(y) {
      ...
      if (S.auto_end_date) for (var C = function() {
        I = new Date(T[E[2]].value, T[E[1]].value, T[E[0]].value, 0, 0), A = g.calculateEndDate({ start_date: I, duration: 1, task: x }), g.form_blocks._fill_lightbox_select(T, E.size, A, E, S);
      }, D = 0; D < 4; D++) T[D].onchange = C;
      var M = g._resolve_default_mapping($);
      typeof M == "string" && (M = { start_date: M });
      var I = x[M.start_date] || /* @__PURE__ */ new Date(), A = x[M.end_date] || g.calculateEndDate({ start_date: I, duration: 1, task: x });
      ...
      return $ = g.form_blocks.getTimePickerValue(S, x), typeof g._resolve_default_mapping(x) == "string" ? $ : { start_date: $, end_date: function(E, C, D) {
        var M = g.form_blocks.getTimePickerValue(E, x, C.size);
        return M <= D && (x.autofix_end !== !1 || x.single_date) ? g.date.add(D, g._get_timepicker_step(), "minute") : M;
      }(S, T, $) };
    }, p.prototype.focus = function(y) {
    ...
    return W(p, m), p.prototype.render = function(y) {
      ...
      !y._dhx_onchange && $.onchange && (y.onchange = $.onchange, y._dhx_onchange = !0), $.single_value ? S[0].checked = !!w : ht(S, function(T) {
    ...
    return W(p, m), p.prototype.render = function($) {
      var S = "<div class='gantt_time_selects'>" + g.form_blocks.getTimePicker.call(this, $) + "</div>", T = " " + g.locale.labels[g.config.duration_unit + "s"] + " ", E = $.single_date ? " style='display:none'" : "", C = $.readonly ? " disabled='disabled'" : "", D = g._waiAria.lightboxDurationInputAttrString($), M = "gantt_duration_value";
      $.formatter && (T = "", M += " gantt_duration_value_formatted");
      ...
      return $.name !== "time" && (A += " gantt_section_" + $.name), "<div style='padding-top:0px;font-size:inherit;' class='" + A + "'>" + S + " " + I + "</div>";
    }, p.prototype.set_value = function($, S, T, E) {
      var C, D, M, I, A = $.getElementsByTagName("select"), L = $.getElementsByTagName("input"), P = L[1], N = [L[0], L[2]], z = $.getElementsByTagName("span")[0], O = E._time_format_order;
      function H() {
      ...
      N[0].onclick = g.bind(function() {
        ...
        F(1 * g.config.duration_step);
      }, this), A[0].onchange = H, A[1].onchange = H, A[2].onchange = H, A[3] && (A[3].onchange = H), P.onkeydown = g.bind(function(V) {
        var j;
      ...
      }, this), P.onchange = g.bind(H, this), typeof (C = g._resolve_default_mapping(E)) == "string" && (C = { start_date: C }), D = T[C.start_date] || /* @__PURE__ */ new Date(), M = T[C.end_date] || g.calculateEndDate({ start_date: D, duration: 1, task: T }), I = Math.round(T[C.duration]) || g.calculateDuration({ start_date: D, end_date: M, task: T }), I = y(E).format(I), g.form_blocks._fill_lightbox_select(A, 0, D, O, E), P.value = I, H();
    ...
    return W(p, m), p.prototype.render = function(x) {
      ...
      var E = x.querySelector("[data-constraint-type-select] select"), C = x.querySelectorAll("[data-constraint-time-select] select"), D = T._time_format_order, M = g._resolve_default_mapping(T);
    ...
    function w(S, T, E, C) {
      ...
      const I = C.single_date ? " style='display:none'" : "", A = C.readonly ? " disabled='disabled'" : "", L = g._waiAria.lightboxDurationInputAttrString(C), P = g.locale.labels.baselines_remove_button;
      let N = "gantt_duration_value";
      C.formatter && (M = "", N += " gantt_duration_value_formatted");
      const z = "<div class='gantt_duration' " + I + "><div class='gantt_duration_inputs'><input type='button' class='gantt_duration_dec' value='−'" + A + "><input type='text' value='5days' class='" + N + "'" + A + " " + L + "><input type='button' class='gantt_duration_inc' value='+'" + A + "></div><div class='gantt_duration_end_date'>" + M + "<span></span></div></div></div>", O = `<div><div class='baseline_delete_button gantt_custom_button'>${P}</div></div>`, H = document.createElement("div");
      H.className = "gantt_section_time gantt_section_duration", H.setAttribute("data-baseline-id", T.id), H.innerHTML = D + z + O + "<br>", S.appendChild(H);
      ...
      function $t() {
        var vt = x.call(g, H, C), Z = $.call(g, H, C), ii = g.calculateEndDate({ start_date: vt, duration: Z, task: E }), ai = g.templates.task_end_date || g.templates.task_date;
        Ht.innerHTML = ai(ii);
      }
      function qt(vt) {
        var Z = q.value;
        Z = y(C).parse(Z), window.isNaN(Z) && (Z = 0), (Z += vt) < 1 && (Z = 1), q.value = y(C).format(Z), $t();
      }
      ...
      }, g), q.onchange = g.bind($t, g), g._resolve_default_mapping(C), F = T.start_date || /* @__PURE__ */ new Date(), V = T.end_date || g.calculateEndDate({ start_date: F, duration: 1, task: E }), j = g.calculateDuration({ start_date: F, end_date: V, task: E }), j = y(C).format(j), g.form_blocks._fill_lightbox_select(st, 0, F, Tt, C), q.value = j, $t();
    }
    function x(S, T) {
    ...
    return W(p, m), p.prototype.render = function(S) {
      ...
      return S.querySelectorAll("[data-baseline-id]").forEach((D) => {
        ...
        I = A ? g.copy(A) : { id: g.uid(), task_id: T.id, text: "Baseline 1" }, I.start_date = x(D, E), I.duration = $(D, E), I.end_date = g.calculateEndDate({ start_date: I.start_date, duration: I.duration, task: T }), C.push(I);
      }), C;
...
<div class="gantt_cal_ltitle_controls">
  ...
  t._init_lightbox_events = function() {
    ...
    const m = [...this._get_typed_lightbox_config()].sort((p, y) => p.name === "time" ? 1 : y.name === "time" ? -1 : 0);
    for (let p = 0; p < m.length; p++) {
      let y = t._lightbox_root.querySelector("#" + m[p].id);
      y = y && y.nextSibling;
      let w = this.form_blocks[m[p].type];
      if (!w) continue;
      let x = w.get_value.call(this, y, g, m[p]), $ = t._resolve_default_mapping(m[p]);
  ...
  }, template: new n(), textarea: new e(), select: new a(), time: new i(), duration: new s(), parent: new l(), radio: new o(), checkbox: new r(), resources: new c(), constraint: new u(), baselines: new h(), typeselect: new d() }, t._is_lightbox_timepicker = function() {
    for (var g = this._get_typed_lightbox_config(), m = 0; m < g.length; m++) if (g[m].name == "time" && g[m].type == "time") return !0;
    return !1;
  }, t._delete_task_confirm = function({ task: g, message: m, title: p, callback: y, ok: w }) {
    t._simple_confirm(m, p, y, w);
  }, t._delete_link_confirm = function({ link: g, message: m, title: p, callback: y, ok: w }) {
    t._simple_confirm(m, p, y, w);
...
function Wr(t) {
  if (!bt.isNode) {
    ...
    t.$ui = e.factory, t.$ui.layers = e.render, t.$mouseEvents = e.mouseEvents, t.$services.setService("mouseEvents", function() {
      ...
      function i(d) {
        return d.$ui.getView("timeline");
      }
      function a(d) {
        return d.$ui.getView("grid");
      }
      function r(d) {
        var h = i(d);
      ...
      function o(d) {
        ...
        }), (h = _ ? a(d) : r(d)) ? l(d, h, "scrollY") : null;
      }
      function s(d) {
        var h = r(d);
        return h && h.id != "grid" ? l(d, h, "scrollX") : null;
      }
      function l(d, h, _) {
        var f = h.$config[_];
        return d.$ui.getView(f);
      }
      var c = "DEFAULT_VALUE";
      function u(d, h, _, f) {
        var k = d(this);
        return k && k.isVisible() ? k[h].apply(k, _) : f ? f() : c;
      }
      return { getColumnIndex: function(d) {
        var h = u.call(this, a, "getColumnIndex", [d]);
        return h === c ? 0 : h;
      }, dateFromPos: function(d) {
        var h = u.call(this, i, "dateFromPos", Array.prototype.slice.call(arguments));
        return h === c ? this.getState().min_date : h;
      }, posFromDate: function(d) {
        var h = u.call(this, i, "posFromDate", [d]);
        return h === c ? 0 : h;
      }, getRowTop: function(d) {
        var h = this, _ = u.call(h, i, "getRowTop", [d], function() {
          return u.call(h, a, "getRowTop", [d]);
        });
        return _ === c ? 0 : _;
      }, getTaskTop: function(d) {
        var h = this, _ = u.call(h, i, "getItemTop", [d], function() {
          return u.call(h, a, "getItemTop", [d]);
        });
        return _ === c ? 0 : _;
      }, getTaskPosition: function(d, h, _) {
        var f = u.call(this, i, "getItemPosition", [d, h, _]);
        return f === c ? { left: 0, top: this.getTaskTop(d.id), height: this.getTaskBarHeight(d.id), width: 0 } : f;
      }, getTaskBarHeight: function(d, h) {
        var _ = this, f = u.call(_, i, "getBarHeight", [d, h], function() {
          return u.call(_, a, "getItemHeight", [d]);
        });
        return f === c ? 0 : f;
      }, getTaskHeight: function(d) {
        var h = this, _ = u.call(h, i, "getItemHeight", [d], function() {
          return u.call(h, a, "getItemHeight", [d]);
        });
        return _ === c ? 0 : _;
      }, columnIndexByDate: function(d) {
        var h = u.call(this, i, "columnIndexByDate", [d]);
        return h === c ? 0 : h;
      }, roundTaskDates: function() {
        u.call(this, i, "roundTaskDates", []);
      }, getScale: function() {
        var d = u.call(this, i, "getScale", []);
        return d === c ? null : d;
      }, getTaskNode: function(d) {
        var h = i(this);
        if (h && h.isVisible()) {
          var _ = h._taskRenderer.rendered[d];
          if (!_) {
            var f = h.$config.item_attribute;
            _ = h.$task_bars.querySelector("[" + f + "='" + d + "']");
          }
          return _ || null;
        }
        return null;
      }, getLinkNode: function(d) {
        var h = i(this);
        return h.isVisible() ? h._linkRenderer.rendered[d] : null;
      }, scrollTo: function(d, h) {
        var _ = o(this), f = s(this), k = { position: 0 }, v = { position: 0 };
        _ && (v = _.getScrollState()), f && (k = f.getScrollState());
        var b = f && 1 * d == d, g = _ && 1 * h == h;
        ...
        }).bind(this)), f.scroll(d), y && y.forEach((function(T) {
        ...
        var h = this.posFromDate(d), _ = Math.max(h - this.config.task_scroll_offset, 0);
        this.scrollTo(_);
      }, showTask: function(d) {
        var h = this.getTaskPosition(this.getTask(d)), _ = h.left;
        this.config.rtl && (_ = h.left + h.width);
        var f, k = Math.max(_ - this.config.task_scroll_offset, 0), v = this._scroll_state().y;
        f = v ? h.top - (v - this.getTaskBarHeight(d)) / 2 : h.top, this.scrollTo(k, f);
        var b = a(this), g = i(this);
        b && g && b.$config.scrollY != g.$config.scrollY && l(this, b, "scrollY").scrollTo(null, f);
      }, _scroll_state: function() {
        var d = { x: !1, y: !1, x_pos: 0, y_pos: 0, scroll_size: this.config.scroll_size + 1, x_inner: 0, y_inner: 0 }, h = o(this), _ = s(this);
        if (_) {
          var f = _.getScrollState();
          f.visible && (d.x = f.size, d.x_inner = f.scrollSize), d.x_pos = f.position || 0;
        }
        if (h) {
          var k = h.getScrollState();
          k.visible && (d.y = k.size, d.y_inner = k.scrollSize), d.y_pos = k.position || 0;
        }
        return d;
      }, getScrollState: function() {
        var d = this._scroll_state();
        return { x: d.x_pos, y: d.y_pos, inner_width: d.x, inner_height: d.y, width: d.x_inner, height: d.y_inner };
      }, getLayoutView: function(d) {
        return this.$ui.getView(d);
      }, scrollLayoutCell: function(d, h, _) {
        const f = this.$ui.getView(d);
        if (!f) return !1;
        if (h !== null) {
      ...
      i.resetSkin || (i.resetSkin = function() {
        ...
        const s = this.skin !== o;
        this.skin = o, i._addThemeClass(), r(), i.$root && (we(!s, i), this.render());
      };
      let a = null;
      function r() {
        const o = i.$root;
      ...
      i.ext || (i.ext = {});
      for (var a = [Pr, null, null], r = 0; r < a.length; r++) a[r] && a[r](i);
      i.ext.zoom = new Or(i);
    }(t), Br(t), Fr(t), function(i) {
      i._extend_to_optional = function(a) {
        var r = a, o = { render: r.render, focus: r.focus, set_value: function(s, l, c, u) {
          var d = i._resolve_default_mapping(u);
          if (!c[d.start_date] || d.start_date == "start_date" && this._isAllowedUnscheduledTask(c)) {
            o.disable(s, u);
            var h = {};
            for (var _ in d) h[d[_]] = c[_];
            return r.set_value.call(i, s, l, h, u);
          }
          return o.enable(s, u), r.set_value.call(i, s, l, c, u);
        }, get_value: function(s, l, c) {
          return c.disabled ? { start_date: null } : r.get_value.call(i, s, l, c);
        }, update_block: function(s, l) {
          if (i.callEvent("onSectionToggle", [i._lightbox_id, l]), s.style.display = l.disabled ? "none" : "", l.button) {
            var c = s.previousSibling.querySelector(".gantt_custom_button_label"), u = i.locale.labels, d = l.disabled ? u[l.name + "_enable_button"] : u[l.name + "_disable_button"];
            c.innerHTML = d;
          }
        ...
        }, button_click: function(s, l, c, u) {
          if (i.callEvent("onSectionButton", [i._lightbox_id, c]) !== !1) {
            var d = i._get_typed_lightbox_config()[s];
            d.disabled ? o.enable(u, d) : o.disable(u, d);
          }
        } };
        return o;
      ...
      for (var c in i._waiAria = { getAttributeString: function(u) {
        var d = [" "];
        for (var h in u) {
          var _ = l(o(u[h]));
          d.push(h + "='" + _ + "'");
      ...
      }, _taskCommonAttr: function(u, d) {
        u.start_date && u.end_date && (d.setAttribute("aria-label", o(i.templates.tooltip_text(u.start_date, u.end_date, u))), u.$dataprocessor_class && d.setAttribute("aria-busy", !0));
      }, setTaskBarAttr: function(u, d) {
        this._taskCommonAttr(u, d), d.setAttribute("role", "img"), !i.isReadonly(u) && i.config.drag_move && (u.id != i.getState("tasksDnd").drag_id ? d.setAttribute("aria-grabbed", !1) : d.setAttribute("aria-grabbed", !0));
      }, taskRowAttr: function(u, d) {
        this._taskCommonAttr(u, d), !i.isReadonly(u) && i.config.order_branch && d.setAttribute("aria-grabbed", !1), d.setAttribute("role", "row"), d.setAttribute("aria-selected", i.isSelectedTask(u.id) ? "true" : "false"), d.setAttribute("aria-level", u.$level + 1 || 1), i.hasChild(u.id) && d.setAttribute("aria-expanded", u.$open ? "true" : "false");
      }, linkAttr: function(u, d) {
        ...
        d.setAttribute("role", "img"), d.setAttribute("aria-label", o(k));
      ...
      }, lightboxHeader: function(u, d) {
        u.setAttribute("aria-label", d);
      }, lightboxSelectAttrString: function(u) {
        var d = "";
        switch (u) {
          case "%Y":
            d = i.locale.labels.years;
            break;
          case "%m":
            d = i.locale.labels.months;
            break;
          case "%d":
            d = i.locale.labels.days;
            break;
          case "%H:%i":
            d = i.locale.labels.hours + i.locale.labels.minutes;
        }
        ...
        return "role='row'";
      }, gridScaleCellAttrString: function(u, d) {
        var h = "";
        if (u.name == "add") h = this.getAttributeString({ role: "columnheader", "aria-label": i.locale.labels.new_task });
        else {
          var _ = { role: "columnheader", "aria-label": i.config.external_render && i.config.external_render.isElement(d) ? "" : d };
      ...
      }, gridCellAttrString: function(u, d, h) {
        var _ = { role: "gridcell", "aria-label": d };
      ...
      }, messageModalAttr: function(u, d) {
        u.setAttribute("role", "dialog"), d && u.setAttribute("aria-labelledby", d);
      }, quickInfoAttr: function(u) {
        u.setAttribute("role", "dialog");
      }, quickInfoHeaderAttrString: function() {
        return " role='heading' aria-level='1' ";
      }, quickInfoHeader: function(u, d) {
        u.setAttribute("aria-label", d);
        ...
        u.setAttribute("aria-hidden", "true");
      } }, i._waiAria) i._waiAria[c] = /* @__PURE__ */ function(u) {
        return function() {
...
const Be = new class {
  ...
  var n = za(t);
  return n.env.isNode || (Wr(n), function(e) {
...
}, $i), Ur = Be.getGanttInstance();
Y.gantt = Ur, Y.Gantt = Be, Be.plugin((t) => {
  Ye() || setTimeout(() => {
    const n = ["Your evaluation period for dhtmlxGantt has expired.", "Please contact us at <a href='mailto:contact@dhtmlx.com?subject=dhtmlxGantt licensing' target='_blank'>contact@dhtmlx.com</a> or visit", "<a href='https://dhtmlx.com/docs/products/dhtmlxGantt' target='_blank'>dhtmlx.com</a> in order to obtain a license."].join("<br>");
    if (!(typeof 1749883503000 > "u")) {
      var e, i;
      setInterval(() => {
        !t.$destroyed && Date.now() - 1749883503000 > 27648e5 && t.message({ type: "error", text: n, expire: -1, id: "evaluation-warning" });
